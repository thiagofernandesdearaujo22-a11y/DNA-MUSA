[dna_musa_42.html](https://github.com/user-attachments/files/32495308/dna_musa_42.html)
<!DOCTYPE html>
<html lang="pt-BR">
<head>
<meta charset="UTF-8">
...
function gerarDiasInferiores(perfil, numDias, seriesTotais, diasCicloAnterior){
  ...
  // Regra de volume mínimo de posteriores: o volume semanal de posterior precisa ser pelo menos
  // 50% do volume semanal de quadríceps (em séries de verdade, contadas em todos os dias de inferiores
  // já montados) — não um número fixo desconectado do quanto de quadríceps a aluna está recebendo.
  // Roda sempre, mesmo quando Posterior já é ênfase/secundário, como uma trava de segurança real.
  if(dias.length > 0){
    const totaisSemana = {};
    ...
    const minimoNecessario = Math.ceil(seriesQuadriceps * 0.5);

    if(seriesQuadriceps > 0 && seriesPosterior < minimoNecessario){
    ...
    // Regra especial quando Glúteos é a ênfase: tanto quadríceps quanto posteriores precisam somar
    // pelo menos 50% do volume semanal de glúteos, cada um — senão os dias de glúteos ficam
    // desequilibrados. Só entra em ação quando glúteos é a prioridade real da aluna.
    if(perfil.enfase === 'Glúteos'){
      const seriesGluteos = totaisSemana['Glúteos'] || 0;
      const minimoQuadriceps = Math.ceil(seriesGluteos * 0.5);
      const minimoPosteriorGluteo = Math.ceil(seriesGluteos * 0.5);
      const seriesQuadricepsAtual = totaisSemana['Quadríceps'] || 0;
      const seriesPosteriorAtual = totaisSemana['Isquiotibiais'] || 0;

      if(seriesGluteos > 0 && seriesQuadricepsAtual < minimoQuadriceps){
      ...
      if(seriesGluteos > 0 && seriesPosteriorAtual < minimoPosteriorGluteo){
...
const exerciciosBanco = [
  ...
  {nome:'DESENVOLVIMENTO FRONTAL COM MOCHILA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=RcHEOOACjHk', unilateral:false},
  {nome:'FLEXÃO DE COTOVELO MOCHILA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=X42BMTwuPMA', unilateral:false},
  ...
  {nome:'FLEXÃO DE COTOVELO COM MOCHILA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/X42BMTwuPMA', unilateral:false},
  ...
  {nome:'TRÍCEPS BANCO EM CASA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/wkaPvUI9VBg', unilateral:false},
  ...
  {nome:'DESENVOLVIMENTRO FRONTAL COM MOCHILA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/RcHEOOACjHk', unilateral:false},
  ...
  {nome:'REMADA CURVADA COM MOCHILA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/7nlABnGxDp0', unilateral:false},
  ...
  {nome:'REMADA UNILATERAL COM MOCHILA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/i__eZEOAYEE', unilateral:true},
  ...
  {nome:'BURP ADAPTADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/CtnnoxhzD7U', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  ...
  {nome:'LEVANTAMENO TERRA EM CASA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Hgas-jdhnEQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
