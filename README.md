...
<div class="phone" id="phone-container">
  ...
  <div class="screen">
    ...
    <!-- DESAFIO SECA E EMPINA -->
    <div class="view" data-view="desafio-seca-empina">
    ...
    <div class="view" data-view="personal">
      ...
      <div id="personal-comunicacao" style="display:none;">
        <div class="local-back" onclick="showPersonalView('dashboard')"><i class="ti ti-arrow-left"></i><span>Painel</span></div>
        <h1 class="page-title" style="margin-top:0;">Comunicação com as alunas</h1>
        ...
        <p class="section-label" style="margin-top:18px;">Enviar mensagem em massa</p>
        <p class="page-sub" style="margin-top:-6px;">Cada botão abaixo já deixa claro por onde a mensagem sai</p>

        <button class="btn-gold" style="background:#25D366;color:#fff;border:none;margin-bottom:8px;" onclick="mostrarPainelEnvio('whatsapp')"><i class="ti ti-brand-whatsapp" style="margin-right:6px;"></i>Mandar via WhatsApp</button>
        <div id="painel-envio-whatsapp"></div>

        <button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);margin-bottom:14px;margin-top:8px;" onclick="mostrarPainelEnvio('app')"><i class="ti ti-device-mobile" style="margin-right:6px;"></i>Mandar via Aplicativo</button>
        <div id="painel-envio-app"></div>

        <p class="section-label" style="margin-top:18px;">Ferramenta específica</p>
        <button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);margin-bottom:14px;" onclick="mostrarFormularioAvisarTreinoAjustado()"><i class="ti ti-barbell" style="margin-right:6px;"></i>Avisar treino ajustado (WhatsApp + Aplicativo juntos)</button>
      ...
      <div id="personal-inteligencia" style="display:none;">
        <div class="local-back" onclick="showPersonalView('dashboard')"><i class="ti ti-arrow-left"></i><span>Painel</span></div>
        <h1 class="page-title" style="margin-top:0;">Inteligência de mercado</h1>
      ...
      <div id="personal-relatorios" style="display:none;">
        <div class="local-back" onclick="showPersonalView('dashboard')"><i class="ti ti-arrow-left"></i><span>Painel</span></div>
        <h1 class="page-title" style="margin-top:0;">Relatórios da semana</h1>
        <p class="page-sub" style="margin-top:-6px;">Só considera alunas Ativas. Bom pra fechar a semana, toda sexta-feira</p>

        <button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);margin-bottom:8px;" onclick="gerarRelatorioFrequenciaSemanal()"><i class="ti ti-calendar-stats" style="margin-right:6px;"></i>Relatório de frequência de treino</button>
        <div id="relatorio-frequencia-area" style="margin-bottom:14px;"></div>

        <button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);margin-bottom:8px;" onclick="gerarRelatorioRespostas()"><i class="ti ti-message-2" style="margin-right:6px;"></i>Relatório de respostas e comentários</button>
        <div id="relatorio-respostas-area" style="margin-bottom:14px;"></div>

        <button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);margin-bottom:8px;" onclick="gerarResumoFechamentoSemana()"><i class="ti ti-clipboard-text" style="margin-right:6px;"></i>Resumo de fechamento da semana</button>
      ...
      <div id="personal-funil" style="display:none;">
        ...
        <div class="filter-row" style="margin-bottom:14px;">
          ...
          <div class="chip" id="funil-tab-recuperacao" onclick="alternarEstagioFunil('recuperacao')">Remarketing/Recuperação</div>
        </div>
      ...
      <div id="personal-conversas" style="display:none;">
        <div class="local-back" id="conversas-voltar" onclick="showPersonalView('dashboard')"><i class="ti ti-arrow-left"></i><span>Dashboard</span></div>
        <h1 class="page-title" id="conversas-titulo" style="margin-top:0;">Conversas</h1>
        <p class="page-sub" id="conversas-subtitulo" style="margin-top:-6px;">Todo mundo que já trocou mensagem pelo WhatsApp com vocês</p>
        <div id="area-lista-conversas"></div>
        <div id="area-conversa-aberta"></div>
      </div>
      ...
      <div id="personal-desafios" style="display:none;">
        ...
        <p class="section-label" style="margin-top:0;">Seca e Empina — em andamento</p>
...
let ultimoRegistroRapidoEm = 0; // guarda o instante do último "segurar", pra abrirDiaDaSemana saber que não é pra abrir o treino agora
function iniciarPressionarDia(indice){
  timerPressionarDia = setTimeout(function(){
    ultimoRegistroRapidoEm = Date.now(); // marca já aqui, mesmo antes de confirmar — ela claramente quis segurar, não tocar rápido
...
function abrirDiaDaSemana(i){
  if(Date.now() - ultimoRegistroRapidoEm < 1200){ return; } // evita abrir o treino logo depois de um "segurar" que acabou de registrar (janela de tempo, mais confiável que uma bandeira sozinha)
...
const alunasPersonal = [
  ...
  {nome:'Leticia graff', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'06x por semana', email:'', telefone:'00000000000', piramide:'Todos', objetivo:'Emagrecer', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2022-03-14', statusPlanoManual:'vencidas'},
  {nome:'Marelize Geruza dos Santos Brites', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'', telefone:'00000000000', piramide:'1 glúteo, 2 coxas, 3 trícrps, 4 panturrilha', objetivo:'Hipertrofia.', restricoes:'Uma pequena dor no joelho, mas era por fazer o exercicíos errado.', academia:'Atitude fit.', dataAnamnese:'2022-03-14', statusPlanoManual:'vencidas'},
  {nome:'Givanildo de castro Trindade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'', telefone:'00000000000', piramide:'1=costas  2=peito  3=braço  4=pernas', objetivo:'Hipertrofia', restricoes:'Nenhuma relatada', academia:'Daidojuku', dataAnamnese:'2022-03-14', statusPlanoManual:'vencidas'},
  {nome:'Fabiana Peres', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1 bumbum 2 meio da coxa  3 peito 4 costa', objetivo:'Melhorar minha saúde e melhorar o corpo', restricoes:'Bursite dói muito kkkk', academia:'Daido juku', dataAnamnese:'2022-03-14', statusPlanoManual:'vencidas'},
  {nome:'Gabriela Souza Duarte', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'', telefone:'00000000000', piramide:'1:glúteos 2: pernas 3:Costa 4:abdômen', objetivo:'Para me motivar, me manter no foco kkk, me oferecer orientação física, alcança o corpo ideal', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Debora Mann', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'', telefone:'00000000000', piramide:'1Glúteo  2Perna 3Braço 4Abdômen', objetivo:'Emagrecer, tonificar/definir', restricoes:'Nenhuma relatada', academia:'Em Sapucaia', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Patrícia Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Pernas Braços  Glúteos  Abdômen', objetivo:'Definir e ganhar massa', restricoes:'Nenhuma relatada', academia:'Yes Fitness', dataAnamnese:'2022-03-15', statusPlanoManual:'ativas'},
  {nome:'Micheli Araujo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'', telefone:'00000000000', piramide:'1- glúteos  2-posterior 3-quadriceps 4-costas', objetivo:'Emagrecer e definir', restricoes:'Não.', academia:'Performance', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Tanaiane da Silva Cardoso', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Pernas,bumbum,braços e barriga', objetivo:'Conquistar um shape melhor. Não quero crescer,quero definição', restricoes:'Nunca', academia:'Onde eu estiver', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Michele Silva dos Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Posterior,glúteo,costas,coxas', objetivo:'Emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Yes fitnes', dataAnamnese:'2022-03-15', statusPlanoManual:'ativas'},
  {nome:'Jessica Nunes de Lima', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1 pernas 2 glúteos 3 costas', objetivo:'Afinar barriga e definir pernas e glúteos', restricoes:'Nenhuma relatada', academia:'Sublime - Alvorada RS', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Jenniffer Barbosa de Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'', telefone:'00000000000', piramide:'1 glúteo,  2 coxa , 3 barriga', objetivo:'Coxa mais grossa e barriga definida', restricoes:'As vezes  muita dor nos joelhos', academia:'Engenharia do corpo  Alvorada', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Dienifer Silveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1 glúteo  2 posterior  3 parte interna da coxa  4 definir todo o corpo', objetivo:'Focar, definir o corpo com ênfase nos glúteos e posterior, ter um treino personalizado', restricoes:'Nenhuma relatada', academia:'Yes Fitness, a partir de abril performance Fitness', dataAnamnese:'2022-03-15', statusPlanoManual:'vencidas'},
  {nome:'Deivid Rogger Rodrigues Batista', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'', telefone:'00000000000', piramide:'Peito Abdômen  Costa Braço', objetivo:'Ganhar massa muscular e definição', restricoes:'Tornozelo a 10 anos atraz', academia:'Yes fitnes', dataAnamnese:'2022-03-16', statusPlanoManual:'vencidas'},
  {nome:'Gabriela Guimarães Andrade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'', telefone:'00000000000', piramide:'Glúteo, quadríceps, posterior e costas', objetivo:'Realizar os exercícios corretamente com foco no meu objetivo', restricoes:'Nenhuma relatada', academia:'Performance fitness', dataAnamnese:'2022-03-16', statusPlanoManual:'vencidas'},
  {nome:'Carolaine de Melo Borges', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1', objetivo:'Glúteos kkk', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-03-19', statusPlanoManual:'vencidas'},
  {nome:'Paloma Alves de Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1° abdômen, 2° pernas, 3° glúteos, 4°braços', objetivo:'Emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Engenharia do Corpo Alvorada', dataAnamnese:'2022-03-20', statusPlanoManual:'vencidas'},
  {nome:'Andri', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'', telefone:'00000000000', piramide:'Coxas Glúteo  Abdômen  Superiores', objetivo:'Emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Performace', dataAnamnese:'2022-03-21', statusPlanoManual:'vencidas'},
  {nome:'Débora', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'', telefone:'00000000000', piramide:'1 - glúteo  2 - perna 3 - braços  4 - abdômen', objetivo:'Definição', restricoes:'Nenhuma relatada', academia:'Sapucaia', dataAnamnese:'2022-03-22', statusPlanoManual:'vencidas'},
  {nome:'Suane Maciel Vieira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'', telefone:'00000000000', piramide:'1° Quadriceps, 2° glúteos, 3° Abdomen, 4° superiores', objetivo:'Definição', restricoes:'Sim, ombro', academia:'Brothers em São Leopoldo', dataAnamnese:'2022-03-29', statusPlanoManual:'vencidas'},
  {nome:'Juliana Rodrigues', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Pernas e glúteo', objetivo:'Melhorar cada vez mais', restricoes:'Somente dor nas costas', academia:'Torq Cidade Baixa', dataAnamnese:'2022-03-30', statusPlanoManual:'vencidas'},
  {nome:'andreza araujo feck', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'', telefone:'00000000000', piramide:'1 gluteo, 2 perna ,3 abdomen , 4 costas', objetivo:'Ter mais enfase em meus treinos assim podendo ver resultados que não vejo , treinando por conta', restricoes:'Nenhuma relatada', academia:'academia de poa', dataAnamnese:'2022-04-04', statusPlanoManual:'vencidas'},
  {nome:'Katheyn Quintero', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Membros inferiores: focando mais nos glúteos Coxas e panturrilhas Membros superiores: ombros, biceps e costas  Sem esquecer o abdômen', objetivo:'Crear massa magra', restricoes:'Tenho os ombros machucados, consigo treinar mas tem alguns exercícios que se me dificulta por exemplo remada alta.', academia:'Smart Fit', dataAnamnese:'2022-04-08', statusPlanoManual:'vencidas'},
  {nome:'Janaína', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'Glúteos, posterior de coxas, quadriceps  tríceps', objetivo:'Ficar gostosa', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo', dataAnamnese:'2022-04-10', statusPlanoManual:'vencidas'},
  {nome:'Miriam Carolini Lessa Gonçalves', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1-abdomem  2- gluteo 3- quadriceps 5- bicips', objetivo:'Me sentir melhor que hoje.', restricoes:'Nunca', academia:'Pretendo começar em casa, no próximo mês academia do Sesc', dataAnamnese:'2022-04-11', statusPlanoManual:'vencidas'},
  {nome:'Ana Paula de Matos Souza', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1° glúteo, 2° coxas, 3° posterior e 4° costas', objetivo:'Quero muito melhorar meu físico, perder gordura localizada. E voltar a me sentir bem comigo mesma.', restricoes:'Nunca.', academia:'Smart fit', dataAnamnese:'2022-04-20', statusPlanoManual:'vencidas'},
  {nome:'Maria Aparecida Pinheiro leal', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'', telefone:'00000000000', piramide:'Quadril', objetivo:'Fortalecer a musculatura e emagrecer', restricoes:'Sim', academia:'Performance', dataAnamnese:'2022-04-26', statusPlanoManual:'vencidas'},
  {nome:'Angelica Cezar de Andrade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'00000000000', piramide:'1. Glúteos 2. Coxa 3. Costas 4. braços', objetivo:'Perda de peso e definição', restricoes:'Nenhuma relatada', academia:'Engenharia do Corpo', dataAnamnese:'2022-04-28', statusPlanoManual:'vencidas'},
  {nome:'Giovana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'giovanathamy@gmail.com', telefone:'00000000000', piramide:'1', objetivo:'Chegar ao meu objetivo , sem barriga e bumbum na nuca', restricoes:'Sim', academia:'perfomace', dataAnamnese:'2022-06-24', statusPlanoManual:'vencidas'},
  {nome:'Rafaella Trindade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'rafaellamtrindade@gmail.com', telefone:'00000000000', piramide:'Barriga , bumbum , perna e costas', objetivo:'Emagrecer  fortalecimento muscular q acho q estou perdendo , e sim né fica gostosa kkk', restricoes:'Lesão não q saiba mas sinto a lombar com frequência', academia:'Performance', dataAnamnese:'2022-07-13', statusPlanoManual:'ativas'},
  {nome:'Gabriela Amador da Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'gabrielaamador284@gmail.com', telefone:'00000000000', piramide:'1/2/3', objetivo:'Me trazer um corpo mais armonico que eu me sinta satisfeita e mega feliz e que eu consiga com seus treinos sair da minha zona de conforto que acho que nunca posso e não vou conseguir', restricoes:'Meu joelho direito tive uma lesão mais isso faz tempo mais ainda dói em alguns exercícios', academia:'Performance fitness', dataAnamnese:'2022-12-06', statusPlanoManual:'ativas'},
  {nome:'Eduarda Machado Coutinho Gadea', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'dudamachado00@outlook.com', telefone:'00000000000', piramide:'1- barriga 2- coxa 3- bumbum 4- costas', objetivo:'Emagrecer e ganhar hipertrofia', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Atricia bez cardoso', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'atricia.cardoso@gmail.com', telefone:'00000000000', piramide:'1 pernas 2 barriga 3 braços', objetivo:'Definição muscular', restricoes:'Não uso barra nas costas devido a coluna', academia:'Em varias devido gmypass', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Sandrine rosa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'sandrine.rosa1@gmail.com', telefone:'51991230734', piramide:'1- bumbum 2-pernas 3-barriga 4-costas/ braços', objetivo:'Evoluir e pegar gosto pelo hábito de ir malhar', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-05-27', statusPlanoManual:'vencidas'},
  {nome:'Yasmin Flores', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'yasminflores550@gmail.com', telefone:'51984572913', piramide:'1° bumbum 2° coxa 3° barriga  4° costas', objetivo:'Massa muscular e definição', restricoes:'Não tive lesão', academia:'Performance', dataAnamnese:'2023-06-24', statusPlanoManual:'vencidas'},
  {nome:'Tainara', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'tainarafarruda@gmail.com', telefone:'51985406087', piramide:'Barriga Coxa Bunda  Costas', objetivo:'Chegar no objetivo.', restricoes:'Sim.', academia:'Performance', dataAnamnese:'2023-08-01', statusPlanoManual:'ativas'},
  {nome:'Fernando Kunzler Paiani', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'Fernandokunzler99@gmail.com', telefone:'51999860995', piramide:'1 ombros  2 peito  3 pernas  4 costas', objetivo:'Quero ganhar massa muscular e emagrecer', restricoes:'Nenhuma lesão', academia:'Performance fitness', dataAnamnese:'2024-09-19', statusPlanoManual:'vencidas'},
  {nome:'IGOR AUGUSTO PIACINI', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'A definir', email:'igor_piacini10@hotmail.com', telefone:'51995861984', piramide:'1 peito 2 costas 3 braços 4 pernas', objetivo:'Ganho de massa e emagrecimento', restricoes:'Nenhuma relatada', academia:'Performance fitnnes', dataAnamnese:'2024-11-13', statusPlanoManual:'ativas'},
  {nome:'Juliana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'juhvandrade@gmail.com', telefone:'51996328299', piramide:'1- Costas  2- coxa 3- braços 4- bumbum', objetivo:'Melhorar minha saúde física... Resultados físicos', restricoes:'Nenhuma relatada', academia:'Uma em Osório', dataAnamnese:'2025-02-20', statusPlanoManual:'vencidas'},
  {nome:'AnaLucia da rosa', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'darosaana799@gmail.com', telefone:'51981862058', piramide:'Glúteo', objetivo:'Ganhar  músculos  e definir', restricoes:'Nenhuma relatada', academia:'Pratique', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Mariel Madlene dos Santos', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'mariel.dos.santos@hotmail.com', telefone:'51996036844', piramide:'3,1,2,4', objetivo:'Perda de peso e definição muscular.', restricoes:'Não.', academia:'Performance', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Tamara Prestes', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'3x por semana', email:'tamaraprestes@yahoo.com', telefone:'51999946085', piramide:'1 e 3', objetivo:'Emagrecimento', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Bruna Melissa', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'bruna.melissa4@gmail.com', telefone:'51981415219', piramide:'Costas, coxa, glúteos, barriga', objetivo:'Ter suporte profissional', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Ariana', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'ariane-ashley@hotmail.com', telefone:'51985981461', piramide:'Bumbum - Costas - Barriga', objetivo:'Melhorar meu desempenho', restricoes:'Nenhuma relatada', academia:'Daido', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Danielle', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'danny_kressin@yahoo.com.br', telefone:'51983140058', piramide:'Quadríceps/ posterior coxa/ glúteos/costas', objetivo:'Hipertrofia', restricoes:'Tive na lombar', academia:'Moinhos', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Adriana', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'schnadelbachadriana90@gmail.com', telefone:'51995027512', piramide:'Todos', objetivo:'Ganhar músculo e perder gordura', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Priscila Becker', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'3x por semana', email:'priscilajbecker20@gmail.com', telefone:'51995987375', piramide:'1 bunda 2 quadríceps 3 ombro e costas 4 posteriores 5 braços', objetivo:'Ficar gostosa', restricoes:'Acho q nada grave', academia:'Engenharia do corpo - Petropolis', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Eduarda Mentz', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'duda.mentz@gmail.com', telefone:'51995105977', piramide:'2,1,3,4', objetivo:'Emagrecimento é definição', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'PATRÍCIA PEREIRA DA SILVA', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'patricia.silva.pps12@gmail.com', telefone:'51997656571', piramide:'1 - coxa, 2 - Bumbum, 3 - costas, 4- Ombros', objetivo:'Dar seguimento', restricoes:'Nenhuma relatada', academia:'YES', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Viviane Moreira', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'vi.dacunha@gmail.com', telefone:'51992907258', piramide:'1 2', objetivo:'Ganhar massa', restricoes:'Nenhuma relatada', academia:'Total', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Andreli vanessa de campos', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'andrelivanessadecampos@gmail.com', telefone:'48988180167', piramide:'3,2,1,4', objetivo:'Emagrecimento', restricoes:'Nenhuma relatada', academia:'Pratique fitnes', dataAnamnese:'2026-07-20', statusPlanoManual:'ativas'},
  {nome:'Eliane fatima crupinski', status:'ok', statusLabel:'Ativa recente', nivel:'Intermediário', freq:'4x por semana', email:'elianecrupinskiagro@gmail.com', telefone:'51996504863', piramide:'1 barriga 2 cintura 3costas 4 gluteo 5 pernas', objetivo:'Definir corpo', restricoes:'Nenhuma relatada', academia:'Ctperformace', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Priscila', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'2x por semana', email:'prytchy@hotmail.com', telefone:'51984816358', piramide:'Barriga 1 Bumbum 2  Costas 3 Coxa 4', objetivo:'Emagrecimento', restricoes:'Aim', academia:'26 fit', dataAnamnese:'2026-07-20', statusPlanoManual:'vencidas'},
  {nome:'Renata Lopes', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'3x por semana', email:'renatalopes2530@gmail.com', telefone:'51998064208', piramide:'1coxa  2 bumbum 2 barriga  4 costas', objetivo:'Harmonizar mais meu corpo ganhar mais músculos e perder gordura abdominal ( milagres)', restricoes:'Joelho', academia:'Ideia Cohab', dataAnamnese:'2026-07-21', statusPlanoManual:'ativas'},
  {nome:'Graziele Steffens', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'', telefone:'', piramide:'1- gluteo 2-coxa 3-abdomen 4-braço', objetivo:'perda de peso e definição (me incomoda muito meu braço e a gordura das costas)', restricoes:'Nenhuma relatada', academia:'academia live fit - canoas', dataAnamnese:'2022-04-28', statusPlanoManual:'vencidas'},
  {nome:'Guilherme Barros', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'guilherminhobarros@yahoo.com.br', telefone:'', piramide:'1°', objetivo:'Evoluir meu corpo.', restricoes:'Não.; Não.; Não.', academia:'Performance', dataAnamnese:'2022-05-04', statusPlanoManual:'vencidas'},
  {nome:'Deise', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'deisemolina.l@icloud.com', telefone:'', piramide:'Bumbum, coxas, barriga e braços', objetivo:'Foco e evolução', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-05-07', statusPlanoManual:'vencidas'},
  {nome:'Fabiana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'fabianabalt@gmail.com', telefone:'', piramide:'1- inferiores -glúteo e coxa 2 - core - abdômen e lombar  3 - postural - escapula aladas ombros e peitos fechado  4 - superiores', objetivo:'Orientação adequada quanto a postura dos exercícios;,  organizar os treinos, aeróbicos  e , flexibilidade dentro da minha disponibilidade e de acordo com o meu porte físico e necessidade do corpo, focando no que eu realmente preciso melhorar. Ter alguém p tirar dúvidas e me dizer o q fazer e qdo fazer.', restricoes:'Tenho Bursite/tendinite no Ombro direito, desgaste manguito , e tive cervicalgia e lombar por falta fortalecimento core e pela intensidade de treinos e corrida. Encurtamento músculo pissoas.; Escoliose em s e hiperlordose .; Não .', academia:'Smartfit / academia casa e aeróbico quartel', dataAnamnese:'2022-05-10', statusPlanoManual:'vencidas'},
  {nome:'Ana Paula', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'anapaulamatossouza5@gmail.com', telefone:'', piramide:'1-Glúteo 2-Coxas 3-Posterior 4-costas', objetivo:'Quero ter uma boa relação com o meu corpo. Me sentir bem comigo mesma.', restricoes:'Nunca tive.; Não.; Nunca', academia:'Star fitniss', dataAnamnese:'2022-05-24', statusPlanoManual:'vencidas'},
  {nome:'gabriel', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'andrades.santosg@gmail.com', telefone:'', piramide:'Braço, peito, costas, abdômen', objetivo:'Crescimento e definição muscular', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-06-01', statusPlanoManual:'vencidas'},
  ...
  {nome:'Cláudia DIEHL Garcia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'claudinha18diehl@gmail.com.br', telefone:'', piramide:'Glúteo , coxa', objetivo:'Otimizar resultados', restricoes:'Condropatia grau 3; Condropatia grau 3', academia:'Acho q smart fitness', dataAnamnese:'2022-06-05', statusPlanoManual:'vencidas'},
  {nome:'Jéssica Marins', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'jessicamarins.direito@gmail.com', telefone:'', piramide:'1', objetivo:'Ficar em forma e com musculação bem como ter acesso ao material para saber quais máquinas utilizar para ter um bom resultado.', restricoes:'Não.', academia:'Ainda não sei, a princípio performace deve ter todas as máquinas abaixo', dataAnamnese:'2022-06-07', statusPlanoManual:'vencidas'},
  {nome:'Keli barbieri', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'barbierikeli@gmail.com', telefone:'', piramide:'1 abdômen  2 glúteos  3 coxa', objetivo:'Definir, emagrecer e perder a celulite', restricoes:'Nenhuma relatada', academia:'Performance  Fitness Em Eldorado', dataAnamnese:'2022-06-08', statusPlanoManual:'vencidas'},
  {nome:'Charlene', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'chatrinsi@yahoo.com', telefone:'', piramide:'Não sei bem como te responder isso. Meu objetivo é emagrecer, eliminar barriga crescer o bumbum e tonificar o corpo.', objetivo:'Ter um guia para emagrecer e tonificar o corpo com saúde.', restricoes:'Sim no joelho direito e lombar; Tenho os ligamentos do joelho danificado. Posso fazer praticamente tudo desde que com cuidado.', academia:'PureGym', dataAnamnese:'2022-06-09', statusPlanoManual:'vencidas'},
  {nome:'Luciano Costa de Sá', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'lucianocostasa@gmail.com', telefone:'', piramide:'quero 1) peito mas preciso atualmente 2) biceps e tricps 3) costas 4) Perna', objetivo:'Ganhar massa muscular e perder gordura corporal', restricoes:'braco esquerdo fraturado que consolidou com desvio; ja tive braco esquerdo fraturado que consolidou com desvio; braço esquerdo articulação do cotovelo com minima limitação', academia:'Performance', dataAnamnese:'2022-06-15', statusPlanoManual:'vencidas'},
  {nome:'Vagner Moura Magalhães', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'vagnerxitapemirim@gmail.com', telefone:'', piramide:'Peito braço e costa', objetivo:'Definir', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-06-22', statusPlanoManual:'vencidas'},
  {nome:'Eduarda lopes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'dudalopes1999@gmail.com', telefone:'', piramide:'1- barriga; 2- bumbum; 3- costas, 4- coxa.', objetivo:'Ter auxílio nos treinos para obter resultados bons.', restricoes:'Rompimento parcial do ligamento do tornozelo.', academia:'Performance', dataAnamnese:'2022-06-28', statusPlanoManual:'vencidas'},
  {nome:'Henrique', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'henriquegarcia1597@gmail.com', telefone:'', piramide:'Barriga e cintura ( flancos )', objetivo:'Emagrecimento e fortalecimento muscular', restricoes:'Nenhuma relatada', academia:'Smartfit', dataAnamnese:'2022-06-29', statusPlanoManual:'ativas'},
  {nome:'Ana Cristina', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'anacpdecastro1@gmail.com', telefone:'', piramide:'1-bumbum 2- coxas 3- barriga 4- costas', objetivo:'Eliminar gordura e ganhar músculo.', restricoes:'Não.', academia:'Performance', dataAnamnese:'2022-07-10', statusPlanoManual:'vencidas'},
  {nome:'Arisson motta', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'arissonmotta17@hotmail.com', telefone:'', piramide:'1-braços 2-peito 3- pernas 4-costas', objetivo:'.', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-07-10', statusPlanoManual:'vencidas'},
  {nome:'Larissa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'larissa.rsantanq@gmail.com', telefone:'', piramide:'1', objetivo:'Ter mais resultados', restricoes:'Nenhuma relatada', academia:'Yes fitness', dataAnamnese:'2022-07-12', statusPlanoManual:'vencidas'},
  {nome:'Franciele Freitas dos Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'francielefreitasdosssantos318@gmail.com', telefone:'', piramide:'Pernas, barriga, costas', objetivo:'Ganhar massa muscular, e manter o meu peso', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2022-07-13', statusPlanoManual:'vencidas'},
  {nome:'Fernanda Soares Rodrigues', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'2x por semana', email:'fernandasr3@gmail.com', telefone:'', piramide:'Bumbum, coxas, barriga, costas', objetivo:'Ter um treino organizado para perder gordura e ganhar massa muscular. Definir pernas e bumbum, criar mais cintura e deixar o abdômen mais definido. Tb quero a consultoria para não  me lesionar e fazer corretamente os exercícios.', restricoes:'As vezes for na lombar e  cervical.', academia:'Engenharia do corpo', dataAnamnese:'2022-07-13', statusPlanoManual:'vencidas'},
  {nome:'Simone da Rosa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'Mony_darosa@hotmail.com', telefone:'', piramide:'1-3-2-4', objetivo:'Perder barriga e definir coxas e bumbum', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-07-27', statusPlanoManual:'vencidas'},
  {nome:'Cibele Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'cibele_rs@hotmail.com', telefone:'', piramide:'Bumbum, barriga, coxa, braços', objetivo:'Treinar com mais certeza de que estou fazendo certo, com objetivo e segurança.', restricoes:'Nada grave.; Discopatia na lombar.; Não.', academia:'Yes Fitness', dataAnamnese:'2022-08-08', statusPlanoManual:'vencidas'},
  {nome:'Márcia Burgert', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'marcia.aidealimoveis@gmail.com', telefone:'', piramide:'1 barriga 2 coxa 3 bumbum 4 costas', objetivo:'Emagrecimento', restricoes:'Nenhuma relatada', academia:'Performace', dataAnamnese:'2022-08-10', statusPlanoManual:'vencidas'},
  {nome:'Wilkner Ladwig', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'wilknerladwig34010@gmail.com', telefone:'', piramide:'2', objetivo:'Ganhar muita massa muscular.', restricoes:'Não.; Não.; Não.', academia:'Performance', dataAnamnese:'2022-08-23', statusPlanoManual:'vencidas'},
  {nome:'Kellen Rosynski', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'kellenrosynski86@gmail.com', telefone:'', piramide:'1,2,3,4', objetivo:'Definição muscular,', restricoes:'Nenhuma relatada', academia:'Corpo e forma', dataAnamnese:'2022-08-24', statusPlanoManual:'vencidas'},
  {nome:'Juliana Machado Saucedo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'juliana.machadosaucedo@gmail.com', telefone:'', piramide:'1- barriga 2- bumbum 3- coxa 4- costas', objetivo:'Emagrecer e definir', restricoes:'Tive uma contratura muscular na lombar', academia:'Yes fitness', dataAnamnese:'2022-08-24', statusPlanoManual:'vencidas'},
  {nome:'Amanda', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'amanda.lilge2012@gmail.com', telefone:'', piramide:'1° barriga 2° bumbum 3° perna 4° costas/braço', objetivo:'Emagrecer e ganhar massa muscular', restricoes:'Nenhuma relatada', academia:'Performance Fitness', dataAnamnese:'2022-09-07', statusPlanoManual:'vencidas'},
  {nome:'Deisi', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'deisirejanesantos@gmail.com', telefone:'', piramide:'1200', objetivo:'Focar e definir o meu corpo de acordo!', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-09-29', statusPlanoManual:'vencidas'},
  {nome:'Fabiana Neves da Silveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'fabilzrn@gmail.com', telefone:'', piramide:'Bumbum, barriga, coxa, costas', objetivo:'Realizar exercícios corretamente', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-10-03', statusPlanoManual:'vencidas'},
  {nome:'Lúcia Scopinski', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'luciascopinski@gmail.com', telefone:'', piramide:'Bumbum, coxa, barriga e tríceps', objetivo:'Definir o corpo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-10-16', statusPlanoManual:'vencidas'},
  {nome:'Rita Fric', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'rita.fric@gmail.com', telefone:'', piramide:'Glúteos, posterior, interna de coxa, abdômen', objetivo:'Redução de gordura e definição', restricoes:'Nenhuma relatada', academia:'Canoas Planet Club e em Lajeado Monte Olimpo', dataAnamnese:'2022-11-09', statusPlanoManual:'vencidas'},
  {nome:'Hellen Teixeira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'teixeirahellen22@gmail.com', telefone:'', piramide:'1 bumbum , 2 barriga , 3 coxas , 4 braços .', objetivo:'Conseguir executar os exercícios de forma correta, para melhor resultados tanto na estética quanto mental .', restricoes:'Não .', academia:'Performance', dataAnamnese:'2022-11-15', statusPlanoManual:'vencidas'},
  {nome:'Karina Ribeiro', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'karinaribeirors@gmail.com', telefone:'', piramide:'Coxas, bumbum, braços e costas', objetivo:'Ganhar massa muscular, definir e perder gordura', restricoes:'Torci o tornozelo mas já passou; Diastase abdominal', academia:'As vezes na Performance outras na high fitnes', dataAnamnese:'2022-11-20', statusPlanoManual:'vencidas'},
  {nome:'Ezequiel Rodrigues Trindade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'ezefuts@gmail.com', telefone:'', piramide:'1-braços, 2-peito, 3-barriga, 4-pernas', objetivo:'Melhora nos resultados dos treinos', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2022-11-20', statusPlanoManual:'vencidas'},
  {nome:'Viviane Pereira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'vivianepera@hotmail.com', telefone:'', piramide:'2 1 3 4', objetivo:'Emagrecimento e massa magra', restricoes:'Sim estou me curando de uma tandinite no ombro', academia:'Performance', dataAnamnese:'2022-11-23', statusPlanoManual:'vencidas'},
  {nome:'Evelyn Vargas', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'evelynvargas725@gmail.com', telefone:'', piramide:'1 - Glúteos  2 - Coxas 3 - Barriga  4 - Costas  Exatamente como no exemplo', objetivo:'Meu objetivo principal é recuperar a minha auto estima. Após a minha segunda gestação (são duas cesarianas) tive muito aquele famoso efeito sanfona, isso acabou muito com a minha auto estima. Relaxei por completo. Com a depressão e ansiedade andando juntas acabei piorando. Chegando no auge dos 82kg (atualmente). A diástase também é minha inimiga, não gostaria de me submeter a procedimentos estéticos sem antes tentar de forma natural em construir o corpo dos meus sonhos.', restricoes:'Sim. Quando estava grávida de 3 meses, em 2017, tive muitas crises de dor na coluna. Fazia muita infiltração com o médico que me tratava na época em Guaíba. Como não passava e estava refém da infiltração procurei uma segunda opinião médica. Então fiz a ressonância magnética e constatou degeneração discal.', academia:'Yes Fitness', dataAnamnese:'2022-11-25', statusPlanoManual:'vencidas'},
  {nome:'Diovana Beatriz Gomes Zelake', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'did.zelake@gmail.com', telefone:'', piramide:'Barriga, bumbum, coxa, braço', objetivo:'Definição e uma barriga chapada', restricoes:'Uma dor no ombro quando faço MT esforço', academia:'Em casa', dataAnamnese:'2022-12-05', statusPlanoManual:'vencidas'},
  {nome:'Alana Silva de Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'alanavitor.mayapepe@gmail.com', telefone:'', piramide:'Todos', objetivo:'Alcançar minhas metas, evoluir chegar aonde eu nunca cheguei.', restricoes:'Nenhuma relatada', academia:'High fitness', dataAnamnese:'2022-12-05', statusPlanoManual:'vencidas'},
  {nome:'Daiana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'daianakohls@hotmail.com', telefone:'', piramide:'1 bumbum,2 coxa, 3 barriga 4 costas', objetivo:'Definição de pernas e glúteos', restricoes:'Joelho, acidente de moto doi se subir muita escada', academia:'Em casa', dataAnamnese:'2022-12-05', statusPlanoManual:'vencidas'},
  {nome:'Karina', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'karina.gds@hotmail.com', telefone:'', piramide:'Barriga  Costas  Bumbum  Coxa', objetivo:'Alcançar meu objetivo de chegar aos 64 kg,e perder um bom pouco da barriga', restricoes:'Luxação na perna a 11 anos', academia:'Em casa/yes fitness', dataAnamnese:'2022-12-06', statusPlanoManual:'vencidas'},
  {nome:'Patrícia Gonzalez', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'patriciagonzalez_rs@hotmail.com', telefone:'', piramide:'1 barriga, 2 coxas, 3 braço, 4 bumbum, 5 costas', objetivo:'Emagrecer, perder barriga, definir o corpo', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo Passo dareia', dataAnamnese:'2022-12-09', statusPlanoManual:'vencidas'},
  {nome:'camilly', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'camillyvescovi40@gmail.com', telefone:'', piramide:'1-barriga 2-bumbum 3-coxa 4-costas', objetivo:'ter resultados melhor', restricoes:'não tive; não tenho', academia:'gauleses', dataAnamnese:'2023-01-09', statusPlanoManual:'vencidas'},
  {nome:'Viviane goulart', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'vivianegoulart97@gmail.com', telefone:'', piramide:'1,2,4,3.', objetivo:'Aprendizado, crescimento de massa muscular.', restricoes:'Megapófise transversa bilateral em L5.', academia:'Academia corpo e forma.', dataAnamnese:'2023-01-09', statusPlanoManual:'vencidas'},
  {nome:'Samanta', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'samantagoulart19@gmail.com', telefone:'', piramide:'Barriga, bumbum, coxa, costas', objetivo:'Ter resultados e aprender mais.', restricoes:'Nenhuma relatada', academia:'Academia Corpo e forma', dataAnamnese:'2023-01-09', statusPlanoManual:'vencidas'},
  {nome:'Bruna Melissa Morais Ferreira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'bruna.melissa4@gmail.com', telefone:'', piramide:'1- glúteos, 2- barriga, 3 costas, 4- coxas', objetivo:'Experimentar a consultoria com toda a minha dedicação e ver os resultados q consigo.', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Brendha', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'brendharodrigues3@gmail.com', telefone:'', piramide:'1-Barriga, 2-Bumbum, 3-braços e 4- coxas', objetivo:'Emagrecimento', restricoes:'Sim, muai thay , joelho esquerdo incomoda um pouco as vezes. (Raramente)', academia:'Performance fitness', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Bruno Freitas Gadea', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'bfg_bruno@hotmail.com', telefone:'', piramide:'1-Abdomem, 2-peito, 3-braços, 4-costas', objetivo:'Buscar um corpo mais bonito, utilizando de forma correta e eficiente os exercícios, chegando a um resultado com saúde.', restricoes:'Dedo em gatilho no dedo médio da mão esquerda (mas não atrapalha nos exercícios). Pretendo operar no 1º semestre de 2023.; Dedo em gatilho na mão direita (nov 2022).', academia:'Performance Fitness (estou me inscrevendo)', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Marília Elusa dos Santos Brites', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'mariliasantosbrites@gmail.com', telefone:'', piramide:'Barriga, braço, coxa, bumbum', objetivo:'Perder gordura, ganhar massa muscular', restricoes:'Escápulas dos joelhos deslocam', academia:'Engenharia do corpo', dataAnamnese:'2023-01-11', statusPlanoManual:'vencidas'},
  {nome:'Douglas soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'doug_mega@hotmail.com', telefone:'', piramide:'Costas, peitoral, pernas, barriga', objetivo:'Hipertrofia', restricoes:'Nenhuma relatada', academia:'Usina do corpo', dataAnamnese:'2023-01-12', statusPlanoManual:'vencidas'},
  {nome:'Luciane fonseca', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'lucianefonseca1980@gmail.com', telefone:'', piramide:'1-2-4', objetivo:'Definição', restricoes:'Nenhuma relatada', academia:'Nation', dataAnamnese:'2023-01-15', statusPlanoManual:'vencidas'},
  {nome:'DAIA Pires trentim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'daianepires201413@gmail.com', telefone:'', piramide:'Coxa', objetivo:'Bombum coxa barriga', restricoes:'51980102684', academia:'Performance', dataAnamnese:'2023-01-15', statusPlanoManual:'vencidas'},
  {nome:'Karina Saucedo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'ksaucedo_rs@yahoo.com', telefone:'', piramide:'1 - barriga  2 - coxa 3 - bumbum 4 - costas', objetivo:'Emagrecimento e hipertrofia', restricoes:'Tive uma cesárea há 4 meses, mas até agora não me limitou.', academia:'MW Porto Alegre', dataAnamnese:'2023-01-17', statusPlanoManual:'vencidas'},
  {nome:'Camila Goulart Camboim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'camila.goulart_1@hotmail.com', telefone:'', piramide:'1° abdômen, 2° braços/costas,  3° bunda, 4° coxas....tipo tudo!', objetivo:'Fortalecer musculatura geral, principalmente abdominal, tirar excesso de gordura nas costas e braços, culotes, definir o corpo.', restricoes:'Tenho Psoríase e uma dor crônica em todo lado direito, discopatia degenerativa, esclerose óssea subcondral, coxartrose, artrite e artrose psorisiática.; Socroileíte bilateral, artropatia degenerativa coxofemoral, tendinopatia do glúteo mínimo e do médio. Estou encaminhando laudos.', academia:'Academia Corpo e Alma-Shopping Total', dataAnamnese:'2023-01-17', statusPlanoManual:'vencidas'},
  {nome:'Gabriel Brites', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'britesgabriel253@gmail.com', telefone:'', piramide:'bíceps,peito,costa,pernas', objetivo:'definição', restricoes:'Nenhuma relatada', academia:'power estancia velha', dataAnamnese:'2023-01-23', statusPlanoManual:'vencidas'},
  {nome:'Evelin', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'evelin.oprach@hotmail.com', telefone:'', piramide:'coxas … bunda … costas … abdômen', objetivo:'Ganho de resistência e de musculatura', restricoes:'Nenhuma relatada', academia:'Eldorado - guaiba ideia', dataAnamnese:'2023-01-24', statusPlanoManual:'vencidas'},
  {nome:'MARIELE', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'mariele.saez@gmail.com', telefone:'', piramide:'3 2 1 4', objetivo:'Perder gordura, fortalecer músculos e treinar para o taf', restricoes:'Nenhuma relatada', academia:'.', dataAnamnese:'2023-02-10', statusPlanoManual:'vencidas'},
  {nome:'Maria Luiza Souza Pereira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'luiza_luh09@hotmail.com', telefone:'77999667009', piramide:'1- bumbum 2-coxa 3- barriga 4-superior no geral', objetivo:'Desenvolver a minha musculatura e evoluir nos treinos', restricoes:'Nenhuma relatada', academia:'Extreme sports', dataAnamnese:'2023-03-05', statusPlanoManual:'vencidas'},
  {nome:'Michele Sirlem Fagundes Cirne', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'michelefagundescirne@gmail.com', telefone:'(51)996020374', piramide:'1- barriga, 2- bumbum, 3- coxa e 4 -braço', objetivo:'Perda de peso e definição', restricoes:'Nenhuma relatada', academia:'Em casa', dataAnamnese:'2023-03-07', statusPlanoManual:'ativas'},
  {nome:'Carine da Silva trapp', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'carinetrapp55@gmail.com', telefone:'51989408029', piramide:'1 - bumbum, 2- coxa, 3 -barriga , 4- costas', objetivo:'Olhar no espelho e me sentir bem com o que vejo, um corpo bonito', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-03-07', statusPlanoManual:'vencidas'},
  {nome:'Gabriela Jacobsen', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'bgabrielajacobsen@gmail.com', telefone:'51996063772', piramide:'Barriga, costas, braços e bumbum', objetivo:'Trabalharmos juntos para obter um resultado satisfatório sobre meu corpo e criar bons hábitos com a musculação', restricoes:'Realizei cesárea', academia:'Performance fitness', dataAnamnese:'2023-03-15', statusPlanoManual:'vencidas'},
  {nome:'Sílvia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'silviastltransportes@outlook.com.br', telefone:'51 995996941', piramide:'1ºbumbum,2º coxas, 3º barriga, 4º costas', objetivo:'Ganhar massa magra, perder gordura e definir', restricoes:'Nenhuma relatada', academia:'Perfomance fitness- Eldorado do Sul', dataAnamnese:'2023-03-17', statusPlanoManual:'vencidas'},
  {nome:'Aline', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'alinedutracosme2000@gmail.com', telefone:'51985312118', piramide:'1, 2, 3 e 4', objetivo:'Ganho de massa', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-03-27', statusPlanoManual:'vencidas'},
  {nome:'Betina', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'betina.vom@gmail.com', telefone:'51995592955', piramide:'Bumbum, peenas, barriga e bracos', objetivo:'Emagrecer e criar massa', restricoes:'Nenhuma relatada', academia:'Usina do corpo', dataAnamnese:'2023-04-04', statusPlanoManual:'vencidas'},
  {nome:'Bruna Melo Soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'brunamelosoares@gmail.com', telefone:'51992293575', piramide:'Barriga,bumbum,coxas e costas', objetivo:'Emagrecer', restricoes:'Nenhuma relatada', academia:'Perfomance', dataAnamnese:'2023-04-10', statusPlanoManual:'vencidas'},
  {nome:'erica da silva abreu', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'ericadasilvaabreu117@gmail.com', telefone:'51995098335', piramide:'1- coxa 2- barriga  3- glúteo  4- costas', objetivo:'me dedicar e alcançar meu objetivos', restricoes:'Nenhuma relatada', academia:'performance', dataAnamnese:'2023-05-13', statusPlanoManual:'vencidas'},
  {nome:'Carolina da Silva abreu', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'abreuc916@gmail.com', telefone:'‪+55 51 98062‑0074‬', piramide:'1', objetivo:'Criar corpo', restricoes:'Nenhum', academia:'Academiaperformacef', dataAnamnese:'2023-05-13', statusPlanoManual:'vencidas'},
  {nome:'Letícia Graff', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'leticiagraff123@gmail.com', telefone:'51980593827', piramide:'Emagrecimento 1 3 4', objetivo:'Emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2023-05-14', statusPlanoManual:'vencidas'},
  {nome:'Maristela Brandão', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'4x por semana', email:'maristelabrandao99@gmail.com', telefone:'51989224998', piramide:'Braço  Coxas Bumbum  Barriga', objetivo:'Emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-05-14', statusPlanoManual:'vencidas'},
  {nome:'Rafaela Mesquita Gouvea', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rafa.gouvea@hotmail.com', telefone:'51985642726', piramide:'1°bumbum 2°coxa 3°barriga 4° definir braço', objetivo:'Auxílio para emagrecer e definição na musculatura', restricoes:'Tive, bursite no quadril e rompi ligamentos do tornozelo direito', academia:'Performance', dataAnamnese:'2023-05-16', statusPlanoManual:'vencidas'},
  {nome:'Juliano de Souza Machado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'juliano38-machado@outlook.com', telefone:'51998741214', piramide:'1- bíceps 2- peito 3- costas 4- panturrilha', objetivo:'Ter bons resultados', restricoes:'Nenhuma relatada', academia:'Sublime - Alvorada/RS', dataAnamnese:'2023-05-28', statusPlanoManual:'vencidas'},
  {nome:'KARINE DE FÁTIMA TRINDADE BARBOSA', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'kaka.fisioterapeuta@gmail.com', telefone:'51992369212', piramide:'1- BARRIGA 2-COSTAS- 3-BRAÇOS - 4-PERNAS 5- BUMBUM', objetivo:'REDUZIR PESO, DEFINIR MUSCULATURA E GANHODE FORÇA MUSCULAR', restricoes:'CONDROMALÁCIA PATELAR E', academia:'PERFORMANCE', dataAnamnese:'2023-06-01', statusPlanoManual:'vencidas'},
  {nome:'Rovena Frenzel', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'ro.frenzel@gmail.com', telefone:'51981457664', piramide:'Exatamente esta ordem - bumbum, coxa, barriga, costas', objetivo:'Me ajudar a ter uma sequência de exercícios eficazes, levando a um corpo mais proporcional e harmonioso, ao invés de eu ficar perdida fazendo exercícios que de repente, não atinjam meus objetivos.', restricoes:'Tenho duas hérnias de disco na lombar, L4-L5 e L5- S1. Sei exatamente que preciso de exercícios pra fortalecimento do core e no geral pra não ter crises/dores e tbem procuro cuidar muito a postura/execução e tipo de exercício.; Só as hérnias que falei acima.', academia:'Performance Fitness', dataAnamnese:'2023-06-06', statusPlanoManual:'vencidas'},
  {nome:'Angélica Fric', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'nutricionistafric@gmail.com', telefone:'51991416225', piramide:'1- glúteos 2- coxas (lateral principalmente) 3 - abdômen 4 - superiores', objetivo:'Hipertrofia,  para superiores não tem problema treinar “pesado”, tenho maior dificuldade em inferiores devido as dores (exames que te mandei). Então se eu conseguir ficar com 60kg mas com maior volume muscular, eu fico bem feliz.', restricoes:'Sim, tá nos exames que enviei.; Já foi enviado.', academia:'Treino na Planet Club em Canoas, mas pretendo mudar para a Engenharia do Corpo mês que vem.', dataAnamnese:'2023-06-10', statusPlanoManual:'vencidas'},
  {nome:'Yasmin Flores de Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'yasminflores550@gmail.com', telefone:'51 984562913', piramide:'1° bumbum 2° coxa 3° barriga  4° costas', objetivo:'Ganhar massa muscular e ter definição.', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-06-26', statusPlanoManual:'vencidas'},
  {nome:'Kenya Balz', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'kenyabalz.16@gmail.com', telefone:'51993634809', piramide:'1 - barriga, 2 - costas, 3 - bumbum, 4 - coxas', objetivo:'Perder peso com o menor grau de flacidez possível e definição muscular', restricoes:'Nenhuma relatada', academia:'Academia Mecca - centro - Gravataí', dataAnamnese:'2023-06-27', statusPlanoManual:'vencidas'},
  {nome:'Luciane Schmitt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'lu.mendes.marques@gmail.com', telefone:'(51)991714828', piramide:'Exatamente a ordem acima', objetivo:'Resultado', restricoes:'Ciático perna direita e braço esquerdo (tendinite); Sim', academia:'Performance', dataAnamnese:'2023-06-28', statusPlanoManual:'vencidas'},
  {nome:'Priscila Bissigo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'priscila.bissigo@hotmail.com', telefone:'51982889430', piramide:'Abdômen - glúteos - coxas - braços - costas', objetivo:'Potencializar os resultados', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2023-07-02', statusPlanoManual:'vencidas'},
  {nome:'Mariana Santana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'periciasrt@yahoo.com.br', telefone:'51999053099', piramide:'Bumbum e barriga', objetivo:'Definir', restricoes:'Nenhuma relatada', academia:'Smart', dataAnamnese:'2023-07-03', statusPlanoManual:'vencidas'},
  {nome:'Mariana conzatti', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'marianaconzatti97@gmail.com', telefone:'51981858087', piramide:'1- coxa 2- glúteo 3- abdômen 4- braço', objetivo:'Ganho de massa magra e definição', restricoes:'Lombar, joelhos; Leve desgaste de cartilagem dos joelhos', academia:'Smart fit', dataAnamnese:'2023-07-07', statusPlanoManual:'vencidas'},
  {nome:'Maiara Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'maiarasantosvez@gmail.com', telefone:'51992851011', piramide:'1 barriga  2 bumbum  3 coxas  4 costas', objetivo:'Perder peso e ganhar definição', restricoes:'Epicondilite no cotovelo direito', academia:'Moinhos fitness', dataAnamnese:'2023-07-07', statusPlanoManual:'vencidas'},
  {nome:'Eduardo barbosa oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'Eduardobarbosaoliveira10@gmail.com', telefone:'51991941368', piramide:'1_abdomem 2-peito3- costas4-pernas', objetivo:'Perda de peso e ganho de massa muscular', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-07-10', statusPlanoManual:'vencidas'},
  {nome:'Carla hendler evaldt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'carlahendler@yahoo.com', telefone:'51 982161600', piramide:'1 bumbum 2 coxa 3 barriga e cintura 4 costas', objetivo:'Definir corpo e ganhar massa muscular', restricoes:'Nenhuma relatada', academia:'Yes fitnes em Eldorado do Sul', dataAnamnese:'2023-07-17', statusPlanoManual:'vencidas'},
  {nome:'Victoria shakur', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'vicshakur78@icloud.com', telefone:'51985446487', piramide:'1 glúteo  2 coxa 3 costas  4 abdômen', objetivo:'Hipertrofia', restricoes:'Naoo', academia:'Yes fitness', dataAnamnese:'2023-07-20', statusPlanoManual:'vencidas'},
  {nome:'Michel Borges Evaldt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'carlaehendler@gmail.com', telefone:'51999597597', piramide:'1 peitoral, 2braco, 3 abdômen, 4 perna', objetivo:'Definir o corpo', restricoes:'Nenhuma relatada', academia:'Yes fitnes', dataAnamnese:'2023-07-26', statusPlanoManual:'vencidas'},
  ...
  {nome:'Aline De Santana Diaz Machado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'alinesdmachado@gmail.com', telefone:'51 986517741', piramide:'Glúteo, quadríceps e posterior (porém o foco é tudo, até superiores haha)', objetivo:'Hipertrofia', restricoes:'Nenhuma', academia:'Titanium', dataAnamnese:'2023-07-27', statusPlanoManual:'vencidas'},
  {nome:'Caroline Castro da Silveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'ccastrosilveira@gmail.com', telefone:'51991672778', piramide:'1 - Glúteo 2 - Coxas 3 - Costas 4 - Braços', objetivo:'Melhora nos resultados', restricoes:'Sim, fratura da clavícula direita', academia:'Performance', dataAnamnese:'2023-07-28', statusPlanoManual:'vencidas'},
  {nome:'Daniele Arl Vertuoso Salvador', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'danivertuoso@gmail.com', telefone:'47992106298', piramide:'1 - bumbum, 2 - quadriceps, 3 - abdômen, 4 - costas', objetivo:'ganhar força muscular', restricoes:'Nenhuma relatada', academia:'movefit', dataAnamnese:'2023-07-28', statusPlanoManual:'vencidas'},
  {nome:'Katiusca dos Santos Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'oliveirakatisantos.ks@gmail.com', telefone:'51 997837527', piramide:'3412', objetivo:'Perder barriga e medidas', restricoes:'Nenhuma relatada', academia:'Performace/ B12', dataAnamnese:'2023-08-01', statusPlanoManual:'vencidas'},
  {nome:'Kamilly Oliveira Tiburski', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'millytiburski08@icloud.com', telefone:'51997117981', piramide:'1 bumbum', objetivo:'coxa definida,bumbum redondo e grande,afinar a cintura e me manter sem barriga', restricoes:'Nenhuma relatada', academia:'casa ou performance', dataAnamnese:'2023-08-01', statusPlanoManual:'vencidas'},
  {nome:'Daiane  Pires trentim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'daianepires@gamail.com.br', telefone:'980102684', piramide:'1', objetivo:'Perder peso ganhar massa', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-08-02', statusPlanoManual:'vencidas'},
  ...
  {nome:'Lisa Rodrigues Naiff', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'lisanaiff69@gmail.com', telefone:'(51)992062805', piramide:'3', objetivo:'Emagrecer', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-08-03', statusPlanoManual:'vencidas'},
  {nome:'Raquel Frenzel', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'raquelfrenzel@gmail.com', telefone:'(51) 999446617', piramide:'Tudo, inclusive MSS', objetivo:'Reforço muscular, emagrecimento', restricoes:'Canelite', academia:'Na performance ou na yes fitnes (aceito sugestões)', dataAnamnese:'2023-08-05', statusPlanoManual:'vencidas'},
  {nome:'Leonardo Colombo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'leolimao210582@gmail.com', telefone:'48 998678948', piramide:'Peito , abdômen, bunda , perna', objetivo:'Peder peso e definição', restricoes:'Sim.  Ombro; Punho', academia:'Engenharia do Corpo', dataAnamnese:'2023-08-06', statusPlanoManual:'vencidas'},
  {nome:'Millene salvado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'millenesalvado83@gmail.com', telefone:'51986490186', piramide:'1 bumbum 2 coxas  3 barriga  4 costas', objetivo:'Crescimento', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-08-09', statusPlanoManual:'vencidas'},
  {nome:'Analu Carvalho', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'ana@caberaf.com.br', telefone:'51998308830', piramide:'1-abdomem 2- cintura 3-pernas e interno das coxas  4- costas', objetivo:'Alcançar com Êxito meu Objetivo', restricoes:'Quebrei  o tornozelo direito, tenho 9 pinos e uma placa, dói as vezes, e tbm nao consigo abaixar muitooo , por exemplo de cócoras nao consigo ficar; tenho Ernia de disco na coluna cervical X5 e X6, escapei de cirurgia que estava agendada para 12/04, mas como sai da crise pedi ao medico que suspendesse; tornozelo direito', academia:'Performance', dataAnamnese:'2023-08-14', statusPlanoManual:'vencidas'},
  {nome:'Leon Kayro', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'leonkayro123@gmail.com', telefone:'51982328548', piramide:'Tudo', objetivo:'Me curar', restricoes:'Sim', academia:'Performance', dataAnamnese:'2023-08-16', statusPlanoManual:'ativas'},
  {nome:'Thales Fontoura Soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'6x por semana', email:'thales_fontoura@hotmail.com', telefone:'51997603480', piramide:'Ombros, perna, costas, bicep e tríceps, peito', objetivo:'Ficar mais gato', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-08-21', statusPlanoManual:'ativas'},
  {nome:'Nathali Albuquerque', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'nathalbuq@gmail.com', telefone:'51989251100', piramide:'1. Barriga, 2. Coxa, 3. Braço, 4. Bumbum', objetivo:'Emagrecer pelo menos 10kg, perder barriga e definir. Tive uma gestação em 2020 que foi até 42 semanas e deixou a barriga bem flácida. Exercícios pra postura também são bem vindos, trabalho 8h direto sentada. Aumentar resistência física.', restricoes:'Rompi o ligamento do tornozelo em dezembro e as vezes ainda sinto um leve desconforto', academia:'High ou Performance', dataAnamnese:'2023-08-21', statusPlanoManual:'vencidas'},
  {nome:'Naiara Machado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'naiara_machado@hotmail.com', telefone:'51980558557', piramide:'3, 1, 2, 4', objetivo:'Emagrecimento e hipertrofia', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-08-22', statusPlanoManual:'vencidas'},
  {nome:'Vivian Oliveira da Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'vivianoliveiradasilva23@gmail.com', telefone:'51989766806', piramide:'1 barriga 2 bumbum 3 coxas 4 costas', objetivo:'Perder peso e ganhar glúteos', restricoes:'Nenhuma relatada', academia:'Cia do corpo', dataAnamnese:'2023-09-01', statusPlanoManual:'vencidas'},
  {nome:'Camila da Silva Estevão', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'camila_estevao15@hotmail.com', telefone:'51 991978368', piramide:'Quadríceps; glúteo; panturrilha; barriga', objetivo:'Hipertrofia', restricoes:'Nenhuma relatada', academia:'Moinhos fitness', dataAnamnese:'2023-09-02', statusPlanoManual:'vencidas'},
  {nome:'Caroline', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'carollalgayer@icloud.com', telefone:'51997975459', piramide:'1 pernas , 2 bumbum, 3 barriga, 4 costas (coluna)', objetivo:'Ganhar definição de pernas e glúteo', restricoes:'Hiperlordose na cervical e lombar e discopatia degenerativa da lombar e da cervical', academia:'Bodytech', dataAnamnese:'2023-09-04', statusPlanoManual:'vencidas'},
  {nome:'Joao carlos l de souza filho', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'joaocarloslemos91@gmail.com', telefone:'51989502000', piramide:'1 barriga 2 peito 3 costas 4 braços', objetivo:'Obter auxílio para. Conseguir resultados mais rapidamente', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-09-05', statusPlanoManual:'vencidas'},
  {nome:'Atricia cardoso', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'atricia.cardoso@gmail.com', telefone:'51995655212', piramide:'1 2 3 4', objetivo:'Definição', restricoes:'Coluna lombar', academia:'Eldo e poa', dataAnamnese:'2023-09-09', statusPlanoManual:'vencidas'},
  {nome:'Mariana Medeiros Castanha', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'castanhamedeiros1@gmail.com', telefone:'(51)98413-3054', piramide:'1, 2, 3 e 4', objetivo:'Fortalecimento muscular e definição', restricoes:'Já tive um tumor no quadril e devido a isso, em alguns aparelhos, sinto um pouco de sensibilidade na região', academia:'Performance', dataAnamnese:'2023-09-18', statusPlanoManual:'vencidas'},
  {nome:'Daiandra Gonçalves', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'daiandra.dasilva@gmail.com', telefone:'51 984809431', piramide:'1 barriga 2 bumbum 3 coxas 4 costas', objetivo:'Voltar ao meu corpo de antes das gestações', restricoes:'Nenhuma relatada', academia:'Em casa', dataAnamnese:'2023-09-20', statusPlanoManual:'vencidas'},
  {nome:'Priscila Justo Becker', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'priscilajbecker20@gmail.com', telefone:'(51) 995987375', piramide:'1 - Gluteos 2 - Coxa 3 - Braços (biceps, triceps, ombro - normalmente acumulo gordura nessa região) 4 - Costas', objetivo:'conseguir um treino que realmente me traga resultados e seja focado no meu corpo', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo - Petrópolis', dataAnamnese:'2023-09-21', statusPlanoManual:'vencidas'},
  {nome:'Pablo Leonardo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'pablodudu2010@gmail.com', telefone:'51999266207', piramide:'1 - Shape 2 - bíceps 3 - Costas 4 - Pernas', objetivo:'Consistência e apoio profissional', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-10-08', statusPlanoManual:'vencidas'},
  {nome:'Luciana Machado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'lumachadorecova02@gmail.com', telefone:'51995502031', piramide:'1 Barriga,  2 bumbum,  3 coxa,  4 costas', objetivo:'Primeiro lugar pela saúde, segundo, definir o corpo', restricoes:'Nenhuma relatada', academia:'Sesc', dataAnamnese:'2023-10-10', statusPlanoManual:'vencidas'},
  ...
  {nome:'Mari Dantas', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'marisete.osorio@hotmail.com', telefone:'51985099341', piramide:'1Barriga 2costas 3Bumbum 4 coxa', objetivo:'Emagrecer e ganhar massa muscular', restricoes:'Nunca tive; Ñ; Ñ', academia:'Barra Thay', dataAnamnese:'2023-11-16', statusPlanoManual:'vencidas'},
  {nome:'Monica Colombo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'monicacolombo2013@gmail.com', telefone:'+5551998080439', piramide:'3 2 1 4', objetivo:'Diminuir medidas e definir perna coxa barriga e bumbum', restricoes:'Pé torção', academia:'Bem estar Cachoeirinha', dataAnamnese:'2023-11-20', statusPlanoManual:'vencidas'},
  {nome:'Natália Rodrigues', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'nathyrodriguess_@hotmail.com', telefone:'51996405068', piramide:'1• bumbum, 2 • coxa, 3 • barriga e 4 • bíceps', objetivo:'Definir bumbum, coxas (tenho muita dificuldade em definir quadríceps), secar barriga', restricoes:'Nenhuma relatada', academia:'Academia Performance', dataAnamnese:'2023-11-20', statusPlanoManual:'vencidas'},
  {nome:'Rubia Bagesteiro', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'bagesteiro.r@gmail.com', telefone:'51995073871', piramide:'1- bumbum, 2-coxa, 3- braços, 4- costas', objetivo:'Diminuir o percentual de gordura e hipertrofia', restricoes:'Nenhuma relatada', academia:'Em primeiro momento em casa, mas penso em futuramente treinar em alguma academia de Eldorado', dataAnamnese:'2023-11-20', statusPlanoManual:'vencidas'},
  {nome:'Thaila Gomes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'6x por semana', email:'thailaandry@gmail.com', telefone:'51994224479', piramide:'3,1,2,4', objetivo:'Saúde e alto estima', restricoes:'Nenhuma relatada', academia:'Arena Humaitá', dataAnamnese:'2023-11-20', statusPlanoManual:'vencidas'},
  {nome:'Luciana Teixeira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'lucianaitaquy@gmail.com', telefone:'51 99180.9978', piramide:'3', objetivo:'Emagrecimento', restricoes:'Nenhuma relatada', academia:'Academia do meu predio por enquanto. Acredito que tenha todos os aparelhos abaixo. Tem bastante pessoas que estão malhando na academia, fazendo musculação.', dataAnamnese:'2023-11-20', statusPlanoManual:'vencidas'},
  {nome:'Flávia da Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'bemvindocliente8101@gmail.com', telefone:'51984681781', piramide:'1- barriga 2- bumbum 3-coxas 4- costas', objetivo:'Melhorar a saúde mental e física ( mto sedentária)', restricoes:'Nenhuma relatada', academia:'Ainda n sei , mas assim que der a YES', dataAnamnese:'2023-11-24', statusPlanoManual:'vencidas'},
  {nome:'Jessica Cabral', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'jessiicabral@live.com', telefone:'51989550503', piramide:'1 barriga 2 costas 3 bumbum 4 coxa', objetivo:'Hipertrofia quero definição no corpo sinto que os inferiores até mudou mas superiores estagnou', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2023-11-24', statusPlanoManual:'vencidas'},
  {nome:'Claudete Dantas Rolim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'clau.rolim8@gmail.com', telefone:'51 99448-9396', piramide:'Os 4', objetivo:'Tentar mudar um pouco o meu corpo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2023-11-28', statusPlanoManual:'vencidas'},
  {nome:'Helen Caroline Brites dos Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'helenbrites342@gmail.com', telefone:'54991550436', piramide:'1- coxa 2-barriga 3- costas 4- bumbum', objetivo:'Atingir o peso ideal e ter o corpo definido', restricoes:'Leve desvio na coluna vertebral.', academia:'Engenharia do corpo', dataAnamnese:'2023-12-17', statusPlanoManual:'vencidas'},
  {nome:'Sonia marzona', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'2x por semana', email:'soniamariamarzona@gmail.com', telefone:'51994527474', piramide:'1) bumbum e costas 2) barriga 3) braço 4) coxa', objetivo:'Definir musculos e emagrecer', restricoes:'Nenhuma relatada', academia:'Performance fitness', dataAnamnese:'2024-01-04', statusPlanoManual:'vencidas'},
  {nome:'Monique Gonzalez Pereira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'moniquepereira645@gmail.com', telefone:'51993721614', piramide:'1 bumbum', objetivo:'Me ajudar a crescer os músculos', restricoes:'Nunca tive', academia:'Performance', dataAnamnese:'2024-01-05', statusPlanoManual:'ativas'},
  {nome:'Jéssica Stéphanie', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'2x por semana', email:'jessica.nasario@gmail.com', telefone:'51-985076707', piramide:'Coxa, bumbum, barriga, braço', objetivo:'Definição muscular', restricoes:'Bursite no ombro direito; cervical retilínea; dores no joelho direito; Sim, bursite e cervical', academia:'BodyGym (Guaíba)', dataAnamnese:'2024-01-09', statusPlanoManual:'vencidas'},
  {nome:'Franciele', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'franciele.morialdo@hotmail.com', telefone:'51995517353', piramide:'Bumbum, coxa, cintura, barriga (gosto de treinar costas também)', objetivo:'Ganhar massa muscular', restricoes:'Nenhuma relatada', academia:'Smart Fit menino Deus', dataAnamnese:'2024-01-10', statusPlanoManual:'vencidas'},
  {nome:'Bruna Pinós Rodzinski', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rodzinskibruna@gmail.com', telefone:'51997209023', piramide:'1 coxa 2 bumbum 3 barriga 4 braço', objetivo:'hipertrofia e emagrecimento', restricoes:'Nenhuma relatada', academia:'performance', dataAnamnese:'2024-01-19', statusPlanoManual:'vencidas'},
  {nome:'Gisele', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'gymanke1986@gmail.com', telefone:'51994031298', piramide:'Bumbum , coxa , barriga e braços', objetivo:'Um corpo definido', restricoes:'Tive na panturrilha esquerda ( trombose ) a seis meses', academia:'Yes fitness', dataAnamnese:'2024-01-22', statusPlanoManual:'ativas'},
  {nome:'Luzeni Alves de oliveira Teixeira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'luzenir329@gmail.com', telefone:'65981325837', piramide:'1.3.4', objetivo:'Fazer os exercícios certo', restricoes:'Nenhuma relatada', academia:'Olímpica', dataAnamnese:'2024-02-08', statusPlanoManual:'vencidas'},
  {nome:'Danielli', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'dfuchs347@gmail.com', telefone:'(51) 989241007', piramide:'1342', objetivo:'Perder peso e ganhar definição muscular', restricoes:'Sentia muita dor no joelho antes de começar a musculação. Já tive problemas na lombar e recentemente senti desconforto na mesma. Quando joguei vôlei ano passado tive uma leve lesão no ombro direito. E tmbm ando sentindo desconforto na escapula direita 🤡; Nada diagnosticado', academia:'Perfomance', dataAnamnese:'2024-02-12', statusPlanoManual:'vencidas'},
  {nome:'Cristiane Soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'6x por semana', email:'cristiane_oliveiraosares@hotmail.com', telefone:'51998313443', piramide:'1- bumbum 2- braço 3- barriga - costas', objetivo:'Fortalecimento', restricoes:'Nenhuma relatada', academia:'Engenharia', dataAnamnese:'2024-02-16', statusPlanoManual:'vencidas'},
  {nome:'Marilda', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'Marilda1972@hotmail.com', telefone:'7865862453', piramide:'1coxa  2 barriga3 bumbum', objetivo:'Treinar com qualidade', restricoes:'Nenhuma relatada', academia:'You Fit', dataAnamnese:'2024-02-21', statusPlanoManual:'vencidas'},
  {nome:'Michele Abreu', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'michele.abreu@auxiliadorapredial.com.br', telefone:'51999571979', piramide:'1- barriga, 2- bumbum, 3- coxas 4- costas', objetivo:'Máximo de ganho muscular possível', restricoes:'Nenhuma relatada', academia:'Performance Fitness', dataAnamnese:'2024-03-07', statusPlanoManual:'vencidas'},
  {nome:'Marilene Lopes Mendonça', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'6x por semana', email:'ml4103766@gmail.com', telefone:'1978 7287503', piramide:'Costa', objetivo:'Perca de peso', restricoes:'Nenhuma relatada', academia:'Não sei o nome', dataAnamnese:'2024-03-14', statusPlanoManual:'vencidas'},
  {nome:'Maria Clara Gomes Brito', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'6x por semana', email:'maclaragomes.b@gmail.com', telefone:'51 996155298', piramide:'1- Barriga 1- Coxa 2- Superiores em geral 4- Bumbum', objetivo:'Eu gostaria de perder gordura e ganhar músculo.', restricoes:'Tenho uma fissura no joelho leve, causada provavelmente por condromalácia patelar.; Vou enviar à ressonância do joelho.; Não.', academia:'Performance de Eldorado.', dataAnamnese:'2024-03-15', statusPlanoManual:'vencidas'},
  {nome:'Eduarda da Silva coelho', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'eduardacoeelho772@gmail.com', telefone:'(51)991983630', piramide:'2143', objetivo:'Hipertrofia', restricoes:'Nenhuma relatada', academia:'-', dataAnamnese:'2024-03-22', statusPlanoManual:'vencidas'},
  {nome:'Elis Agostini', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'elis_agostini@hotmail.com', telefone:'51996901050', piramide:'1 - gluteos 2 - perna quadríceps, posterior  3 - abdômen', objetivo:'Definição e ganho de massa muscular', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-03-26', statusPlanoManual:'vencidas'},
  {nome:'Ana carolina luigi', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'anacarolinacruzbessa@gmail.com', telefone:'321-9469794', piramide:'Bumbum, coxa, barriga, costas', objetivo:'Definir( acima não sei bem os nomes das máquinas ) então não sei o que tem )', restricoes:'Nenhuma relatada', academia:'Planet fitness', dataAnamnese:'2024-03-29', statusPlanoManual:'vencidas'},
  {nome:'Suehen', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'nattalleoliveira@hotmail.com', telefone:'5616744100', piramide:'1 barriga ,2 braços 3 pernas 4 costas', objetivo:'Aprender a me exercitar', restricoes:'No joelho', academia:'Ufit', dataAnamnese:'2024-04-04', statusPlanoManual:'vencidas'},
  {nome:'Karin Paliosa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'karin.radiologia@gmail.com', telefone:'51 993616934', piramide:'Glúteo, perna, costas, barriga', objetivo:'Aumentar massa magra, definição, aumentar glúteo', restricoes:'Nenhuma relatada', academia:'Fenix', dataAnamnese:'2024-04-05', statusPlanoManual:'vencidas'},
  {nome:'Natália Andrade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'ailatan_edardna@hotmail.com', telefone:'051984656980', piramide:'Barriga/Bumbum/coxa/costas', objetivo:'Definição, emagrecimento, bem estar', restricoes:'Nenhuma relatada', academia:'Idéia Fitness Academia (Santa Rita)', dataAnamnese:'2024-04-08', statusPlanoManual:'vencidas'},
  {nome:'Edna Pereira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'edanamaria17@gmail.com', telefone:'3212084274', piramide:'1 e 3', objetivo:'Saúde', restricoes:'Não lembro', academia:'Nem uma', dataAnamnese:'2024-04-09', statusPlanoManual:'vencidas'},
  {nome:'laura Basso', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'laurabasso101@gmail.com', telefone:'51980132544', piramide:'1-bumbim 2-barriga 3-coxa  4-costas', objetivo:'Fazer os exercícios corretamente e ganhar massa muscular', restricoes:'nao tenho', academia:'performance', dataAnamnese:'2024-04-16', statusPlanoManual:'vencidas'},
  {nome:'Lugar preferido?', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'pamella.correaft@gmai.com', telefone:'+1 561 929 0601', piramide:'1 barriga; 2 coxa, 3 bumbum, 4 braço', objetivo:'Emagrecer', restricoes:'Nenhuma relatada', academia:'YouFit', dataAnamnese:'2024-04-18', statusPlanoManual:'vencidas'},
  {nome:'Claudia correa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'4x por semana', email:'cld_maria@hotmail.com', telefone:'7742970981', piramide:'1 2,3,4,', objetivo:'Saúde definição', restricoes:'Pescoço', academia:'Planets fitness', dataAnamnese:'2024-05-01', statusPlanoManual:'ativas'},
  {nome:'Schariana Larrea', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'schariana.grandini@gmail.com', telefone:'51989175126', piramide:'3', objetivo:'Exercício corretos e ter definição muscular', restricoes:'Nenhuma relatada', academia:'Yes fitness', dataAnamnese:'2024-06-10', statusPlanoManual:'vencidas'},
  {nome:'Fabiana Azambuja dos Santos Barbosa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'pytty.faby@gmail.com', telefone:'51995452270', piramide:'1-barriga- 2 pernas 3- bracos 4- bumbum', objetivo:'Perder gordura e aumentar a massa magra', restricoes:'Nenhuma relatada', academia:'Em casa', dataAnamnese:'2024-06-23', statusPlanoManual:'vencidas'},
  {nome:'Giselle Troglio', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'gitroglio@hotmail.com', telefone:'51999869394', piramide:'Perna, barriga, costas, abdômen, braco', objetivo:'Ganhar massa muscular e perder gordura', restricoes:'Nenhuma relatada', academia:'Moinhos', dataAnamnese:'2024-06-26', statusPlanoManual:'vencidas'},
  {nome:'Kimberlyn', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'kimberlynvitoriamoura@gmail.com', telefone:'51986108215', piramide:'1- barriga  2- bumbum 3- coxa 4- costas', objetivo:'É perder a gordura localizada da minha barriga', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-07-16', statusPlanoManual:'vencidas'},
  {nome:'Luana Cardoso Borges', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'luannacardoso275@gmail.com', telefone:'51996374480', piramide:'Barriga - costas - bumbum - coxa', objetivo:'Meu objetivo é principalmente diminuir a barriga que me incomoda bastante e deixar o bumbum mais durinho', restricoes:'Nenhuma relatada', academia:'Rt fitness jardim dos lagos', dataAnamnese:'2024-07-18', statusPlanoManual:'vencidas'},
  {nome:'Stephani Azevedo Araujo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'stephaniaraujo.2000@gmail.com', telefone:'51997308801', piramide:'1° Glúteos  2° Perna  3° barriga  4° costas/ parte superior no geral', objetivo:'definir o corpo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-07-19', statusPlanoManual:'vencidas'},
  {nome:'Laís Silva Peixoto', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'laispeixotosilva123@gmail.com', telefone:'51993921086', piramide:'1- coxa 2 bumbum- barriga 4 braços', objetivo:'Preciso de treinos específicos, para o pouco que eu posso treinar que é 3 dias sejam bem aplicados. Tenho uma rotina corrida, filho de 1.8 meses então tudo precisa ser otimizado', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-07-30', statusPlanoManual:'vencidas'},
  {nome:'Flavia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'fs456651@gmail.com', telefone:'2027517987', piramide:'2', objetivo:'Emagrecer e tirar a flacidez das pernas e dos braços', restricoes:'Nenhuma relatada', academia:'La fitness', dataAnamnese:'2024-08-02', statusPlanoManual:'vencidas'},
  {nome:'Caren Batista', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'cb030885@gmail.com', telefone:'51998503010', piramide:'1- bumbum 2- coxa 3-barriga 4-costas', objetivo:'Emagrecimento e definição.', restricoes:'Sim, desgaste na L5 (coluna); desgaste na L5 (coluna), não tenho exames recentes.', academia:'Studio Training', dataAnamnese:'2024-08-05', statusPlanoManual:'vencidas'},
  {nome:'Renata Jaqueline Sampaio', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rjaquesampaio@gmail.com', telefone:'51996038856', piramide:'1- 3- 2-4', objetivo:'Glúteos ….. emagrecimento e ganho massa muscular', restricoes:'Tenho tendinite no joelho direito, normalmente sinto dor na canela ao fazer esteira', academia:'Engenharia do Corpo', dataAnamnese:'2024-08-05', statusPlanoManual:'vencidas'},
  {nome:'Isnelen Piacini', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'isnelenpiacini06@gmail.com', telefone:'51993072923', piramide:'1° coxas, 2° bumbum, 3° Abdômen, 4° braços', objetivo:'Perda de peso e tonificação', restricoes:'Nenhuma relatada', academia:'No parque esportivo da Puc', dataAnamnese:'2024-08-12', statusPlanoManual:'vencidas'},
  {nome:'Ana Lúcia da rosa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'darosaana799@gmail.com', telefone:'51 98186-2058', piramide:'1', objetivo:'Secar e definir meu corpo', restricoes:'Nenhuma relatada', academia:'Star fit', dataAnamnese:'2024-08-13', statusPlanoManual:'ativas'},
  {nome:'Guilherme L Souza', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'guilibretti@gmail.com', telefone:'051995229547', piramide:'1- Braço completo (tríceps, bíceps e ante braço) 2- Perna completa 3- Ombros 4- Abdômen', objetivo:'Adquirir um shape com volume e seco', restricoes:'Não.; Não.; Não.', academia:'Performance Residencial', dataAnamnese:'2024-08-14', statusPlanoManual:'vencidas'},
  {nome:'Maria Eduarda Guimarães de Souza', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'mariasouzaguimaraes2002@gmail.com', telefone:'51999070872', piramide:'1 bumbum 2 quadríceps 3 abdômen 4 braço', objetivo:'Emagrecer', restricoes:'Escoliose', academia:'Corpo e forma', dataAnamnese:'2024-08-14', statusPlanoManual:'vencidas'},
  {nome:'Nathalia Gonçalves San Martin', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'6x por semana', email:'natthaliag21@gmail.com', telefone:'51994239423', piramide:'Glúteo, coxa, barriga e costas', objetivo:'Definir o corpo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-08-14', statusPlanoManual:'vencidas'},
  {nome:'Fabiana Peres Trindade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'fabiana_perestrindade@hotmail.com', telefone:'51980457289', piramide:'Na real eu preciso de força muscular e emagrecer', objetivo:'Focar na academia', restricoes:'Bursite; Bha as águas levaram kkk', academia:'Atualmente na usina', dataAnamnese:'2024-08-16', statusPlanoManual:'vencidas'},
  {nome:'Felipe Carneiro de Araújo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'fca.carneiro03@gmail.com', telefone:'48996015089', piramide:'1- peito , 2 - barriga , 3 - biceps , 4 - coxa', objetivo:'Emagrecer e definir', restricoes:'Nenhuma relatada', academia:'Engenharia do Corpo', dataAnamnese:'2024-08-19', statusPlanoManual:'vencidas'},
  {nome:'Rafaela Guimarães', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'rafaela.pguimaraess@gmail.com', telefone:'5199272-6842', piramide:'2 1 3 4', objetivo:'Emagrecer e definir', restricoes:'Nenhuma relatada', academia:'Yes ou Performance', dataAnamnese:'2024-08-19', statusPlanoManual:'vencidas'},
  {nome:'Tatiane da Costa Caon', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'tatianecc007@gmail.com', telefone:'51985621412', piramide:'1- 3 - 2 -4', objetivo:'Perder medidas e definir', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-08-20', statusPlanoManual:'vencidas'},
  {nome:'Jessika cunha', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'jeekcunha@gmail.com', telefone:'51 996547272', piramide:'Glúteos, coxas, barriga e costas', objetivo:'Me auxiliar nos treinos', restricoes:'Apenas vertigem, tontura, quando faço algum exercício de subida e descida, ou algo muito rápido!', academia:'Em casa', dataAnamnese:'2024-08-21', statusPlanoManual:'vencidas'},
  {nome:'Francely Andrades Waszak', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'5x por semana', email:'francely.waszak@gmail.com', telefone:'51992952415', piramide:'1 barriga 2 perna 3 bumbum 4 peito', objetivo:'Perca de peso, definição muscular, bem estar', restricoes:'Desgaste de cartilagem no joelho esquerdo.', academia:'Performace', dataAnamnese:'2024-08-24', statusPlanoManual:'vencidas'},
  {nome:'gabriely waszak souza', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'gabriely.waszak@icloud.com', telefone:'51992070541', piramide:'barriga', objetivo:'emagrecimento', restricoes:'Nenhuma relatada', academia:'performance', dataAnamnese:'2024-08-24', statusPlanoManual:'vencidas'},
  {nome:'Priscila Ambos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'prisilambos@gmail.com', telefone:'51 98266-0725', piramide:'1-bumbum urgente  2 coxas urgente   3-barriga urgente  4 - braços', objetivo:'Depois que eu perdi 22 kg eu tô.com as pernas finas e o bumbum caído isso me encomenda  muito fora a barriga  os braços  gordinhos ainda mais não quero emagrecer  quero deixar  essas partes do meu corpo melhores bumbum empinado  barriga com.menos gordura  braços  também  e coxas mais grossas  não estou feliz com meu corpo tudo fica feio', restricoes:'Não tenho nada som preguiça', academia:'Ainda nao sei preciso  de indicação  nao consigo  me adaptar  e tenho vergonha  de ir pra academia', dataAnamnese:'2024-09-07', statusPlanoManual:'vencidas'},
  {nome:'Giovana mazuhim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'giovanathamy@gmail.com', telefone:'51986592605', piramide:'Todos', objetivo:'Consiga atingir o máximo dos meus objetivos', restricoes:'Tenho o menisco discoide; Não enchente levou', academia:'Performance', dataAnamnese:'2024-09-07', statusPlanoManual:'ativas'},
  {nome:'Paula Oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'paula95817338@gmail.com', telefone:'51997762225', piramide:'1-barriga 2bumbum 3coxa 4costas', objetivo:'Sair da zona de conforto, parar de treinar fofo e pegar pesado', restricoes:'Nenhuma relatada', academia:'Talvez na yes fitness em eldorado, vou sair da academia que estou em Guaíba.', dataAnamnese:'2024-09-07', statusPlanoManual:'vencidas'},
  {nome:'Eduarda Machado Coutinho Gadêa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'dudamachado00@outlook.com', telefone:'51999381049', piramide:'1 bumbum, 2 coxas, 3 barriga e 4 braços', objetivo:'Ganhar massa muscular', restricoes:'Escoliose lombar', academia:'Academia MOVE EXTREME JURERÊ', dataAnamnese:'2024-09-08', statusPlanoManual:'vencidas'},
  {nome:'Carolina Machado Vargas', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'carolinamachadovargas@gmail.com', telefone:'51992391295', piramide:'1 Barriga 2 bumbum 3 coxa 4 costas e reforço muscular devido a minha lesão por causa da ginástica artística como já fizemos em 2019 nos treinos', objetivo:'Tirar as minhas dores da cervical, lombar, e joelho, fazer o reforço muscular, emagrecer, perder a barriga e definir o corpo', restricoes:'Sim na coluna compressão disco, tendinite braço direito, lesão cervical perda de força braço direito e encurtamento perna esquerda devido a lesão fêmur....Nariga tinha treino específico pra isso em 2019; Tenho mas não tenho mais os exames por causa da enchente; As questões da coluna', academia:'Acredito que vou alternar academia e treino em casa ou em parque/ caminhada ao ar livre depende mto do meu tempo/ dia', dataAnamnese:'2024-09-09', statusPlanoManual:'vencidas'},
  {nome:'Thalia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'kohlsthalia@gmail.com', telefone:'51980490716', piramide:'1- barriga  2-bumbum  3-coxas  4- costas', objetivo:'Me ajudar a alcançar os objetivos traçados de emagrecimento e definição', restricoes:'Somente dores na lombar; Tenho', academia:'Na Yes fitnes por enquanto', dataAnamnese:'2024-09-16', statusPlanoManual:'vencidas'},
  {nome:'Jenifer dos Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'jenifer_dro@hotmail.com', telefone:'51984949198', piramide:'1- Bumbum 2- Pernas 3- Barriga 4- Costas', objetivo:'Emagrecer, criar bumbum e perna e definir a bartiga', restricoes:'Não tenho e nunca tive', academia:'Acredito que na Yes', dataAnamnese:'2024-09-30', statusPlanoManual:'vencidas'},
  {nome:'Aline Ribeiro Garcias', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'alinematosgrcias@gmail.com', telefone:'51992756129', piramide:'1 e 3', objetivo:'Emagrecer ,pretendo ficar com uns 59 k,afinar cintura,definir pernas ,mas n quero engrossar mais,bumbum só definir e empinar ,tmb n quero aumentar bumbum', restricoes:'Nenhuma relatada', academia:'Sencefit', dataAnamnese:'2024-10-03', statusPlanoManual:'vencidas'},
  {nome:'Angelina Paz Mendes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'angelinatst@gmail.com', telefone:'51985659734', piramide:'1 - costas  2 - braços  3 - glúteos  4 - coxas', objetivo:'Emagrecimento e definição muscular', restricoes:'Nenhuma relatada', academia:'Busata', dataAnamnese:'2024-10-09', statusPlanoManual:'vencidas'},
  {nome:'Renata Pedrotti Franco', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'renata.pedrotti@hotmail.com', telefone:'51989105995', piramide:'1 costas,2ombro e bíceps, 3 quadríceps 4 gluteo', objetivo:'Emagrecimento e definição muscular', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo', dataAnamnese:'2024-10-16', statusPlanoManual:'vencidas'},
  {nome:'Ester correa zanotelli', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'ester.zanotelli@gmail.com', telefone:'51 997216657', piramide:'1 barriga, 2 bumbum, 3 coxa 4 peitoral', objetivo:'Direcionar para que eu faça exercícios corretamente para poder atingir com eficácia meus objetivos.', restricoes:'Eu sinto dor no meu joelho direito que preciso investigar.', academia:'Performance', dataAnamnese:'2024-10-21', statusPlanoManual:'vencidas'},
  ...
  {nome:'Priscila Ribeiro Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'2x por semana', email:'prytchy@hotmail.com', telefone:'51984816358', piramide:'3 -4- 2- 1', objetivo:'Eliminar peso e ganhar massa muscular', restricoes:'Já quebrei o 5º metatarso do pé direito e já tive uma torção no pé esquerdo', academia:'Engenharia do Corpo - São Leopoldo', dataAnamnese:'2024-10-28', statusPlanoManual:'vencidas'},
  {nome:'JEFERSON RODRIGUES BARBOSA', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'Jefersonbarbosa1993@hotmail.com', telefone:'51 986600078', piramide:'Barriga, costas, braços, pernas para corrida', objetivo:'Aprender a sair do plano feito pela acadêmia', restricoes:'Ondo', academia:'Cia do corpo', dataAnamnese:'2024-11-05', statusPlanoManual:'vencidas'},
  {nome:'Ana Carolina Pereira Fedatto', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'6x por semana', email:'anacarolinapereirafedatto@gmail.com', telefone:'51996452269', piramide:'1-3-2-4', objetivo:'Me preparar para o teste de aptidao fisica, ganhar resistência e massa muscular', restricoes:'sim, no ligamento do dedao do pe direito', academia:'Performance Fitness', dataAnamnese:'2024-11-11', statusPlanoManual:'ativas'},
  {nome:'Fernando kunzler', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'Fernandokunzler99@gmail.com', telefone:'51999860995', piramide:'Ombros e peito', objetivo:'Perder gordura e ganhar massa magra', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2024-11-14', statusPlanoManual:'vencidas'},
  {nome:'Rafael Brito Feck', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'rafaelbfeck@gmail.com', telefone:'51980114807', piramide:'Barriga, peitoral, bicips e costas', objetivo:'Emagrecimento/hipertrofia', restricoes:'Nenhuma relatada', academia:'Usina do corpo', dataAnamnese:'2024-11-14', statusPlanoManual:'vencidas'},
  {nome:'Renata Rolim', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'4x por semana', email:'contato.renatarolim@gmail.com', telefone:'55 98153-7900', piramide:'1 bumbum 2 barriga 3 ombro 4 costas', objetivo:'Quero perder gordura e ganhar massa magra, melhorar o treino pra atingir mais rapido o objetivo', restricoes:'Realizando o exercício na cadeira adutora, senti um estalo na junção dos ossos púbicos além de dor. Fiquei sem fazer o exercício por 2 semanas e quando voltei a fazer, iniciei com peso bem baixo por medo de me machucar novamente.; Tenho 26 graus de escoliose rotatória em C. Pulso esquerdo tem um cisto e impossibilita de realizar atividades que precise do apoio das mãos. Principalmente quando o braço e a mão formam um ângulo de 90 graus (por exemplo, ao fazer flexao)', academia:'Perfil (em gravatai)', dataAnamnese:'2024-11-15', statusPlanoManual:'vencidas'},
  {nome:'nicolsa batista fernandes dos santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'nicolasbckp612@Gmail.com', telefone:'51986136039', piramide:'1 - Barriga 2 - Coxas 3 - Costas 4 - Bumbum', objetivo:'Conseguir ter uma definição', restricoes:'Nenhuma relatada', academia:'Unisa do Corpo - Centro Historico', dataAnamnese:'2024-11-17', statusPlanoManual:'vencidas'},
  {nome:'Micaela de Souza Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'2x por semana', email:'souza-micaela@outlook.com', telefone:'51980297897', piramide:'Barriga, costas, busto, bumbum', objetivo:'Melhor disposição física. Redução de medidas', restricoes:'Nunca tive', academia:'Ideal - Guaíba bairro santa rita', dataAnamnese:'2024-11-18', statusPlanoManual:'vencidas'},
  {nome:'Luisa Camila Buchert Moser', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'luisacbmoser@hotmail.com', telefone:'51999931612', piramide:'1 barriga 2 bumbum 3 pernas', objetivo:'Definição e crescimento', restricoes:'Nenhuma relatada', academia:'Yes fitness', dataAnamnese:'2024-11-21', statusPlanoManual:'vencidas'},
  {nome:'Carla Simone Viafore', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'carlasviafore@gmail.com', telefone:'51992116538', piramide:'Barriga, bumbum, braços e coxas', objetivo:'Emagrecer e ganhar massa muscular', restricoes:'joelho', academia:'Usina do corpo', dataAnamnese:'2024-11-25', statusPlanoManual:'vencidas'},
  {nome:'Denilson Bitencourt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'denilsonnasbit23@gmail.com', telefone:'51982074359', piramide:'1- barriga (concentração de gordura)  2- glúteo (concentração de gordura) 3- costas 4- Coxa', objetivo:'- Atingir e manter o peso entre 85/87kg  - Definição abdominal - Melhorar meu desempenho nos esporte', restricoes:'Sim, desgaste nos discos da coluna.Tive nesse ano fratura e deslocamente em um dedo do pé, mas hoje já não influencia em movimento e treinos; Sim, Lordose lombar, e desgaste de alguns discos.', academia:'Em casa, tenho alguns equipamentos e da para trabalhar todos os musculos, somente perna que faço um trabalho maior com o próprio peso do corpo', dataAnamnese:'2024-11-26', statusPlanoManual:'vencidas'},
  {nome:'Yasmin lara Souza gutheil', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'yasmimlara12345@gmail.com', telefone:'51992353733', piramide:'1- bumbum 2- coxa 3-barriga 4-braços', objetivo:'Secar a barriga e definir os músculos', restricoes:'Não tenho', academia:'Sesc', dataAnamnese:'2024-12-02', statusPlanoManual:'vencidas'},
  {nome:'Vinicius Bittencourt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'vinicius.m.bittencourt@hotmail.com', telefone:'51985692526', piramide:'Não tenho um específico', objetivo:'Emagrecimento e definição', restricoes:'Fratura nos dois braços', academia:'Do condomínio', dataAnamnese:'2024-12-06', statusPlanoManual:'vencidas'},
  {nome:'Ednaldo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'edh.protec@hotmail.com', telefone:'7745783341', piramide:'1234', objetivo:'Melhor postura, fortalecimento geral e melhor disposição', restricoes:'Nenhuma relatada', academia:'', dataAnamnese:'2024-12-07', statusPlanoManual:'vencidas'},
  {nome:'Tais Lavarda', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'angela.lavarda97@gmail.com', telefone:'51994280158', piramide:'Barriga, braços, bunda e coxa', objetivo:'Perder gordura', restricoes:'Tenho o ligamento do lado de dentro do joelho direito inflamado, já fiz ecografia e consultas', academia:'Yes fitness', dataAnamnese:'2024-12-07', statusPlanoManual:'vencidas'},
  {nome:'Jêniffer Tavares Zeferino', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'jeniffer.poa@hotmail.com', telefone:'51989491930', piramide:'1 membros inferiores 2 barriga 3 bumbum 4 costas', objetivo:'Melhorar o condicionamento físico e aumento de massa magra.', restricoes:'Sim. Tenho pés cavos, no qual tenho limitações: como por exemplo exercícios de impacto: correr.; Sim, cirurgia dos 6 aos 13 anos para correção dos pés cavos.', academia:'Academia condomínio', dataAnamnese:'2024-12-12', statusPlanoManual:'vencidas'},
  {nome:'Manassés Rafael dos Santos Torales', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'manassestorales@gmail.com', telefone:'51984239326', piramide:'3,4,2,1', objetivo:'Emagrecer, ganhar massa muscular', restricoes:'Nenhuma relatada', academia:'Smartfit', dataAnamnese:'2024-12-14', statusPlanoManual:'vencidas'},
  {nome:'Pedro Henrique Mendes Geremias Moreira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'pedromendes.drope@gmail.com', telefone:'51998501796', piramide:'1° barriga 2° braço 3° perna 4° costas', objetivo:'Emagrecer e definir', restricoes:'Faz uns 4 meses quebrei a cabeça do radio do braço esquerdo, próximo ao cotovelo', academia:'Ac fitnes', dataAnamnese:'2024-12-14', statusPlanoManual:'vencidas'},
  {nome:'Adriane Lopes Hermel', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'Adriane.hermel@gmail.com', telefone:'51995744214', piramide:'1- bumbum,2-coxa, 3- barriga, 4- costas', objetivo:'Ter os exercícios corretos pra certinho e ter um bom resultado', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo ou moinhos', dataAnamnese:'2025-01-29', statusPlanoManual:'vencidas'},
  {nome:'Felipe Motta batista', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'felipemottabatista@gmail.com', telefone:'51996957053', piramide:'3', objetivo:'Emagrecer e ganhar músculo', restricoes:'Nenhuma relatada', academia:'Em casa', dataAnamnese:'2025-02-10', statusPlanoManual:'vencidas'},
  {nome:'Caroline Pilastro', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'A definir', email:'carolinepilastro@outlook.com', telefone:'51990121794', piramide:'1 3 4 2', objetivo:'Alcançar meu objetivo com o foco personalizado', restricoes:'Não tenho, porem no periodo de colicas menstruais preferia fazer apenas superiores ou pesar mais no superior e menos inferior', academia:'Não treino', dataAnamnese:'2025-02-17', statusPlanoManual:'vencidas'},
  {nome:'Sindel piacini', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'sindelpiacini02@gmail.com', telefone:'51995806920', piramide:'Glúteo, costas, pernas, braços', objetivo:'Fortalecimento muscular.', restricoes:'Não.; Escoliose, mt dor ombros e lombar.', academia:'Smart Fit', dataAnamnese:'2025-02-21', statusPlanoManual:'vencidas'},
  {nome:'Daniel Valenzuela', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'dazanval10@gmail.com', telefone:'4704030613', piramide:'1) pernas, 2- barriga 3) ombros 4) braços', objetivo:'Hipertrofia e uma rotina de treino balanceada e eficiente', restricoes:'Nenhuma relatada', academia:'LA Fitness', dataAnamnese:'2025-02-21', statusPlanoManual:'vencidas'},
  {nome:'Larissa Santana', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'larissa.rsantana@gmail.com', telefone:'51992704595', piramide:'1-  bumbum, 2- barriga, 3- coxa, 4- braços', objetivo:'Definir e crescer. Sempre treinei e vi resultado apenas quando estava foçada de verdade e pegando bastante peso, agora que sei que consigo preciso de uma rotina mais regrada.  Meu foco é o mesmo de quase toda mulher, crescer glúteo e pernas, diminuir a barriga, definir superiores', restricoes:'Não que eu lembre', academia:'Performance', dataAnamnese:'2025-02-25', statusPlanoManual:'ativas'},
  {nome:'André Vitor Padilha da Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'a.padilhasilva@gmail.com', telefone:'53984568409', piramide:'Parelho', objetivo:'Perda de gordura, ganho de massa muscular', restricoes:'Sim, deslocamento patelar nos dois joelhos, patela rasa.; Patela rasa; Sim, dois joelhos', academia:'Engenharia do corpo', dataAnamnese:'2025-02-25', statusPlanoManual:'vencidas'},
  {nome:'Luiza Gabriella', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'lugabi0804@gmail.com', telefone:'51992079874', piramide:'1 bunda, 2 quadríceps, 3 barriga e cintura, 4 costas e braços', objetivo:'Chegar nos meus objetivos com treinos feitos pra mim. Ter auxílios em algumas dúvidas que eu não possa responder e entender um pouco mais sobre como funciona a transformação do corpo.', restricoes:'No meu primeiro dia de academia, fiz algo de errado, e no segundo treino depois de voltar pra casa, apareceu os sintomas de uma contratura na panturrilha. (Isso faz 1 mês) Repousei na época por uma semana praticamente, e depois continuei. Não conseguia andar normal.    Na enchente de 2023 já cai e bati o joelho bem forte no chão também, então talvez em algum momento tenha que fortalecer essas áreas.; Não.', academia:'Performance', dataAnamnese:'2025-03-02', statusPlanoManual:'vencidas'},
  {nome:'Camila Figueiredo Lemos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'cami.lemosfl@gmail.com', telefone:'51 98617-0882', piramide:'1, 2, 3 e 4', objetivo:'Perda de peso.', restricoes:'Não.; Não.', academia:'.', dataAnamnese:'2025-03-07', statusPlanoManual:'vencidas'},
  {nome:'Gabriela fortes Cabral dos santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'gabrielacabral229@gmail.com', telefone:'51990143996', piramide:'1-bumbum 2-barriga 3- coxa 4- costas', objetivo:'Corpo definido', restricoes:'Nenhuma relatada', academia:'Yes fitness', dataAnamnese:'2025-03-12', statusPlanoManual:'vencidas'},
  {nome:'Aline da silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'A definir', email:'aline_nathally@hotmail.com', telefone:'9547085856', piramide:'1. 2. 3. 4', objetivo:'Criar um corpo lindo e saudável', restricoes:'Nenhuma relatada', academia:'Lá fitness', dataAnamnese:'2025-03-23', statusPlanoManual:'vencidas'},
  {nome:'joao carlos lemos de souza filho', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'joao.contabil@yahoo.com.br', telefone:'51989502000', piramide:'1 abdômen 2 peito 3 costas  4 pernas', objetivo:'Fica igual ao ramon dino', restricoes:'Mao; Mao', academia:'Ctpeamce', dataAnamnese:'2025-04-05', statusPlanoManual:'vencidas'},
  {nome:'Renata', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'renatalopes2530@gmail.com', telefone:'51998064208', piramide:'1º Barriga, 2º pernas,3º coxa e 4º costas', objetivo:'Perder peso, perder barriga principalmente e ganhar mais pernas e bumbum', restricoes:'Dor no joelho direito , ele é para dentro e sinto que se Faço leg puxa pra dentro; Só esse detalhe no joelho parece que puxo para dentro quando faço leg ou agachamento', academia:'Ideia fitness', dataAnamnese:'2025-04-09', statusPlanoManual:'vencidas'},
  {nome:'Ana Paula Furini', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'paula_furini@hotmail.com', telefone:'51 984846432', piramide:'1-coxa, 2-abdômen, 3-gluteo, 4-biceps', objetivo:'Definição muscular e hipertrofia', restricoes:'Desloquei uma vértebra da lombar, normalmente não sinto dores ao praticar exercícios, mas cuido bastante ao aumentar a carga no treino das pernas pq forço e fica incomodando as vezes', academia:'Perfect da assis brasil', dataAnamnese:'2025-04-28', statusPlanoManual:'vencidas'},
  {nome:'Fernando Pereira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'fernandosilp1@gmail.com', telefone:'51998163667', piramide:'Abdome, ombros, panturrilha, costas', objetivo:'Corrigur os níveis de musculatura corporal em ambos os lados do corpo e ter músculos mais estéticos.', restricoes:'Lombar: Nada grave, mas sinto pressão na lombar fazendo agachamento com barra, sumô e encolhimento de ombros. Ombros: Sinto ombro ao fazer supino com barra, prefiro sempre com alteres.; Não.; Não.', academia:'Perfect', dataAnamnese:'2025-04-28', statusPlanoManual:'vencidas'},
  {nome:'Graziele Stefanno', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'graziele.moura.stefanno@gmail.com', telefone:'55996889180', piramide:'1 coxa, 2 barriga, 3 bumbum e 4 ombro, biceps, costas e  triceps', objetivo:'Perder gordura', restricoes:'Não! De vez em quando sinto dor no joelho esquerdo', academia:'Smartfit', dataAnamnese:'2025-05-12', statusPlanoManual:'ativas'},
  {nome:'Diego Almir da Trindade Machado', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'diego.trindade.machado@gmail.com', telefone:'51 991712262', piramide:'3, 4, 2, 1', objetivo:'Perder gordura', restricoes:'Leve no tendão de Aquiles; Não! Apenas uma leve dificuldade de mobilidade.', academia:'ALL Fit ou 26 Fit (pelo Gym Pass)', dataAnamnese:'2025-05-13', statusPlanoManual:'vencidas'},
  {nome:'Aline Gomes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'linygomes@gmail.com', telefone:'51993473308', piramide:'barriga, bumbum, coxa, braços', objetivo:'emagrecer', restricoes:'Nenhuma relatada', academia:'Allpfit', dataAnamnese:'2025-05-14', statusPlanoManual:'vencidas'},
  {nome:'Camila Couto Frazão', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'6x por semana', email:'ccoutofrazao@gmail.com', telefone:'(51) 9 8973-2353', piramide:'1- glúteo  2- coxa 3- posterior  4- costas', objetivo:'Perder peso q ganhei na gestação e melhorar a aparência dos músculos', restricoes:'Nenhuma relatada', academia:'CT performance', dataAnamnese:'2025-05-20', statusPlanoManual:'vencidas'},
  {nome:'Paula Schmitt', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'paulaschmittc@gmail.com', telefone:'+351 967981658', piramide:'1- coxa;  2- bumbum;  3- barriga;  4- costas;', objetivo:'Ganhar inferiores: meu corpo é em formato de triângulo invertido, meu objetivo é deixar mais harmônico.', restricoes:'Sim, hipertensão patelar no joelho esquerdo, estou tomando medicação por 60 dias. Acaba final de junho. Rantudil 90 retard. Em pouco tempo (1 ano) aumentei muito o peso e sobrecarregou meu joelho esquerdo, que é a perna mais fraca.; Sim, não posso treinar com muito peso, nem corrida ou impactos.', academia:'Uma academia de bairro', dataAnamnese:'2025-06-02', statusPlanoManual:'ativas'},
  {nome:'Barbara furnari', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'Barbaraalbieri@hotmail.com', telefone:'±14074856686', piramide:'Barriga', objetivo:'Perder peso', restricoes:'Nenhuma relatada', academia:'YMCA dr Philips', dataAnamnese:'2025-06-11', statusPlanoManual:'vencidas'},
  {nome:'Rafaela', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rafaelax3@live.com', telefone:'7742255608', piramide:'3', objetivo:'Weight loss', restricoes:'No; No; No', academia:'Planet fitness', dataAnamnese:'2025-06-12', statusPlanoManual:'ativas'},
  {nome:'Íris Fernanda Kessa de Jesus', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'iris-fernanda122011@hotmail.com', telefone:'17991789886', piramide:'1- barriga, 2- coxas, 3- bumbum, 4- costas', objetivo:'Emagrecer', restricoes:'Nenhuma relatada', academia:'Treinar em casa', dataAnamnese:'2025-06-14', statusPlanoManual:'vencidas'},
  {nome:'Kamila Milena Moura da Silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'kamilamoura98@hotmail.com', telefone:'51984502809', piramide:'barriga - 1 bumbum - 2 coxa - 3 costas - 4', objetivo:'emagrecimento e definição', restricoes:'Nenhuma relatada', academia:'Yes fitness', dataAnamnese:'2025-06-16', statusPlanoManual:'ativas'},
  {nome:'Mariana Gonçalves Soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'4x por semana', email:'soaresgmari.12@gmail.com', telefone:'9785309369', piramide:'Barriga, coxa, bumbum, costas', objetivo:'Alcançar o corpo dos meus sonhos', restricoes:'Nenhuma relatada', academia:'Planet fitness e em casa', dataAnamnese:'2025-06-17', statusPlanoManual:'vencidas'},
  {nome:'Fernando Soares', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'soaresfernando024@gmail.com', telefone:'51985006949', piramide:'Barriga, peito, braços e costas', objetivo:'Perder peso', restricoes:'Nenhuma relatada', academia:'Ainda estudando locais', dataAnamnese:'2025-07-18', statusPlanoManual:'vencidas'},
  {nome:'Fabiane severo de oliveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'fabiane_severo@outlook.com', telefone:'51-992635974', piramide:'Barriga, costas (reduzir as bordas de catupiry), bumbum, braços e coxas', objetivo:'Emagrecer', restricoes:'Cesárea há 4 meses', academia:'Em casa.', dataAnamnese:'2025-07-22', statusPlanoManual:'vencidas'},
  {nome:'Tamia Cunha', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'4x por semana', email:'lolocunha38@gmail.com', telefone:'(51)996449210', piramide:'1,3,4,2', objetivo:'Fortalecimento muscular', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2025-08-07', statusPlanoManual:'ativas'},
  {nome:'Andrieli Laux', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'andrieli.lauxx@gmail.com', telefone:'51997972100', piramide:'1- glúteo  2- quadríceps  3-ombro e costas  4- barriga', objetivo:'Emagrecimento é definição', restricoes:'Nenhuma relatada', academia:'CT commando (ao lado da minha casa)', dataAnamnese:'2025-08-07', statusPlanoManual:'vencidas'},
  {nome:'Lisandra Bernardy', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'lili.bernardy@gmail.com', telefone:'51997889980', piramide:'1º barriga  2º braços  3º glúteos  4º coxa', objetivo:'Perda de peso', restricoes:'Tive fratura na tíbia', academia:'CTperformance - Eldorado do Sul', dataAnamnese:'2025-08-07', statusPlanoManual:'ativas'},
  {nome:'Sofia Bamberg Cardozo dos Santos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'5x por semana', email:'sofibamberg@gmail.com', telefone:'51985464624', piramide:'1 bumbum (glúteo médio principalmente, não gosto do meu glúteo de lado) 2cintura  3quadriceps  4costas  Quero braços definidos também', objetivo:'Conseguir chegar em uma meta de estética de corpo', restricoes:'Tenho problema na canela por conta do futvôlei, mas maior maior dificuldade é em corrida', academia:'Smart Fit Shopping Walling', dataAnamnese:'2025-08-07', statusPlanoManual:'vencidas'},
  {nome:'Eduardo Denti', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'eng.eduardodenti@gmail.com', telefone:'51981151411', piramide:'1 . Costas 2. Ombros 3. Peito 4. Abdomen', objetivo:'Estética', restricoes:'Nenhuma relatada', academia:'Do meu condominio', dataAnamnese:'2025-08-07', statusPlanoManual:'vencidas'},
  {nome:'Bianca', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'biancasilveirars22@gmail.com.br', telefone:'51995219092', piramide:'3- 2-1-4 coloquei assim mas todas estão em primeiro', objetivo:'Perca de gordura e ganhar musculo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2025-08-07', statusPlanoManual:'vencidas'},
  {nome:'Bruna Bruniszaki', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'A definir', email:'bruniszakibru@gmail.com', telefone:'51995419409', piramide:'1costas, 2 bracos, 3 pernas, 4 abdomen', objetivo:'Acelerar a queima', restricoes:'Tenho síndrome do túnel do carpo', academia:'Performance ( sans souci )', dataAnamnese:'2025-08-09', statusPlanoManual:'vencidas'},
  {nome:'Victoria Medeiros Dorneles', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'medeirosdornelesv@gmail.com', telefone:'51983526469', piramide:'3 - 2 - 1 - 4', objetivo:'Emagrecimento e ganho de massa magra (Fortalecimento tb para ter mais desenvoltura no Cross)', restricoes:'Ano passado tive uma lesão no pescoço (torcicolo) fazendo um movimento no Crossfit.', academia:'Academia Perfect', dataAnamnese:'2025-08-12', statusPlanoManual:'vencidas'},
  {nome:'saimon guevara brum', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'saimonbrum@gmail.com', telefone:'51991397251', piramide:'1 costas 2 braço 3 coxa 4 abdomen', objetivo:'Emagrecimento é definicao', restricoes:'Dedo médio da mão direita mas não senti alterações', academia:'Perfect', dataAnamnese:'2025-08-15', statusPlanoManual:'vencidas'},
  {nome:'Mauro', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'mauro_junior1@hotmail.com', telefone:'51 999636817', piramide:'1- braços 2 - pernas 3 - peito 4 - costas', objetivo:'Emagrecimento e ganho de massa muscular', restricoes:'Ruptura do reto femural e ligamentos de tornozelo', academia:'Engenharia do corpo', dataAnamnese:'2025-08-19', statusPlanoManual:'vencidas'},
  {nome:'Angélica Nobre', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'nobangelica2019@gmail.com', telefone:'51989164716', piramide:'1- Barriga 2- costas 3- braços  4- bumbum', objetivo:'Emagrecer e fortalecer músculos', restricoes:'Sim, trinquei o cóccix há mais de 10 anos.', academia:'Sani academia', dataAnamnese:'2025-08-20', statusPlanoManual:'vencidas'},
  {nome:'Liane Cardoso guilherme', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'lcguilherme90@gmail.com', telefone:'51994661446', piramide:'1 - barriga, 2 - bumbum, 3 - costas e 4 - coxa', objetivo:'Emagrecimento e definição', restricoes:'Joelho', academia:'Não sei ainda', dataAnamnese:'2025-08-29', statusPlanoManual:'vencidas'},
  {nome:'Kewelin Alba', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'kewelinalba4@gmail.com', telefone:'51997340897', piramide:'1- Bumbum, 2- Barriga, 3- Coxa , 4- Costas', objetivo:'Desenvolver minha musculatura, gostar do que vejo no espelho.', restricoes:'Sim.; Sim. Ok, enviarei.; Não.', academia:'Perfect Academia Baltazar', dataAnamnese:'2025-08-31', statusPlanoManual:'vencidas'},
  {nome:'Juliana Viana Andrade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'juhvandrade@gmail.com', telefone:'51996328299', piramide:'costas, coxa, barriga e bumbum', objetivo:'resultado de definição', restricoes:'Nenhuma relatada', academia:'no litoral', dataAnamnese:'2025-09-01', statusPlanoManual:'vencidas'},
  {nome:'Jennifer Araujo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'jenniferaraujos@gmail.com', telefone:'51984725089', piramide:'1- barriga  2- pernas 3- braços  4- bumbum', objetivo:'Definir meu corpo, ter uma rotina ativa e saudável.', restricoes:'Nenhuma relatada', academia:'Yes', dataAnamnese:'2025-09-04', statusPlanoManual:'vencidas'},
  {nome:'Cindy Kittler', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'kittlercindy@gmail.com', telefone:'51991941952', piramide:'Barriga, costas, coxa, bumbum, (mas também adoro treino de ombro).', objetivo:'definir e perder gordura.', restricoes:'Tenho condromalácia no joelho direito. As vezes me dói ao treinar.', academia:'Rt Fitnees em Guaíba.', dataAnamnese:'2025-09-08', statusPlanoManual:'ativas'},
  {nome:'Gabriel Garcia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'gabrielgarcias@live.com', telefone:'51996662407', piramide:'1 - peito  2 - ombro  3 - tríceps  4 - costa', objetivo:'treinos personalizados', restricoes:'tenho uma lesão no joelho no momento onde me impede exercícios de pernas, estou aguardando a data da cirurgia (janeiro) para retornar, por hora o médico indicou somente superiores, caminhada e bike; enviarei', academia:'engenharia do corpo', dataAnamnese:'2025-09-08', statusPlanoManual:'vencidas'},
  {nome:'Jucelaine Antunes Simoes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'jucesimoes@hotmail.com', telefone:'51984554782', piramide:'3-1-2-4', objetivo:'Conseguir me dedicar com ajuda aos treinos', restricoes:'Nenhuma relatada', academia:'Top1  e Coliseu', dataAnamnese:'2025-09-11', statusPlanoManual:'ativas'},
  {nome:'Vanessa Macedo Pacheco', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'nessamp10@hotmail.com', telefone:'51996598036', piramide:'Barriga, coxa, bumbum, braços', objetivo:'Emagrecer e fortalecer o corpo', restricoes:'Tenho dores no quadril, mas quando fazia atividade não sentia nada; Não até o momento', academia:'Pretendo na performance', dataAnamnese:'2025-09-15', statusPlanoManual:'vencidas'},
  {nome:'Tarso marcuzzo', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'tcmarcuzzo13@gmail.com', telefone:'55996751312', piramide:'Peitoral', objetivo:'Perder bf', restricoes:'Nenhuma relatada', academia:'Eng do corpo', dataAnamnese:'2025-09-18', statusPlanoManual:'vencidas'},
  {nome:'Anna Caroliny Bordinhão Jorge', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'A definir', email:'anabordy29@gmail.com', telefone:'47997911252', piramide:'1. bumbum 2. coxa 3.  barriga 4. braços  5. costas', objetivo:'crescer', restricoes:'Nenhuma relatada', academia:'Roldão', dataAnamnese:'2025-09-25', statusPlanoManual:'ativas'},
  {nome:'Rodrigo da Cruz Lisboa', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'lisboa_r@hotmail.com', telefone:'53981554957', piramide:'Perna, peito, braço e costas', objetivo:'Ajustar meus treinos na academia/reforço muscular', restricoes:'Nenhuma relatada', academia:'Moinhos fitness - menino deus', dataAnamnese:'2025-09-26', statusPlanoManual:'ativas'},
  {nome:'Luara', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'2x por semana', email:'luara.franceschini@yahoo.com.br', telefone:'51989099916', piramide:'1 barriga/2 pernas / 3 bumbun/ 4 braços', objetivo:'Emagrecer 12/15 kg e voltar minha alto estima', restricoes:'Nenhuma relatada', academia:'Ideia ou RT jardim dos lagos', dataAnamnese:'2025-09-29', statusPlanoManual:'vencidas'},
  {nome:'Débora Rischtter', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'3x por semana', email:'debora.rischtter@gmail.com', telefone:'51 99102-4492', piramide:'3 - 1 - 2 - 4', objetivo:'Diminuir percentual gordura', restricoes:'Não lesão, mas senti a lombar no sumo terra e tive que reduzir carga e fortalecer.', academia:'WKT centro de treinamento', dataAnamnese:'2025-09-29', statusPlanoManual:'vencidas'},
  {nome:'Luigi Canova', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'luigi.canova@gmail.com', telefone:'51 993598824', piramide:'Barriga, peito, costas, pernas', objetivo:'Emagrecimento junto com a dieta do Thales', restricoes:'Braço quebrado quando criança', academia:'Não sei', dataAnamnese:'2025-09-30', statusPlanoManual:'vencidas'},
  {nome:'Bruna', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'brunalarissafreire90@gmail.com', telefone:'38988167643', piramide:'Braços e Costa  Tem mais gordura acumulada)', objetivo:'Emagrecimento', restricoes:'Escoliose e Lordose', academia:'Em dúvida FULLTIME ou WCT', dataAnamnese:'2025-10-01', statusPlanoManual:'vencidas'},
  {nome:'Rafaela Carvalho da Silveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'6x por semana', email:'raffasilveira_@hotmail.com', telefone:'5199346-8765', piramide:'Coxa, costas, bumbum, barriga', objetivo:'Definir mais o Corpo', restricoes:'Nenhuma relatada', academia:'Wave', dataAnamnese:'2025-10-06', statusPlanoManual:'vencidas'},
  {nome:'Ariane Garcia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'1x por semana', email:'ferraz-ariane@hotmail.com', telefone:'51989546730', piramide:'1- barriga 2- pernas 3-bracos 4-costa', objetivo:'Perder peso e criar uma rotina ativa de exercícios', restricoes:'Nenhuma relatada', academia:'Em casa', dataAnamnese:'2025-10-12', statusPlanoManual:'vencidas'},
  {nome:'Flávia', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'fonoflaviamachado@gmail.com', telefone:'51996355144', piramide:'coxa, costas, barriga, bumbum', objetivo:'Ganho de massa e definição', restricoes:'Nenhuma relatada', academia:'Olympo', dataAnamnese:'2025-10-15', statusPlanoManual:'vencidas'},
  {nome:'Marciele Girelli', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'marcielegirelli3@gmail.com', telefone:'51996500970', piramide:'1bumbum ,3barriga,2coxa 4 costas', objetivo:'Perder barriga e ganhar massa magra', restricoes:'Já tive no joelho', academia:'Em Porto Alegre , não conheço academia ainda não sei quais os aparelhos ela tem , vou ir nela por ser do lado da empresa', dataAnamnese:'2025-10-15', statusPlanoManual:'ativas'},
  {nome:'Ariana Marques', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'ariane-ashley@hotmaiil.com', telefone:'51985981461', piramide:'Abdômen e bumbum', objetivo:'Definir', restricoes:'Nenhuma relatada', academia:'Engenharia do corpo', dataAnamnese:'2025-10-16', statusPlanoManual:'vencidas'},
  {nome:'Rafaella Martins Trindade', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rafaellamtrindade@gmail.com', telefone:'51 996403997', piramide:'Barriga , bumbum ,coxa, costas', objetivo:'Resultado de um corpo moldado e fortalecido', restricoes:'Tornozelo nada grave', academia:'Performance', dataAnamnese:'2025-10-20', statusPlanoManual:'vencidas'},
  {nome:'Luciane', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'apenasluh@hotmail.com', telefone:'51984503225', piramide:'1,2,3,4', objetivo:'Definição', restricoes:'Abdominoplastia', academia:'Ct performance', dataAnamnese:'2025-10-21', statusPlanoManual:'vencidas'},
  {nome:'Lucianna Moraes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'4x por semana', email:'luaparticheli@gmail.com', telefone:'(51)993866665', piramide:'Emagrecer, músculo venha depois. 3 e 4', objetivo:'Emagrecer', restricoes:'Sim, já fraturei o metatarso correndo na esteira quando fazia academia de segunda a sábado.', academia:'Smart Fit', dataAnamnese:'2025-10-29', statusPlanoManual:'vencidas'},
  {nome:'Tania Maris Reus Salam von Saltiel', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Intermediário', freq:'3x por semana', email:'tania.salam@hotmail.com', telefone:'(51)992801119', piramide:'3', objetivo:'Perder peso', restricoes:'Fiz uma cirurgia de artrodese na lombar; Cirurgia coluna; Não posso posso realizar treino Legpress', academia:'Sesc Protasio Alves', dataAnamnese:'2025-11-07', statusPlanoManual:'vencidas'},
  {nome:'Sabrina', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'sabrinarockenbach.mkt@gmail.com', telefone:'51993555544', piramide:'1 - bumbum / 2 - cintura/barriga / 3 - costas. 4 - coxas', objetivo:'Afinar cintura e dar forma no bumbum.', restricoes:'Nenhuma relatada', academia:'Cia do corpo', dataAnamnese:'2025-11-08', statusPlanoManual:'vencidas'},
  {nome:'Fernanda', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'fernandareichimbak@gmail.com', telefone:'51991878598', piramide:'1-barriga,2- coxa, 3-bumbum, 4-costas', objetivo:'Perder gordura e definir', restricoes:'Nenhuma relatada', academia:'Ct Performance', dataAnamnese:'2025-11-11', statusPlanoManual:'vencidas'},
  {nome:'Tacianne Abreu', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'4x por semana', email:'tacianneabreu@gmail.com', telefone:'51996623475', piramide:'1, 3, 4,2', objetivo:'Definição muscular', restricoes:'Não possuo', academia:'Smart Fit', dataAnamnese:'2025-11-17', statusPlanoManual:'ativas'},
  {nome:'Rita Lenise de Vargas Ascenço', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rita.lenise@gmail.com', telefone:'51999165692', piramide:'3 2 4 1', objetivo:'Emagrecer, para saúde e auto estima', restricoes:'Sim', academia:'Posso treinar em casa ou só somente na academia', dataAnamnese:'2025-11-18', statusPlanoManual:'vencidas'},
  {nome:'Lea costa Borges', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'leaborges12@gmail.com', telefone:'51999899593', piramide:'Posterior e glúteos, costas abdômen', objetivo:'Treino mais específico', restricoes:'Sim; Sim; Só no ombro, mas só tenho muito força', academia:'Yes academia', dataAnamnese:'2025-11-18', statusPlanoManual:'ativas'},
  {nome:'Ester Dalla Valle Lopes', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'A definir', freq:'3x por semana', email:'ester.lopes17@gmail.com', telefone:'51992920228', piramide:'bumbum, coxa, barriga e costas...', objetivo:'Fazer um treinamento assertivo para ter mais qualidade nos treinos, para ter perda de peso, ganho de massa magra, melhorar o condicionamento físico, fortalecer a coluna', restricoes:'tive uma lesão na coluna, não me recordo se era inicio de uma hernia, mas isso foi em 2018, fiz pilates e melhorou muito, como tenho seios muito grandes, sinto muita dor nas costas,; não tenho nenhuma cirurgia que  comprometa os movimentos', academia:'SESC - navegantes', dataAnamnese:'2025-11-19', statusPlanoManual:'vencidas'},
  {nome:'Rafaela Carvalho', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'rafaela.c99@gmail.com', telefone:'51995680554', piramide:'1 glúteo 2 barriga 3 coxa,4- costas', objetivo:'Alavancar meus resultados e chegar com o corpo que desejo', restricoes:'Sinto dores isoladas no joelhos, diante a uma consulta foi diagnosticado desgaste mas até o momento não interferiu nos treinos.', academia:'Ct performance', dataAnamnese:'2025-11-19', statusPlanoManual:'vencidas'},
  {nome:'Íris Cunha', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'4x por semana', email:'iriscunha60@gmail.com', telefone:'923804940', piramide:'3 e 1', objetivo:'Emagrecer e firmar o corpo', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2025-11-20', statusPlanoManual:'ativas'},
  {nome:'Sandrine', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'sandrine.rosa1@gmail.com', telefone:'51991230734', piramide:'1- bumbum 2-coxas 3-bracos 4-costas', objetivo:'Emagrecimento e definição', restricoes:'Desvio na cervical', academia:'Performance', dataAnamnese:'2025-11-20', statusPlanoManual:'vencidas'},
  {nome:'Carolina Prates', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'carolinaprates99@gmail.com', telefone:'51992101341', piramide:'1º- barriga, 2º perna, 3º bumbum, 4º costas', objetivo:'Emagrecer e definir', restricoes:'Nenhuma relatada', academia:'Pretendo treinar na Performance', dataAnamnese:'2025-11-21', statusPlanoManual:'vencidas'},
  {nome:'Igor Piacini', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'igor_piacini10@hotmail.com', telefone:'51995861984', piramide:'Conjunto completo.', objetivo:'Melhorar a qualidade do treino e consequentemente o fisico', restricoes:'Nenhuma relatada', academia:'Performance fitnnes', dataAnamnese:'2025-11-27', statusPlanoManual:'vencidas'},
  {nome:'schaienne corsini da silva', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'6x por semana', email:'schaicorsini@gmail.com', telefone:'55991967136', piramide:'coxa, bumbum, costas, barriga', objetivo:'fortalecer musculatura, evitar recidiva cancer', restricoes:'estou com um pouco de limitação no braço, devido a cirurgia de retirada de mamas e linfonodos que foi feita em fevereiro deste ano.; somente o braço que sinto um pouco', academia:'26fit ou up nova tramandai', dataAnamnese:'2025-11-28', statusPlanoManual:'vencidas'},
  {nome:'Ândria Rosa Dias', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'andriadias193@gmail.com', telefone:'51 999448920', piramide:'3,4,1,2', objetivo:'Quero secar e definir ao maximo. Não quero aquele corpo monstro, quero o mais slim possivel.', restricoes:'Sim, na perna direita. Quebrei a tibia e a fibia e com isso tenho menos fornça nessa perna e também sinto dores nas costas pq usei muleta por muito tempo.; Sim e tenho Hipertensão Arterial Pulmonar; Na tibia e na fibia', academia:'Varia muito, pois estou sempre viajando', dataAnamnese:'2025-12-01', statusPlanoManual:'ativas'},
  {nome:'Lizandra da Silva Melos', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'lizanelos@yahoo.com.br', telefone:'51-998274337', piramide:'3- 1- 2 - 4', objetivo:'Ser cobrada para chegar aos resultado.', restricoes:'Coloquei silicone a 2 meses e meio', academia:'Performance', dataAnamnese:'2025-12-02', statusPlanoManual:'ativas'},
  {nome:'Gabriela', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'A definir', email:'gabrielaamador284@gmail.com', telefone:'51997328538', piramide:'1 bumbum 2 barriga  3 coxa  4 costas', objetivo:'Chegar ao meu desejo não só em estética mais com saúde e bem estar', restricoes:'Sim; Tenho uma hérnia discal ,e um emagioma na coluna e desgaste de algumas vértebras', academia:'26 fitnes', dataAnamnese:'2025-12-08', statusPlanoManual:'vencidas'},
  {nome:'Eduarda Reis', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'3x por semana', email:'eduardareisfreitas00@gmail.com', telefone:'51 999712424', piramide:'1, 2, e 3', objetivo:'Definir o corpo', restricoes:'Sim, no tornozelo', academia:'Smart fit', dataAnamnese:'2025-12-08', statusPlanoManual:'ativas'},
  {nome:'Gabriel de Matos Teixeira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'gabmattos489@gmail.com', telefone:'51993890755', piramide:'1° Braços 2° Ombro e costas 3° Peito e Abdômen 4° Pernas', objetivo:'Ganhar massa muscular, definição e conseguir desenvolver força. Tenho também a vontade de aprender melhor a fazer cada exercício e seguir um treino que me faça evoluir de fato.', restricoes:'Nenhuma relatada', academia:'Smart Fit Boubron Wallig', dataAnamnese:'2025-12-09', statusPlanoManual:'vencidas'},
  {nome:'Adriana Schnadelbach', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Avançado', freq:'5x por semana', email:'schnadelbachadriana90@gmail.com', telefone:'51995027512', piramide:'Coxa, bumbum, barriga e costas', objetivo:'Meu objetivo é fazer um treino que me de resultados.', restricoes:'Não tenho lesão, porém me sinto bem fraca nos últimos tempos', academia:'Performance', dataAnamnese:'2025-12-10', statusPlanoManual:'vencidas'},
  {nome:'Amanda de Lima Silveira', status:'lead', statusLabel:'Lead antigo · a confirmar', nivel:'Iniciante', freq:'5x por semana', email:'amandalimasilveira0905@outlook.com', telefone:'51 980476473', piramide:'1 bumbum 2 coxa 3 costas barriga', objetivo:'ter constância nos exercicios fisicos no ano de 2026, estetica', restricoes:'Nenhuma relatada', academia:'yes ou performance', dataAnamnese:'2025-12-30', statusPlanoManual:'vencidas'},
  {nome:'Herica', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'A definir', email:'herica.pin.lorena@gmail.com', telefone:'51997491112', piramide:'Bumbum,barriga,costas ,pernas', objetivo:'Perder gordura e criar massa muscular', restricoes:'Nenhuma relatada', academia:'Perfomece', dataAnamnese:'2026-01-08', statusPlanoManual:'vencidas'},
  {nome:'Henrique Cogo', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'henriquecogo97@gmail.com', telefone:'51998480868', piramide:'1- OMBRO E TRAPEZIO  2- PEITO 3-COXAS 4-BRAÇOS', objetivo:'BUSCAR MELHOR FORMA', restricoes:'NADA', academia:'PERFOMECE', dataAnamnese:'2026-01-08', statusPlanoManual:'vencidas'},
  {nome:'Tainara Arruda', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'6x por semana', email:'tainarafarruda@gmail.com', telefone:'51985406087', piramide:'Barriga, bumbum, coxas, costas', objetivo:'Secar, perder peso, definição', restricoes:'Nenhuma relatada', academia:'Yes fitnes', dataAnamnese:'2026-01-09', statusPlanoManual:'vencidas'},
  {nome:'Jefferson nichetti', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'peckerjefferson@gmail.com', telefone:'51981606548', piramide:'Peito , costas , tríceps e perna', objetivo:'Secar e definir', restricoes:'Desgaste na l3 l4 e l4 l5; Sim', academia:'Yesfitnes', dataAnamnese:'2026-01-09', statusPlanoManual:'vencidas'},
  {nome:'Agatha', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'3x por semana', email:'agathasije@gmail.com', telefone:'51989602709', piramide:'1 barriga  2 coxa  3 bunda  4 costa', objetivo:'Emagrecer e definir', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-01-10', statusPlanoManual:'ativas'},
  {nome:'Rafaela Vargas da Silva', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'vargasdasilvarafaela@gmail.com', telefone:'51 982262405', piramide:'1- barriga, 2- bumbum, 3- coxas, 4- costas', objetivo:'Hipertrofia', restricoes:'Nenhuma relatada', academia:'A do meu condomínio', dataAnamnese:'2026-01-12', statusPlanoManual:'vencidas'},
  {nome:'Josiani', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'josiani.davila@gmail.com', telefone:'51 98121 1879', piramide:'1 bumbum, 2 barriga, 3 coxa, 4 costa', objetivo:'Ter resultado eficientes', restricoes:'Nenhuma relatada', academia:'Sani corpori', dataAnamnese:'2026-01-12', statusPlanoManual:'vencidas'},
  {nome:'Bruna Tierling', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'3x por semana', email:'brunnaltier@gmail.com', telefone:'+52 561 1124600', piramide:'Glúteo, coxas, costas, braços', objetivo:'Fortalecimento muscular', restricoes:'Sim; Condromalasia grau 3, quase 4, com indicação de cirurgia e aplicação de Synvisc one 1x ao ano', academia:'Predio', dataAnamnese:'2026-01-16', statusPlanoManual:'vencidas'},
  {nome:'Katheyn', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'katheyncarrero31@gmail.com', telefone:'51984843108', piramide:'1234', objetivo:'Melhorar a qualidade de vida e a estética física', restricoes:'Nenhuma relatada', academia:'Quero um treino q possa executar em casa e parques, mas também em academia', dataAnamnese:'2026-01-17', statusPlanoManual:'vencidas'},
  {nome:'Nathália Pomina da Silva', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'4x por semana', email:'nathaliapomina@gmail.com', telefone:'51989489995', piramide:'1 bumbum , 2 barriga, 3 coxa, 4 superiores', objetivo:'Ter o corpo que desejo e me sentir bem', restricoes:'Cirurgia no tornozelo mas não compromete em nada', academia:'Usina do corpo', dataAnamnese:'2026-01-19', statusPlanoManual:'vencidas'},
  {nome:'Kryslen Ribeiro', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'2x por semana', email:'krys.leen@gmail.com', telefone:'51993829822', piramide:'1º reforço de glúteo e quadríceps 2° costas e ombro 3° abdômen', objetivo:'Fortalecimento para corrida', restricoes:'Sim, no quadril', academia:'Perfect', dataAnamnese:'2026-01-23', statusPlanoManual:'vencidas'},
  {nome:'Francielli Loss Volpatto', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'franlvolpatto@gmail.com', telefone:'51 989316231', piramide:'1: Coxas; 2: triceps; 3: glúteo 4: costas', objetivo:'Definição muscular, hipertrofia', restricoes:'Nenhuma relatada', academia:'Engenharia do Corpo', dataAnamnese:'2026-01-26', statusPlanoManual:'vencidas'},
  {nome:'Yasmin', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'yasminccamilo28@gmail.com', telefone:'51996529938', piramide:'1º bumbum 2º coxas 3º barriga 4º costas', objetivo:'alcançar o corpo desejado', restricoes:'lesão ligamentar no tornozelo direito; não tenho', academia:'performance', dataAnamnese:'2026-01-29', statusPlanoManual:'vencidas'},
  {nome:'Luany Braga Prado', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'3x por semana', email:'luanybraga57@gmail.com', telefone:'51980359459', piramide:'1: glúteo 2: coxa 3: costas  4: barriga', objetivo:'Ganhar massa muscular, ficar definida, ganhar mais bumbum e coxas', restricoes:'Nenhuma relatada', academia:'Ct performance', dataAnamnese:'2026-01-29', statusPlanoManual:'vencidas'},
  {nome:'Danielle Kressin', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'kressindanny@iclod.com', telefone:'51983140058', piramide:'Coxa/bumbum/costas/abdome', objetivo:'Crescer massa muscular', restricoes:'Já tive lombar', academia:'Moinhos fit', dataAnamnese:'2026-02-03', statusPlanoManual:'ativas'},
  {nome:'Édlyn Carvalho Schenkel', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'edlyncarvalhoschenkel@gmail.com', telefone:'51992266540', piramide:'costas, braços (foco em acabar o tchauzinho e ficar com os braços fortes), barriga, pernas/bumbum', objetivo:'definição, foco específico e dicas (aprender ir até a falha, progredir carga, equilibrar os exercícios corretamente)', restricoes:'nao, canelite as vezes na corrida', academia:'ct performance', dataAnamnese:'2026-02-03', statusPlanoManual:'vencidas'},
  {nome:'Raquel Teixeira', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'raquelsonda79@gmail.com', telefone:'51992841469', piramide:'1 - coxa 2 - braços  3 - bumbum  4- flanco (pneu)', objetivo:'Emagrecer e definir', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-02-11', statusPlanoManual:'vencidas'},
  {nome:'Sabine do Nascimento', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'3x por semana', email:'ssabinenascimento@gmail.com', telefone:'21979064840', piramide:'1° Abdômen, 2° braços( Tríceps e costas), 3° Glúteos e 4° Pernas (Posterior e Interno de coxas).', objetivo:'Queima de gordura e melhora na composição corporal.', restricoes:'Sim, estou com bursite no quadril esquerdo.; Bursite no quadril esquerdo em tratamento, fisioterapeuta.; Não.', academia:'Smart fit', dataAnamnese:'2026-02-16', statusPlanoManual:'ativas'},
  {nome:'Kátia Fabiane Schneider', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'A definir', email:'katiadj12@hotmail.com', telefone:'51 99207-0519', piramide:'1-bumbum 2-coxa 3-posterior e o 4 deixo a critério do profissional, conforme necessidade.', objetivo:'Deixar o corpo mais harmônico, aumentando os inferiores', restricoes:'coluna lombar - vou enviar o laudo e uma imagem', academia:'Athrix Academia em Feliz e feriados e folgas Mais Fitnnes em Presidente Lucena', dataAnamnese:'2026-02-19', statusPlanoManual:'ativas'},
  {nome:'Vanessa Morfan', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'vanessamorfan@icloud.com', telefone:'51995836100', piramide:'Barriga - bumbum - coxa e costas', objetivo:'Melhirar o corpo kk e Melhorar a qualidade de vida ênfase na saúde também', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-02-20', statusPlanoManual:'ativas'},
  {nome:'Maria eduarda', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'2x por semana', email:'eduardacruzermachado2003@gmail.com', telefone:'51986076491', piramide:'barriga,coxa e bunda', objetivo:'ajuda pra melhorar o necessário', restricoes:'escoliose', academia:'perfomance', dataAnamnese:'2026-02-21', statusPlanoManual:'vencidas'},
  {nome:'Sabrina Menger', status:'ok', statusLabel:'Ativa recente', nivel:'Intermediário', freq:'3x por semana', email:'sabrinamenger@gmail.com', telefone:'(51)994962597', piramide:'1 - glúteos 2 - braços 3 - costas 4 - posterior coxa', objetivo:'Definir', restricoes:'Nenhuma relatada', academia:'Moinhos', dataAnamnese:'2026-02-23', statusPlanoManual:'vencidas'},
  {nome:'Cinara Bairros Vida', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'4x por semana', email:'cinarabairros@gmail.com', telefone:'51999592399', piramide:'Barriga, coxa, bumbum e costas', objetivo:'Meu objetivo massa muscular, fortalecimento, definição e melhorar desempenho na corrida.', restricoes:'Não tenho a cartilagem no joelho e tive tendinite pata de ganso devido a corrida.; Somente o cuidado com o joelho.', academia:'Moinhos', dataAnamnese:'2026-02-26', statusPlanoManual:'ativas'},
  {nome:'Márcia Alves', status:'ok', statusLabel:'Ativa recente', nivel:'Intermediário', freq:'3x por semana', email:'dinerei.marcia@icloud.com', telefone:'51 995475409', piramide:'1 barriga 2 bumbum 3 costas 4 coxa', objetivo:'Emagrecimento', restricoes:'Desgaste de patela do joelho direito e esquerdo - direito está mais comprometido.', academia:'Tons de rosa ou Yess', dataAnamnese:'2026-03-05', statusPlanoManual:'ativas'},
  {nome:'Simoni Adão Rocha', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'simoninutricionista@gmail.com', telefone:'51997974377', piramide:'Glúteo, posterior, abdômen costas', objetivo:'Me ajudar a sair desta estagnação', restricoes:'Um pouco de dores nos joelhos e encaixe da coxa no quadril nos últimos meses', academia:'CT performance e moinhos fitness Bourbon shopping', dataAnamnese:'2026-03-07', statusPlanoManual:'ativas'},
  {nome:'Renata Brogni da Silva', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'A definir', email:'renata_brogni@hotmail.com', telefone:'981818145', piramide:'1-2-4-3', objetivo:'Melhorar a questão do condicionamento físico e flacidez', restricoes:'Tenho tenosinovite no tornozelo direito', academia:'Performace', dataAnamnese:'2026-03-27', statusPlanoManual:'ativas'},
  {nome:'Betina Gauna Vincenti', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'betinagauna504@gmail.com', telefone:'51989601705', piramide:'3- barriga, 1-bumbum, e o resto todo', objetivo:'Emagrecer e ganhar massa moscular', restricoes:'Nenhuma relatada', academia:'CT PERFORMANCE', dataAnamnese:'2026-03-27', statusPlanoManual:'ativas'},
  {nome:'Eliane Fátima Crupinski', status:'ok', statusLabel:'Ativa recente', nivel:'Intermediário', freq:'3x por semana', email:'elianecrupinskiagro@gmail.com', telefone:'51996504863', piramide:'1- barriga afinar cintura, pochete e peneuzinho. 2- Glúteo 3 perna 4 costas', objetivo:'definir corpo', restricoes:'Nenhuma relatada', academia:'ct performance', dataAnamnese:'2026-03-29', statusPlanoManual:'ativas'},
  {nome:'JEFFERSON RUBIM FERREIRA', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'A definir', email:'rubim_07@hotmail.com', telefone:'51999175222', piramide:'1° Braços 2° Peito 3° Ombro 4° Barriga', objetivo:'Hipertrofia', restricoes:'Joelho esquerdo (mais desconforto na corrida) Motivo: futebol; Apenas desgaste nos joelhos', academia:'Smartfit', dataAnamnese:'2026-04-09', statusPlanoManual:'vencidas'},
  {nome:'Jaíne Menezes dos Santos', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'4x por semana', email:'jaine_menezes13@hotmail.com', telefone:'51997867893', piramide:'Braços Coxa Barriga Bumbum', objetivo:'Saúde e emagrecimento', restricoes:'Fiz cirurgia de Joanete a uns 7 anos atrás.', academia:'Do condomínio', dataAnamnese:'2026-04-09', statusPlanoManual:'ativas'},
  {nome:'Renata Melo Rodrigues', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'renatamelorodrigues1@gmail.com', telefone:'55 51985538500', piramide:'1 abdômen  2 coxas  3 bumbum  4 costas', objetivo:'Emagrecimento sem flacidez', restricoes:'Inflamação no ombro', academia:'Focus gym', dataAnamnese:'2026-04-10', statusPlanoManual:'ativas'},
  {nome:'Rafael Ferrao', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'3x por semana', email:'rafsferrao@gmail.com', telefone:'51989437623', piramide:'Barriga; Peito; Costas; Coxa', objetivo:'Perder peso', restricoes:'Nenhuma relatada', academia:'Panobianco', dataAnamnese:'2026-04-13', statusPlanoManual:'vencidas'},
  {nome:'Janderson Cunha Roszkowski', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'3x por semana', email:'jandogremista@gmail.com', telefone:'51995752625', piramide:'2,3,1,4', objetivo:'Fortalecer e trincar kkkk', restricoes:'Nenhuma relatada', academia:'Vou começar com o seu plano', dataAnamnese:'2026-04-13', statusPlanoManual:'vencidas'},
  {nome:'Franciele Traub', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'traubfranciele@gmail.com', telefone:'51997547979', piramide:'Barriga  Coxa Bumbum Costas', objetivo:'Perder barriga, definir cintura, perna e glúteo kkkkk', restricoes:'Nenhuma relatada', academia:'Ideia fitness', dataAnamnese:'2026-04-17', statusPlanoManual:'ativas'},
  {nome:'Marcelo dos Reis da silva junior', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'4x por semana', email:'marceloreissilvajr@gmail.com', telefone:'51996808282', piramide:'Peito ombro Costa barriga', objetivo:'Resultado', restricoes:'Sim tenho uma lesão no joelho direito já fiz cirurgia; No joelho direito', academia:'Smart Fit bourbon Ipiranga e ct performance', dataAnamnese:'2026-04-19', statusPlanoManual:'ativas'},
  {nome:'Eduarda', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'4x por semana', email:'duda.mentz@gmail.com', telefone:'51995105977', piramide:'2, 1, 3 e 4', objetivo:'Emagrecer e tonificar', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-04-22', statusPlanoManual:'vencidas'},
  {nome:'Gislaine Preto Pires', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'2x por semana', email:'gpretopires@gmail.com', telefone:'(51) 99986-7884', piramide:'2', objetivo:'Resultado', restricoes:'Nenhuma relatada', academia:'Performance', dataAnamnese:'2026-04-22', statusPlanoManual:'ativas'},
  {nome:'Gisele de Abreu Moraes', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'4x por semana', email:'GiseleMoraes1704@gmail.com', telefone:'51980122191', piramide:'1-coxas 2-glúteo  3- braços (emagreci muito na época que descobri diabetes e fiquei com sobra de pele) 4- costas', objetivo:'Potencializar resultados junto com acompanhamento nutricional, buscando melhorar o processo de emagrecimentos e de hipertrofia', restricoes:'Sim! Rompimento do menisco esquerdo! Cirurgia de meniscectomia realizada em dezembro de 2024 mas hoje vida normal não me limita em nada.; Cirurgia de retirada de menisco', academia:'Prime fit', dataAnamnese:'2026-04-23', statusPlanoManual:'vencidas'},
  {nome:'Viviane da Cunha Moreira', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'4x por semana', email:'vi.dacunha@gmail.com', telefone:'51 992907258', piramide:'Barriga, coxa, bumbum, costas', objetivo:'Crescer', restricoes:'Não, mas meu joelho está dando sinais da idade, hahahh', academia:'Total', dataAnamnese:'2026-05-04', statusPlanoManual:'ativas'},
  {nome:'Joice Lima', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'5x por semana', email:'joiceclima97@gmail.com', telefone:'51995207714', piramide:'1 quadríceps, bumbum, barriga e costas', objetivo:'Definição', restricoes:'Nenhuma relatada', academia:'Ct performance', dataAnamnese:'2026-05-17', statusPlanoManual:'ativas'},
  {nome:'Carolina pires', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'4x por semana', email:'carolzitz@gmail.com', telefone:'51 999845911', piramide:'Glúteo/ barriga / glúteo / barriga/ glúteo', objetivo:'Emagrecimento e hipertrofia', restricoes:'Nenhuma relatada', academia:'Usina do corpo', dataAnamnese:'2026-05-22', statusPlanoManual:'ativas'},
  {nome:'Ariel Morais Ferreira', status:'ok', statusLabel:'Ativa recente', nivel:'Avançado', freq:'3x por semana', email:'ariiel.pdc@gmail.com', telefone:'51982362037', piramide:'1 coxa, 2 bumbum, 3 tríceps, 4 barriga', objetivo:'Definir coxa e bumbum', restricoes:'Condromalacia patelar nos dois joelhos e desgaste no menisco no joelho esquedo; Ressonância magnética dos dois joelhos', academia:'Ideia Fitness', dataAnamnese:'2026-06-07', statusPlanoManual:'vencidas'},
  {nome:'Sinara', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'2x por semana', email:'silvasinara704@gmail.com', telefone:'51980165964', piramide:'3 barriga', objetivo:'Perder abdômen', restricoes:'Nenhuma relatada', academia:'Casa', dataAnamnese:'2026-06-11', statusPlanoManual:'vencidas'},
  {nome:'JULIANA HERMES PEREIRA REGO', status:'ok', statusLabel:'Ativa recente', nivel:'A definir', freq:'5x por semana', email:'jhpr16@gmail.com', telefone:'51998005817', piramide:'1- braço / 2 - coxa / 3 - barriga / 4 - costas', objetivo:'Emagrecer e tornear', restricoes:'Nenhuma relatada', academia:'Yes Fitnnes', dataAnamnese:'2026-06-22', statusPlanoManual:'vencidas'},
  {nome:'Andriele Caroline Rubert', status:'ok', statusLabel:'Ativa recente', nivel:'Iniciante', freq:'5x por semana', email:'andrielecarolinerubert@gmail.com', telefone:'51991423839', piramide:'1- bumbum, 2- barriga(perder), 3- pernas, 4- costas', objetivo:'Esse ano decidi que vou focar no meu corpo e deixar ele de um jeito que eu me sinta bem, já perdi muito peso mas nunca fiquei com o corpo que queria, agora estou focada nisso e acredito que tu pode me auxiliar nesse processo', restricoes:'Dor no joelho ao fazer afundo ou stiff, excluídos da prescrição inicial', academia:'Performance', dataAnamnese:'2026-06-30', idade:30, queixaDor:true, regiaoQueixa:'Joelho', desviosPosturaisConfirmados:['ombros_protrusos'], statusPlanoManual:'ativas',
   peso:'101 kg', altura:'1,69 m', imc:'35,4 (fase 1: emagrecimento prioritário)',
   treinoAtual:{fase:'Base', volume:'Dentro do limite de iniciante · até 20 séries/semana por ênfase',
     dias:[
...
const mensagensExemploFunil = {
  ...
  recuperacao: [
    { titulo: 'Sentimos sua falta', texto: 'Oi! Faz um tempo que a gente não se fala. Como você está? Queria saber se ainda faz sentido pra você retomar os treinos com a gente.' },
    { titulo: 'Retorno com condição especial', texto: 'Preparei uma condição especial pra quem está voltando. Topa marcar uma conversa rápida pra eu te contar?' },
    { titulo: 'Check-in sem compromisso', texto: 'Sem cobrança nenhuma, só queria saber como você está e se o objetivo de antes ainda é o mesmo. Se quiser retomar, estou por aqui.' }
  ]
};
...
function alunaNoEstagioFunil(a, estagio){
  ...
  if(estagio === 'recuperacao') return false; // recuperação é sempre manual, nunca calculado sozinho — evita confundir com o estágio "Fim"
...
function personalizarMensagemFunil(nomeCompleto, texto){
  const primeiroNome = nomeCompleto.split(' ')[0];
  if(texto.indexOf('Oi!') === 0) return texto.replace('Oi!', 'Oi, ' + primeiroNome + '!');
  return 'Oi, ' + primeiroNome + '! ' + texto;
}

async function enviarMensagemFunilParaAluna(nomeAluna, estagio, indice){
  const a = alunasPersonal.find(function(x){ return x.nome === nomeAluna; });
  const area = document.getElementById('resultado-funil-' + nomeAluna.replace(/[^a-zA-Z0-9]/g,''));
  if(!a) return;
  if(!a.telefone){ if(area) area.innerHTML = '<p class="txt" style="font-size:11px;color:#C9784A;">Essa aluna não tem telefone cadastrado.</p>'; return; }
  const template = mensagensExemploFunil[estagio][indice];
  if(!template) return;
  const mensagem = personalizarMensagemFunil(a.nome, template.texto);
  if(!confirm('Mandar essa mensagem pro WhatsApp de ' + a.nome.split(' ')[0] + '?\n\n"' + mensagem + '"')) return;
  if(area) area.innerHTML = '<p class="txt" style="font-size:11px;color:var(--text-faint);">Enviando...</p>';
  const saida = await enviarWhatsApp(a.telefone, mensagem);
  if(!area) return;
  area.innerHTML = saida.sucesso
    ? '<p class="txt" style="font-size:11px;color:var(--success);">✓ Enviada.</p>'
    : '<p class="txt" style="font-size:11px;color:#C9784A;">Não consegui enviar: ' + (saida.motivo || 'motivo não informado') + '</p>';
}
...
async function enviarMensagemFunilEmMassa(estagio, indice){
  ...
  if(alunasNoEstagio.length === 0){ alert('Nenhuma aluna com telefone cadastrado nesse estágio agora.'); return; }
  if(!confirm('Mandar "' + template.titulo + '" pro WhatsApp de ' + alunasNoEstagio.length + ' aluna(s), já com o nome de cada uma. Continuar?')) return;

  const area = document.getElementById('resultado-funil-massa-' + estagio + '-' + indice);
  let enviadas = 0, erros = [];
  for(let i = 0; i < alunasNoEstagio.length; i++){
    const a = alunasNoEstagio[i];
    if(area) area.innerHTML = '<p class="txt" style="font-size:11px;color:var(--text-faint);">Enviando ' + (i+1) + ' de ' + alunasNoEstagio.length + '... (de propósito, com pausa entre cada uma, pra proteger seu WhatsApp de bloqueio)</p>';
    const mensagem = personalizarMensagemFunil(a.nome, template.texto);
    const saida = await enviarWhatsApp(a.telefone, mensagem);
    if(saida.sucesso) enviadas++; else erros.push({ nome: a.nome, motivo: saida.motivo });
    await pausaEntreEnviosWhatsApp();
  }
  if(!area) return;
  const listaErros = erros.length ? '<ul style="margin:6px 0 0;padding-left:16px;font-size:10.5px;color:var(--text-faint);">' + erros.map(function(e){ return '<li>' + e.nome + ': ' + (e.motivo || 'motivo não informado') + '</li>'; }).join('') + '</ul>' : '';
  area.innerHTML = '<p class="txt" style="font-size:11px;color:' + (erros.length ? '#C9784A' : 'var(--success)') + ';">' + (erros.length ? '' : '✓ ') + enviadas + ' enviada(s)' + (erros.length ? ' · ' + erros.length + ' com erro' : '') + '</p>' + listaErros;
}

let mostrarListaAlunasFunil = false;

function alternarListaAlunasFunil(){
  mostrarListaAlunasFunil = !mostrarListaAlunasFunil;
  alternarEstagioFunil(estagioFunilAtivo);
}

function alternarEstagioFunil(estagio){
  estagioFunilAtivo = estagio;
  ['inicio','meio','fim','recuperacao'].forEach(function(e){
  ...
  const comTelefone = alunasNoEstagio.filter(function(a){ return a.telefone; }).length;
  const lista = document.getElementById('funil-lista-alunas');

  const rotulos = { inicio: 'Início', meio: 'Meio', fim: 'Fim', recuperacao: 'Remarketing/Recuperação' };
  let html = '<div class="info-box" style="text-align:center;margin-bottom:12px;">' +
    '<p style="font-size:28px;font-weight:700;margin:0;color:var(--gold-soft);">' + alunasNoEstagio.length + '</p>' +
    '<p class="txt" style="font-size:12px;color:var(--text-faint);margin:2px 0 0;">aluna(s) no estágio ' + rotulos[estagio] + (comTelefone < alunasNoEstagio.length ? ' · ' + (alunasNoEstagio.length - comTelefone) + ' sem telefone' : '') + '</p>' +
  '</div>';

  html += mensagensExemploFunil[estagio].map(function(m, idx){
    const idResultado = 'resultado-funil-massa-' + estagio + '-' + idx;
    return '<div class="info-box" style="margin-bottom:10px;">' +
      '<p class="lbl">' + m.titulo + '</p>' +
      '<p class="txt" style="margin-bottom:10px;">' + m.texto + '</p>' +
      '<button class="btn-gold" style="background:#25D366;color:#fff;border:none;" onclick="enviarMensagemFunilEmMassa(\'' + estagio + '\',' + idx + ')"><i class="ti ti-brand-whatsapp" style="margin-right:6px;"></i>Enviar pra todas (' + comTelefone + ')</button>' +
      '<div id="' + idResultado + '" style="margin-top:6px;"></div>' +
    '</div>';
  }).join('');

  html += '<p style="font-size:12px;color:var(--gold-soft);cursor:pointer;margin-top:14px;" onclick="alternarListaAlunasFunil()">' + (mostrarListaAlunasFunil ? 'Esconder lista de alunas' : 'Ver lista de alunas (' + alunasNoEstagio.length + ')') + '</p>';

  if(mostrarListaAlunasFunil){
    html += alunasNoEstagio.length === 0
      ...
      : alunasNoEstagio.map(function(a){
          ...
          return '<div class="aluna-card" style="flex-direction:column;align-items:stretch;gap:6px;">' +
            '<div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;" onclick="openAlunaDetail(' + i + ')">' +
              '<div><p class="aluna-name">' + a.nome + '</p><p class="aluna-meta">' + a.nivel + (a.estagioFunilManual ? ' · movida manualmente' : '') + (!a.telefone ? ' · sem telefone' : '') + '</p></div>' +
              '<i class="ti ti-chevron-right" style="color:var(--text-faint);"></i>' +
            '</div>' +
            '<select class="form-select" style="font-size:11px;padding:4px 6px;" onchange="moverAlunaNoFunil(\'' + a.nome.replace(/'/g,"\\'") + '\',this.value)">' +
              ...
              '<option value="recuperacao"' + (estagio==='recuperacao'?' disabled':'') + '>Remarketing/Recuperação</option>' +
  ...
  filtradas.slice(0, window.alunasQtdVisivel).forEach(function(a){
    ...
    el.innerHTML =
      '<div style="display:flex;justify-content:space-between;align-items:center;cursor:pointer;gap:8px;" data-abrir="1">' +
        ...
        '<div style="flex:1;"><p class="aluna-name">' + a.nome + (a.recemChegadaDaAnamnese ? ' <span class="tag" style="background:var(--success-soft);color:var(--success);">nova</span>' : '') + '</p><p class="aluna-meta">' + a.nivel + ' · ' + a.freq + (a.statusPlanoManual ? ' · movida manualmente' : '') + (a.valorPlano != null ? ' · R$ ' + a.valorPlano.toFixed(2).replace('.', ',') : '') + '</p></div>' +
...
function getDataEmEdicaoCheckIn(){
  return dataEmEdicaoCheckIn || getDataHojeISO();
}

function getCheckInHoje(nome){
  const prog = getProgressoAluna(nome);
  if(!prog.checkinsEmocionais) prog.checkinsEmocionais = {};
  const data = getDataEmEdicaoCheckIn();
  if(!prog.checkinsEmocionais[data]) prog.checkinsEmocionais[data] = { data: data };
  return prog.checkinsEmocionais[data];
}
...
function getHabitosHojeAluna(nome){
  ...
  const data = getDataEmEdicaoCheckIn();
  if(!prog.habitosDoDia[data]) prog.habitosDoDia[data] = {};
  return prog.habitosDoDia[data];
}
...
// É um registro totalmente separado, guardado na própria data de ontem.
function abrirPreenchimentoDeOntem(nome){
  dataEmEdicaoCheckIn = getDataOntemISO();
  renderCheckInEHabitos(nome);
}

function voltarParaPreenchimentoDeHoje(nome){
  dataEmEdicaoCheckIn = null;
  renderCheckInEHabitos(nome);
}

// Só aqui que salva de verdade no banco — os cliques nos campos acima só mexem na memória,
// pra dar tempo dela ajustar tudo antes de confirmar.
function salvarProgressoDoDiaAgora(nome){
  salvarProgressoNoSupabase(nome);
  const el = document.getElementById('confirmacao-salvar-progresso-dia');
  if(el){
...
function renderCheckInEHabitos(nome){
  ...
  function botaoEscala(campo, valorAtual){
    return [1,2,3,4,5].map(function(n){
      return '<span class="chip" style="width:38px;height:38px;display:flex;align-items:center;justify-content:center;padding:0;' + (valorAtual === n ? 'background:var(--gold-soft);color:#1A1409;border-color:var(--gold-soft);' : '') + '" onclick="salvarCampoCheckInEmocional(\'' + nome.replace(/'/g,"\\'") + '\',\'' + campo + '\',' + n + ')">' + n + '</span>';
    }).join('');
  }

  const editandoOntem = !!dataEmEdicaoCheckIn;
  const dataFormatadaEmEdicao = new Date(getDataEmEdicaoCheckIn() + 'T00:00:00').toLocaleDateString('pt-BR', { day:'2-digit', month:'2-digit' });

  let html = '<p class="section-label" style="margin-top:0;margin-bottom:10px;">' + (editandoOntem ? 'Preenchendo o dia de ontem (' + dataFormatadaEmEdicao + ')' : 'Meu progresso hoje') + '</p>' +
    (editandoOntem
      ? '<span class="chip" style="cursor:pointer;margin-bottom:16px;display:inline-block;" onclick="voltarParaPreenchimentoDeHoje(\'' + nome.replace(/'/g,"\\'") + '\')"><i class="ti ti-arrow-back" style="font-size:11px;vertical-align:-1px;margin-right:4px;"></i>Voltar pra hoje</span>'
      : '<span class="chip" style="cursor:pointer;margin-bottom:16px;display:inline-block;" onclick="abrirPreenchimentoDeOntem(\'' + nome.replace(/'/g,"\\'") + '\')"><i class="ti ti-calendar-time" style="font-size:11px;vertical-align:-1px;margin-right:4px;"></i>Esqueci de preencher ontem</span>') +
    '<div class="info-box" style="margin-bottom:14px;">' +
      '<p class="txt" style="font-size:12px;margin-bottom:8px;text-align:center;">Humor</p><div style="display:flex;justify-content:center;gap:10px;margin-bottom:16px;">' + botaoEscala('humor', checkin.humor) + '</div>' +
      '<p class="txt" style="font-size:12px;margin-bottom:8px;text-align:center;">Ansiedade</p><div style="display:flex;justify-content:center;gap:10px;margin-bottom:16px;">' + botaoEscala('ansiedade', checkin.ansiedade) + '</div>' +
      '<p class="txt" style="font-size:12px;margin-bottom:8px;text-align:center;">Disposição</p><div style="display:flex;justify-content:center;gap:10px;">' + botaoEscala('disposicao', checkin.disposicao) + '</div>' +
    '</div>' +
    '<div class="info-box" style="margin-bottom:14px;">' +
  ...
  html += '<button class="btn-gold" style="margin-top:20px;" onclick="salvarProgressoDoDiaAgora(\'' + nome.replace(/'/g,"\\'") + '\')">' + (editandoOntem ? 'Salvar informações de ontem' : 'Salvar informações de hoje') + '</button>' +
    '<p id="confirmacao-salvar-progresso-dia" style="text-align:center;font-size:12px;margin-top:6px;height:16px;"></p>';
...
// ===== ENVIO EM MASSA UNIFICADO — mesmo painel de opções, usado tanto pra WhatsApp quanto pra Aplicativo,
// deixando bem claro em qual canal a mensagem sai (a cor e o texto do botão já entregam isso) =====
function mostrarPainelEnvio(canal){
  const area = document.getElementById('painel-envio-' + canal);
  if(!area) return;
  if(area.innerHTML){ area.innerHTML = ''; return; } // clica de novo no botão de cima = fecha
  ...
  let html = '<div class="info-box">' +
    '<div class="form-group"><label class="form-label">Pra quem?</label>' +
      '<select class="form-select" id="publico-envio-' + canal + '" onchange="alternarSelecaoEspecificaEnvio(\'' + canal + '\')">' +
        ...
        '<option value="risco">Risco de abandono</option>' +
        '<option value="especificas">Selecionar alunas específicas</option>' +
      '</select></div>' +
    '<div id="lista-especificas-envio-' + canal + '" style="display:none;">' +
      '<input class="form-input" id="busca-envio-' + canal + '" placeholder="Buscar aluna por nome..." oninput="filtrarListaEnvio(\'' + canal + '\')" style="margin-bottom:8px;">' +
      '<div class="row" style="gap:8px;margin-bottom:8px;"><span class="chip" style="cursor:pointer;" onclick="marcarTodasEnvio(\'' + canal + '\',true)">Marcar todas</span><span class="chip" style="cursor:pointer;" onclick="marcarTodasEnvio(\'' + canal + '\',false)">Limpar seleção</span></div>' +
      '<div style="max-height:200px;overflow-y:auto;border:1px solid var(--border);border-radius:10px;padding:8px;margin-bottom:10px;">' +
        elegiveis.map(function(a){
          return '<label class="linha-envio-' + canal + '" data-nome-busca="' + a.nome.toUpperCase().replace(/"/g,'&quot;') + '" style="display:flex;align-items:center;gap:8px;padding:6px 4px;font-size:13px;cursor:pointer;">' +
            '<input type="checkbox" class="checkbox-envio-' + canal + '" value="' + a.nome.replace(/"/g,'&quot;') + '"> ' + a.nome +
          '</label>';
        }).join('') +
      '</div>' +
    '</div>' +
    '<div class="form-group"><label class="form-label">Mensagem</label><textarea class="form-input" id="texto-envio-' + canal + '" rows="4" placeholder="Escreve aqui...' + (canal === 'whatsapp' ? ' pode usar {nome} que substitui pelo primeiro nome de cada aluna' : '') + '"></textarea></div>' +
    '<button class="btn-gold" style="' + (canal === 'whatsapp' ? 'background:#25D366;color:#fff;border:none;' : '') + '" onclick="confirmarEnvioMassa(\'' + canal + '\')">' + (canal === 'whatsapp' ? 'Enviar pelo WhatsApp' : 'Enviar pelo Aplicativo') + '</button>' +
    '<div id="resultado-envio-' + canal + '" style="margin-top:10px;"></div>' +
  '</div>';
  area.innerHTML = html;
}

function alternarSelecaoEspecificaEnvio(canal){
  const modo = document.getElementById('publico-envio-' + canal).value;
  const areaLista = document.getElementById('lista-especificas-envio-' + canal);
  if(areaLista) areaLista.style.display = modo === 'especificas' ? 'block' : 'none';
}

function filtrarListaEnvio(canal){
  const termo = (document.getElementById('busca-envio-' + canal).value || '').toUpperCase().trim();
  document.querySelectorAll('.linha-envio-' + canal).forEach(function(linha){
    linha.style.display = (!termo || linha.getAttribute('data-nome-busca').indexOf(termo) !== -1) ? 'flex' : 'none';
  });
}

function marcarTodasEnvio(canal, marcar){
  ...
  document.querySelectorAll('.linha-envio-' + canal).forEach(function(linha){
    ...
    const chk = linha.querySelector('.checkbox-envio-' + canal);
    if(chk) chk.checked = marcar;
  });
}

async function confirmarEnvioMassa(canal){
  await sincronizarListaAlunasDoSupabase(); // sempre busca a lista mais nova antes de decidir quem recebe, nunca manda com dado velho
  const publico = document.getElementById('publico-envio-' + canal).value;
  const texto = document.getElementById('texto-envio-' + canal).value.trim();
  if(!texto){ alert('Escreve a mensagem antes de enviar.'); return; }

  let destinatarias;
  if(publico === 'especificas'){
    const nomesEscolhidos = Array.prototype.slice.call(document.querySelectorAll('.checkbox-envio-' + canal + ':checked')).map(function(el){ return el.value; });
    if(nomesEscolhidos.length === 0){ alert('Marca pelo menos uma aluna antes de enviar.'); return; }
    destinatarias = alunasPersonal.filter(function(a){ return nomesEscolhidos.indexOf(a.nome) !== -1; });
  } else {
    destinatarias = alunasPersonal.filter(function(a){ return true; });
    if(publico === 'ativas') destinatarias = destinatarias.filter(function(a){ return statusDoPlano(a) === 'ativas'; });
    if(publico === 'porvencer') destinatarias = destinatarias.filter(function(a){ return statusDoPlano(a) === 'porvencer'; });
    if(publico === 'vencidas') destinatarias = destinatarias.filter(function(a){ return statusDoPlano(a) === 'vencidas'; });
    if(publico === 'risco') destinatarias = destinatarias.filter(function(a){ return detectarRiscoAbandonoPorConstancia(a.nome); });
  }

  const nomeCanal = canal === 'whatsapp' ? 'pelo WhatsApp' : 'pelo Aplicativo';
  if(!confirm('Vai mandar essa mensagem ' + nomeCanal + ' pra ' + destinatarias.length + ' aluna(s). Continuar?')) return;

  const areaResultado = document.getElementById('resultado-envio-' + canal);

  if(canal === 'whatsapp'){
    const comTelefone = destinatarias.filter(function(a){ return a.telefone; });
    let enviadas = 0, erros = [];
    for(let i = 0; i < comTelefone.length; i++){
      const a = comTelefone[i];
      areaResultado.innerHTML = '<p class="txt" style="color:var(--text-faint);">Enviando ' + (i+1) + ' de ' + comTelefone.length + '... (com pausa entre cada uma, pra proteger seu WhatsApp de bloqueio)</p>';
    ...
    areaResultado.innerHTML = '<div class="info-box" style="border-color:var(--success);"><p class="txt">✓ ' + enviadas + ' de ' + comTelefone.length + ' enviada(s) pelo WhatsApp' + (destinatarias.length > comTelefone.length ? ' · ' + (destinatarias.length - comTelefone.length) + ' sem telefone cadastrado' : '') + '</p>' +
      (erros.length > 0 ? '<p class="txt" style="font-size:11px;color:var(--text-faint);margin-top:6px;">Falharam: ' + erros.map(function(e){ return e.nome; }).join(', ') + '</p>' : '') +
    '</div>';
  } else {
    let enviados = 0, recusaram = 0;
    destinatarias.forEach(function(a){
      if(enviarAvisoInApp(a.nome, texto, 'massa')) enviados++; else recusaram++;
    });
    areaResultado.innerHTML = '<div class="info-box" style="border-color:var(--success);"><p class="txt">✓ ' + enviados + ' recebida(s) no aplicativo' + (recusaram > 0 ? ' · ' + recusaram + ' não recebem avisos (recusaram no primeiro acesso)' : '') + '</p></div>';
  }
  document.getElementById('texto-envio-' + canal).value = '';
}
...
async function confirmarAvisarTreinoAjustado(){
  ...
  for(let i = 0; i < nomesMarcados.length; i++){
    ...
    const a = alunasPersonal.find(function(x){ return x.nome === nome; });
    if(!a) continue;
    area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Enviando ' + (i+1) + ' de ' + nomesMarcados.length + '... (de propósito, com pausa entre cada uma, pra proteger seu WhatsApp de bloqueio)</p>';
...
// Conta só dias de TREINO de verdade dentro do que foi registrado na semana — um dia de descanso
// registrado (ela fez alguma atividade nele) não deve contar pra "quantos treinos ela já fez".
function contarDiasDeTreinoConcluidos(prog){
  const nomesDiasDeTreino = dias.filter(function(d){ return !d.descanso; }).map(function(d){ return d.n; });
  const registrados = prog.diasConcluidos[prog.semana] || [];
  return registrados.filter(function(nome){ return nomesDiasDeTreino.indexOf(nome) !== -1; }).length;
}

function checarConclusaoSemana(nomeAluna){
  const prog = getProgressoAluna(nomeAluna);
  const total = totalDiasDeTreino();
  const concluidos = contarDiasDeTreinoConcluidos(prog);
  if(concluidos >= total){
    prog.semana += 1;
...
// Regra fixa da metodologia: nunca dois exercícios de Afundo (nem variações dele) no mesmo treino.
// Essa é uma trava final, roda depois de tudo pronto, garantindo isso não importa por qual caminho
// um segundo Afundo tenha entrado no dia (reforço de tempo, bi-set, etc.)
// Nunca repetir o mesmo exercício duas vezes no mesmo treino (mesmo cruzando bi-sets)
function extrairTodosNomesDeLinha(linha){
  if(linha.indexOf('|||') !== -1){
    const partes = linha.split('|||');
    return [(partes[1]||'').split(' · ')[0], (partes[2]||'').split(' · ')[0]].filter(Boolean);
  }
  return [linha.split(' · ')[0]];
}

function removerExerciciosExatamenteRepetidosNoDia(dia){
  const nomesVistos = {};
  dia.ex = dia.ex.filter(function(linha){
    const nomes = extrairTodosNomesDeLinha(linha);
    const jaRepetiu = nomes.some(function(n){ return nomesVistos[n.toUpperCase().trim()]; });
    nomes.forEach(function(n){ nomesVistos[n.toUpperCase().trim()] = true; });
    return !jaRepetiu;
  });
}

// Detecta se um exercício é unilateral — agora usando o campo `unilateral` real, confirmado pelo
// Thiago exercício por exercício (383 revisados). A busca por palavra-chave fica só como rede de
// segurança, pra exercício novo que ainda não foi classificado manualmente.
function ehExercicioUnilateral(nome){
  ...
  if(exBanco && typeof exBanco.unilateral === 'boolean') return exBanco.unilateral;
  const upper = nome.toUpperCase();
  const palavrasChave = ['UNILATERAL', 'AFUNDO', 'BÚLGARO', 'BULGARO', 'STEP UP', 'PASSADA', '4 APOIOS', '3 APOIOS', 'COICE'];
  if(palavrasChave.some(function(p){ return upper.indexOf(p) !== -1; })) return true;
  if(/\bUNI\b/.test(upper)) return true;
  return false;
}
...
function removerVariacoesDuplicadasDeAfundo(dia){
  ...
  dia.ex = dia.ex.filter(function(linha){
    if(linha.toUpperCase().indexOf('AFUNDO') === -1) return true;
    if(jaTemAfundo) return false;
    jaTemAfundo = true;
    return true;
  });
}
...
// o exercício. Exercício sem classificação ainda (nivelMinimo não preenchido) fica liberado pra todo
// mundo, pra nunca travar a geração de treino de grupos que ainda não foram classificados.
function nivelPermiteExercicio(nivelAluna, e){
  if(!e.nivelMinimo) return true;
  const nivelAlunaNum = ORDEM_NIVEL_TREINO[nivelAluna] || 1;
  const nivelMinimoNum = ORDEM_NIVEL_TREINO[e.nivelMinimo] || 1;
  return nivelAlunaNum >= nivelMinimoNum;
}

function obterPoolRealDoGrupo(grupoInterno, ambiente, evitarPliometrico, nivelAluna){
  ...
  const baseAmbiente = exerciciosBanco.filter(function(e){
    ...
    if(!nivelPermiteExercicio(nivelAluna, e)) return false;
...
function selecionarExerciciosVariados(grupoInterno, nomeAluna, quantidade, indiceCiclo, padroesJaUsados, ambiente, evitarPliometrico, nivelAluna){
  const pool = obterPoolRealDoGrupo(grupoInterno, ambiente, evitarPliometrico, nivelAluna);
...
// ============================================================
// MOTOR DO DESAFIO "SECA E EMPINA" — completamente separado do motor adaptativo normal.
// Não usa histórico da aluna nem perfil dela pra decidir nada; só recebe metas de volume fixas
// por estrutura (1 a 10) e monta a semana pra bater essas metas, dentro das regras específicas
// desse desafio (Glúteo Médio restrito a 3 exercícios, alguns unilaterais banidos, etc.)
// ============================================================

const ESTRUTURAS_SECA_EMPINA = [
  { quadriceps: 18, posterior: 18, gluteoMedio: 9,  gluteoMaximo: 14 },
  { quadriceps: 22, posterior: 18, gluteoMedio: 9,  gluteoMaximo: 20 },
  { quadriceps: 26, posterior: 15, gluteoMedio: 19, gluteoMaximo: 27 },
  { quadriceps: 25, posterior: 22, gluteoMedio: 18, gluteoMaximo: 24 },
  { quadriceps: 25, posterior: 24, gluteoMedio: 20, gluteoMaximo: 26 },
  { quadriceps: 25, posterior: 26, gluteoMedio: 21, gluteoMaximo: 27 },
  { quadriceps: 24, posterior: 27, gluteoMedio: 23, gluteoMaximo: 28 },
  { quadriceps: 24, posterior: 28, gluteoMedio: 24, gluteoMaximo: 28 },
  { quadriceps: 24, posterior: 28, gluteoMedio: 24, gluteoMaximo: 29 },
  { quadriceps: 24, posterior: 28, gluteoMedio: 25, gluteoMaximo: 29 }
];

const FATOR_REDUCAO_3X_DESAFIO = 0.65; // pra quem faz o desafio em 3x, reduz o volume total (não dá pra manter o de 5x em menos sessões)

// Exercícios permitidos pro desafio, por grupo — listas específicas dessa metodologia, não o banco geral
const GLUTEO_MEDIO_DESAFIO = ['CADEIRA ABDUTORA', 'CADEIRA ABDUTORA-TRONCO A FRENTE', 'CADEIRA ABDUTORA EM PÉ 45º'];
const BANIDOS_DESAFIO = ['CADEIRA EXTENSORA UNI', 'CADEIRA FLEXORA UNILATERAL']; // nunca usar nesse desafio, mesmo que sirvam no motor normal

function obterMetaVolumeDesafio(numeroEstrutura, frequencia){
  const base = ESTRUTURAS_SECA_EMPINA[numeroEstrutura - 1];
  if(!base) return null;
  if(frequencia >= 5) return base;
  return {
    quadriceps: Math.round(base.quadriceps * FATOR_REDUCAO_3X_DESAFIO),
    posterior: Math.round(base.posterior * FATOR_REDUCAO_3X_DESAFIO),
    gluteoMedio: Math.round(base.gluteoMedio * FATOR_REDUCAO_3X_DESAFIO),
    gluteoMaximo: Math.round(base.gluteoMaximo * FATOR_REDUCAO_3X_DESAFIO)
  };
}
...
function poolDesafio(grupoBanco, restringirGluteoMedio){
  ...
  return exerciciosBanco.filter(function(e){
    if((e.ambiente || 'Academia') !== 'Academia') return false;
...
function gerarSemanaDesafio(numeroEstrutura, frequencia, seedVariacao){
  ...
  for(let d = numDias; d < 7; d++){
    dias.push({ n: nomesDias[d], foco: 'Descanso', ex: [], descanso: true });
  }
  return { numeroEstrutura: numeroEstrutura, frequencia: frequencia, dias: dias, metaVolume: meta };
}

// ===== PERSONAL: inscrever aluna, listar quem está ativa, avançar de estrutura =====
function mostrarFormularioInscricaoSecaEmpina(){
  const area = document.getElementById('form-inscricao-seca-empina');
  if(!area) return;
...
function renderListaSecaEmpina(){
  ...
  if(!area) return;
  const ativas = alunasPersonal.filter(function(a){ return a.desafioAtivo; });
  if(ativas.length === 0){
    area.innerHTML = '<div class="info-box"><p class="txt" style="color:var(--text-faint);">Nenhuma aluna no Seca e Empina agora.</p></div>';
    return;
  }
  area.innerHTML = ativas.map(function(a){
...
// Usa obterAlunaLogadaOuCriar() (não alunasPersonal.find direto) — sessão da aluna não tem a lista
// completa carregada, e essa é a forma segura já estabelecida no resto do app pra esse caso.
function renderBotaoDesafio(){
  const area = document.getElementById('botao-desafio-area');
  if(!area) return;
...
function gerarDiasInferiores(perfil, numDias, seriesTotais, diasCicloAnterior){
  ...
  // A garantia de volume mínimo (secundário/terciário vs primário) agora roda como verificação FINAL,
  // depois do corte por tempo — ver aplicarVolumeMinimoTerciario(), chamada no fim de gerarTreinoSemanal.
  // Rodar só aqui, antes do corte, permitia que o corte por tempo removesse exatamente esse exercício
  // de segurança sem ninguém perceber.

  return dias;
}

// Regra de volume mínimo GENERALIZADA (substitui as 2 regras específicas que existiam antes, que
// tinham um erro de digitação — comparava com 'Glúteos' no plural, mas o valor real é 'Glúteo' no
// singular, então aquele bloco nunca executava de verdade). Agora: qualquer que seja o grupo
// PRIMÁRIO (ênfase), os outros 2 grupos (secundário e terciário) precisam ter, cada um, pelo menos
// 50% do volume do primário — sempre, sem depender de qual combinação específica foi escolhida.
// Roda como verificação FINAL, depois de qualquer corte por tempo — se algo cortou o exercício de
// segurança antes, essa checagem final recoloca, garantindo que a regra realmente se sustente.
function aplicarVolumeMinimoTerciario(diasInferiores, perfil, reps){
  if(!diasInferiores || diasInferiores.length === 0) return;
  const GRUPOS_INFERIORES = { 'Glúteo': 'Glúteos', 'Quadríceps': 'Quadríceps', 'Posterior': 'Isquiotibiais' };
  const primario = perfil.enfase;
  if(!GRUPOS_INFERIORES[primario]) return; // ênfase não é um dos 3 grupos de inferiores, não se aplica

  const totaisSemana = {};
  diasInferiores.forEach(function(d){
    const totaisDia = calcularVolumePorGrupoNoDia(d.ex);
    Object.keys(totaisDia).forEach(function(grupo){
      totaisSemana[grupo] = (totaisSemana[grupo] || 0) + totaisDia[grupo];
    });
  });

  const seriesPrimario = totaisSemana[GRUPOS_INFERIORES[primario]] || 0;
  if(seriesPrimario === 0) return; // sem volume nenhum do primário ainda, não dá pra calcular 50% de nada

  const outrosGrupos = Object.keys(GRUPOS_INFERIORES).filter(function(g){ return g !== primario; }); // secundário + terciário
  outrosGrupos.forEach(function(grupoInterno){
    const nomeBanco = GRUPOS_INFERIORES[grupoInterno];
    const seriesAtual = totaisSemana[nomeBanco] || 0;
    ...
    const faltam = minimoNecessario - seriesAtual;
    const seriesDoExercicio = Math.min(5, Math.max(seriesMinimas, faltam)); // teto de 5 séries — nunca uma prescrição absurda tipo "15x15" só pra fechar volume de uma vez

    // Insere no PRIMEIRO dia da lista (não no último) — o corte por tempo remove sempre do fim pra
    // início, então começar pelo primeiro dia protege melhor esse exercício de ser cortado depois.
    const diaAlvo = diasInferiores[0];
    diaAlvo.ex.push(nomeBanco === 'Isquiotibiais' ? (opcoes[0] + ' · ' + seriesDoExercicio + 'x' + Math.min(15, reps + 2) + ' (volume mínimo de ' + grupoInterno.toLowerCase() + ': 50% do volume de ' + primario.toLowerCase() + ', garantido pela metodologia)')
      : (opcoes[0] + ' · ' + seriesDoExercicio + 'x' + reps + ' (volume mínimo de ' + grupoInterno.toLowerCase() + ': 50% do volume de ' + primario.toLowerCase() + ', garantido pela metodologia)'));
  });
}
...
function gerarTreinoSemanal(perfil){
    ...
    if(perfil.tempoDisponivel && perfil.bloco !== 'deload' && !perfil.volumeReduzidoPorFadiga){
      ...
      if(ehInferior || ehSuperior){
        ...
        while(calcularDuracaoDia(dia) < perfil.tempoDisponivel * 0.9 && tentativasAdicionar < 8 && dia.ex.length < tetoExerciciosDoDia){
          ...
          const candidatos = selecionarExerciciosVariados(grupoDoDia, (perfil.nomeAluna || '') + '_extra' + tentativasAdicionar, 8, perfil.indiceCiclo, null, perfil.ambienteTreino, perfil.evitarPliometrico, perfil.nivel);
          let escolhido = null;
          if(candidatos){
          ...
          if(!escolhido){
            ...
            const todoOBanco = exerciciosBanco.filter(function(e){
              ...
              if(!nivelPermiteExercicio(perfil.nivel, e)) return false;
  ...
  // Verificação FINAL de volume mínimo (secundário/terciário vs primário) — roda só agora, depois do
  // corte por tempo e da reinforço, garantindo que nada removido por esses passos tenha desfeito a
  // regra. diasInf contém as mesmas referências de objeto que estão dentro de semana, então mexer
  // aqui já reflete direto nela, sem precisar reconstruir nada.
  const repsParaVolumeMinimo = perfil.bloco === 'deload' ? 14 : (perfil.bloco === 'choque' ? 7 : (perfil.fase === 'Desempenho' ? 9 : 11));
  aplicarVolumeMinimoTerciario(diasInf, perfil, repsParaVolumeMinimo);

  // Renomeia sequencialmente: Treino A, Treino B, Treino C... e a descrição vem da análise real
  // de volume por grupo muscular prescrito nesse dia (motor de nomenclatura dinâmica), não do que
  // foi planejado inicialmente pela pirâmide.
  let letraTreino = 0;
  semana.forEach(function(dia){
    dia.foco = 'Treino ' + String.fromCharCode(65 + letraTreino) + ' · ' + gerarDescricaoTreinoPorVolume(dia.ex);
    letraTreino++;
  });
...
function renderProgressao(a){
  ...
  const concluidosSemana = contarDiasDeTreinoConcluidos(prog);
...
// ===== MOTOR DE NOMENCLATURA DINÂMICA (nome do treino calculado pelo volume real prescrito, não fixo) =====
// Sempre que o treino é gerado OU reajustado manualmente (adicionar/remover/trocar exercício,
// mudar séries), o nome é recalculado do zero a partir do que está de fato no dia, nunca fica preso
// ao que foi planejado inicialmente pela ênfase/secundário da pirâmide.
const GRUPOS_RELEVANTES_PARA_NOME_TREINO = ['Glúteos', 'Isquiotibiais', 'Quadríceps', 'Costas', 'Ombros', 'Peito', 'Bíceps', 'Tríceps'];
const LABEL_GRUPO_NOME_TREINO = { 'Glúteos': 'Glúteos', 'Isquiotibiais': 'Posteriores', 'Quadríceps': 'Quadríceps', 'Costas': 'Costas', 'Ombros': 'Ombro', 'Peito': 'Peito', 'Bíceps': 'Bíceps', 'Tríceps': 'Tríceps' };

function calcularVolumePorGrupoNoDia(diaEx){
  const totais = {};
  (diaEx || []).forEach(function(linhaOriginal){
    ...
    linhas.forEach(function(linha){
      ...
      const exBanco = buscarExercicioNoBanco(nomeEx);
      if(!exBanco) return;
      const grupo = exBanco.grupo || exBanco.categoria;
      // Panturrilha e Abdômen são sempre acessórios de baixo volume, nunca definem a identidade do dia
      if(GRUPOS_RELEVANTES_PARA_NOME_TREINO.indexOf(grupo) === -1) return;
...
function juntarNomesComE(lista){
  if(lista.length === 0) return '';
  if(lista.length === 1) return lista[0];
  return lista.slice(0, -1).join(', ') + ' e ' + lista[lista.length - 1];
}

function gerarDescricaoTreinoPorVolume(diaEx){
  const totais = calcularVolumePorGrupoNoDia(diaEx);
  const gruposOrdenados = Object.keys(totais).sort(function(g1, g2){ return totais[g2] - totais[g1]; });
  if(gruposOrdenados.length === 0) return 'Treino do dia'; // salvaguarda, não deveria acontecer com um dia de treino de verdade
  const enfaseLabel = LABEL_GRUPO_NOME_TREINO[gruposOrdenados[0]] || gruposOrdenados[0];
  const restantes = gruposOrdenados.slice(1).map(function(g){ return LABEL_GRUPO_NOME_TREINO[g] || g; });
  if(restantes.length === 0) return enfaseLabel;
  const palavraEstimulo = restantes.length === 1 ? 'estímulo' : 'estímulos';
  return enfaseLabel + ' com ' + palavraEstimulo + ' em ' + juntarNomesComE(restantes);
}

function extrairLetraDoTreino(foco){
  const m = (foco || '').match(/^Treino\s+(\S+)/);
  return m ? m[1] : null;
}

// Recalcula o nome de um dia específico a partir do treino que está de fato prescrito agora,
// preservando a letra do treino (Treino A, B, C...) já atribuída. Chame isso sempre que
// adicionar, remover, trocar exercício, ou mudar séries/reps de um dia já existente.
function recalcularNomeDoTreinoPorVolume(dia){
  if(dia.descanso || dia.tabata) return; // dia de descanso ou Tabata não segue essa lógica de nome
  const letra = extrairLetraDoTreino(dia.foco);
  const descricao = gerarDescricaoTreinoPorVolume(dia.ex);
  dia.foco = (letra ? 'Treino ' + letra + ' · ' : '') + descricao;
}
...
// Rótulo de fase simplificado pra mostrar pra aluna — nunca a razão técnica por trás (override de IMC,
// objetivo declarado, fadiga, etc.), só o nome do bloco que ela está treinando agora.
function rotuloFaseParaAluna(a){
  const infoBloco = calcularBlocoAtual(a);
  const rotulos = { base: 'Base', choque: 'Choque', deload: 'Regenerativo' };
  return rotulos[infoBloco.blocoTecnico] || 'Base';
}
...
async function auditarVolumePosteriores(){
  ...
  const semPosterior = [];
  const abaixoDoMinimo = []; // tem posterior, mas abaixo de 50% do volume semanal de quadríceps
  ...
  comTreino.forEach(function(a){
    ...
    const seriesQuadriceps = totaisSemana['Quadríceps'] || 0;
    const seriesPosterior = totaisSemana['Isquiotibiais'] || 0;

    if(seriesPosterior === 0){
      semPosterior.push(a.nome);
    } else if(seriesQuadriceps > 0 && seriesPosterior < seriesQuadriceps * 0.5){
      abaixoDoMinimo.push(a.nome + ' (posterior: ' + seriesPosterior + ' séries · quadríceps: ' + seriesQuadriceps + ' séries · mínimo esperado: ' + Math.ceil(seriesQuadriceps * 0.5) + ')');
    }
  });

  if(semPosterior.length === 0 && abaixoDoMinimo.length === 0){
    area.innerHTML = '<div class="info-box" style="border-color:var(--success);"><p class="lbl" style="color:var(--success);">✓ Nenhum problema encontrado</p><p class="txt">Revisei ' + comTreino.length + ' treino(s) — todas com pelo menos 50% do volume de quadríceps em posteriores.</p></div>';
    return;
  }

  let html = '';
  if(semPosterior.length > 0){
    html += '<div class="info-box" style="border-color:#E2A33D;margin-bottom:8px;"><p class="lbl" style="color:#E2A33D;">⚠ ' + semPosterior.length + ' aluna(s) sem NENHUM posterior no treino atual</p>' +
      '<p class="txt" style="font-size:11px;color:var(--text-faint);">' + semPosterior.join(', ') + '</p></div>';
  }
  if(abaixoDoMinimo.length > 0){
    html += '<div class="info-box" style="border-color:#E2A33D;"><p class="lbl" style="color:#E2A33D;">⚠ ' + abaixoDoMinimo.length + ' aluna(s) com posterior abaixo de 50% do volume de quadríceps</p>' +
      '<p class="txt" style="font-size:11px;color:var(--text-faint);">' + abaixoDoMinimo.join('; ') + '</p></div>';
  }
  html += '<p class="txt" style="font-size:11px;margin-top:8px;color:var(--text-faint);">Esses são treinos gerados antes dessa regra dos 50% entrar em vigor no motor. A auditoria já identifica certo — mas o motor de geração ainda precisa ser atualizado numa próxima etapa pra já nascer respeitando essa proporção (por enquanto ele só garante 1 exercício fixo, sem relação com o volume de quadríceps).</p>';
  area.innerHTML = html;
}
...
function mostrarOpcoesDeTrocaMesmoAssim(diaIndex, exIndex){
  ...
  // dentro de cada grupo (mesma família ou não), organiza em ordem alfabética
...
function responderSugestaoTabata(nomeAluna, quer){
  ...
  if(quer){
    setTimeout(function(){
      alert('Beleza! Vai em "Resumo completo" → "Transformar um dia em Tabata de casa", escolhe o dia e confirma.');
    }, 100);
  }
}
      ...
      html += '<div class="list-item exercicio-linha" data-dia="' + di + '" data-ex="' + ei + '" style="flex-direction:column;align-items:stretch;gap:4px;padding:8px 12px;">' +
        '<div style="display:flex;justify-content:space-between;align-items:center;">' +
          '<div style="display:flex;align-items:center;gap:8px;min-width:0;">' +
            '<span class="alca-arrastar" data-dia="' + di + '" data-ex="' + ei + '" onpointerdown="iniciarArrastarExercicio(event)" style="cursor:grab;color:var(--text-faint);touch-action:none;flex-shrink:0;font-size:16px;line-height:1;">⠿</span>' +
...
// Recalcula o nome do dia pela metodologia de volume real e já atualiza o título na tela,
// sem precisar recarregar a ficha inteira (que perderia o dia aberto e a rolagem da tela).
function recalcularEAtualizarNomeDoDia(di){
  if(!alunaAberta || !alunaAberta.treinoAtual || !alunaAberta.treinoAtual.dias[di]) return;
  const dia = alunaAberta.treinoAtual.dias[di];
  recalcularNomeDoTreinoPorVolume(dia);
  const tituloEl = document.getElementById('dia-titulo-' + di);
  if(tituloEl) tituloEl.textContent = dia.foco.indexOf(' · ') !== -1 ? dia.foco.split(' · ').slice(1).join(' · ') : dia.foco;
}
...
// Ferramenta movida pra cá (fora do dia-a-dia de edição de treino) porque era fácil clicar sem
// querer ali e perder um treino pronto. Agora exige escolher o dia numa lista e confirmar antes.
function renderFerramentaTabata(a){
  if(!a.treinoAtual || !a.treinoAtual.dias) return '<div class="info-box"><p class="txt" style="color:var(--text-faint);">Gere um treino primeiro pra poder transformar um dos dias em Tabata.</p></div>';
  const idSufixo = a.nome.replace(/[^a-zA-Z0-9]/g,'');
  const opcoesDias = a.treinoAtual.dias.map(function(d, di){
    return '<option value="' + di + '">' + d.n + ' — ' + d.foco + '</option>';
  }).join('');
  return '<div class="info-box" style="border-color:#C9784A;">' +
    '<p class="txt" style="color:#C9784A;font-weight:600;margin-bottom:8px;"><i class="ti ti-alert-triangle" style="vertical-align:-2px;margin-right:4px;"></i>Atenção: isso substitui TODOS os exercícios do dia escolhido por um treino Tabata de casa. Não dá pra desfazer.</p>' +
    '<select class="form-select" id="select-dia-tabata-' + idSufixo + '" style="margin-bottom:8px;">' + opcoesDias + '</select>' +
    '<button class="btn-gold" style="width:auto;padding:8px 16px;font-size:12px;background:#C9784A;color:#fff;border:none;" onclick="confirmarTransformarEmTabata(\'' + a.nome.replace(/'/g,"\\'") + '\')">Transformar dia selecionado em Tabata de casa</button>' +
  '</div>';
}

function confirmarTransformarEmTabata(nomeAluna){
  const idSufixo = nomeAluna.replace(/[^a-zA-Z0-9]/g,'');
  const seletor = document.getElementById('select-dia-tabata-' + idSufixo);
  if(!seletor) return;
  const di = parseInt(seletor.value, 10);
  const a = alunasPersonal.find(function(x){ return x.nome === nomeAluna; });
  if(!a || !a.treinoAtual || !a.treinoAtual.dias[di]) return;
  const nomeDia = a.treinoAtual.dias[di].n;
  if(!confirm('Tem certeza? Isso vai apagar todos os exercícios de "' + nomeDia + '" e substituir por um treino Tabata de casa. Não dá pra desfazer.')) return;
  regerarTabataParaAluna(nomeAluna, di);
  abrirResumoCompletoAluna(nomeAluna);
  mostrarConfirmacaoSalvamento(true, '"' + nomeDia + '" transformado em Tabata de casa.');
}
...
function marcarStatusControleCiclo(nomeAluna, novoStatus){
  const a = alunasPersonal.find(function(x){ return x.nome === nomeAluna; });
  if(!a) return;
  a.statusControleCiclo = novoStatus;
  if(novoStatus === 'verde'){
    a.dataFicouVerde = new Date().toISOString();
    const maiorOrdemAtual = alunasPersonal.reduce(function(max, x){ return Math.max(max, x.ordemConclusaoCicloAtual || 0); }, 0);
...
async function enviarCredenciaisPorWhatsApp(nomeAluna){
  ...
  if(saida.sucesso){
    ...
    marcarStatusControleCiclo(a.nome, 'verde');
  } else {
...
function renderSidebarPersonal(){
  ...
  // 3 grupos temáticos, pra não ficar uma lista enorme solta — cada um abre/fecha, e o grupo com a
  // tela ativa já vem aberto sozinho (ver atualizarSidebarAtiva)
  const grupos = [
    { nome: 'Atendimento & Suporte', views: ['dashboard-suporte','sinalizacoes','conversas','controle'] },
    { nome: 'Bancos de referência', views: ['exercicios','patologias','desvios','corrida'] },
...
function alternarGrupoSidebar(indice){
  const el = document.getElementById('sidebar-grupo-' + indice);
  const chevron = document.getElementById('chevron-sidebar-grupo-' + indice);
  if(!el) return;
...
function atualizarSidebarAtiva(which){
  ...
  // Sempre abre sozinho o grupo que contém a tela em que você está — assim nunca fica "escondida"
  // dentro de um grupo fechado sem você perceber onde foi parar
  const itemAtivo = sidebar.querySelector('.side-item.ativo[data-side-view="' + which + '"]');
  if(itemAtivo){
    const grupoPai = itemAtivo.closest('[id^="sidebar-grupo-"]');
    if(grupoPai){
      grupoPai.style.display = 'block';
      const indice = grupoPai.id.replace('sidebar-grupo-', '');
      const chevron = document.getElementById('chevron-sidebar-grupo-' + indice);
...
// A anamnese guarda a data em formato brasileiro (dd/mm/aaaa, às vezes com pontos, dois pontos, ou
// colada sem separador). O app precisa em formato aaaa-mm-dd. Se não conseguir reconhecer o formato
// com segurança, devolve null — prefere deixar vazio a guardar uma data errada.
function converterDataNascimentoParaISO(valor){
  ...
  if(comSeparador){
    let dia = comSeparador[1], mes = comSeparador[2], ano = comSeparador[3];
  ...
  if(colado){
    const dia = colado[1], mes = colado[2], ano = colado[3];
    if(parseInt(mes,10) < 1 || parseInt(mes,10) > 12) return null;
    return ano + '-' + mes + '-' + dia;
  }
  return null;
...
function showPersonalView(which){
  ...
  if(which === 'conversas'){ renderListaConversas(); }
  if(which === 'controle'){
    renderControleTreinos();
    // Mesma lógica do Dashboard: sincroniza com o que há de mais recente (inclusive quem acabou de
    // virar Ativa) antes de mostrar a lista, pra nunca ficar com dado desatualizado nessa tela.
  ...
  if(which === 'desafios'){ renderDesafios(); populateTreinoSelect(); renderListaSecaEmpina(); }
...
const exerciciosBanco = [
  {nome:'REMADA UNILATERAL COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=kS0KNWUgU7I', unilateral:true},
  {nome:'REMADA ABERTA COM HALTERES SUP', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=giQfgYKYrGc', unilateral:false},
  {nome:'STIFF UNILATERAL COM HALTERES', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=GyDrpYEPWQw', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'STIFF UNILATERAL SEM APOIO', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=E6QF1VWS1qY', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'DESENVOLVIMENTO FRONTAL COM MOCHILA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=RcHEOOACjHk', unilateral:false},
  {nome:'FLEXÃO DE COTOVELO MOCHILA', grupo:'Peito', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=X42BMTwuPMA', unilateral:false},
  {nome:'CADEIRA FLEXORA YES', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=ocV8ovygjkM', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CADEIRA ABDUTORA INCLINADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=0ICPVbRiqNM', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AGACHAMENTO JACK YES', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=jGooGCaCDI8', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'TRICEPS COM BARRA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=CUCYMqymsZ4', unilateral:false},
  {nome:'TRICEPS POLIA ALTA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=jHd_qtTZINc', unilateral:false},
  {nome:'TRÍCEPS COICE POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=2juXdMcTg28', unilateral:true},
  {nome:'CRUCIFIXO INVERSO COM APOIO', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=5WCO_sbLEAk', unilateral:false},
  {nome:'CRUCIFIXO INVERSO UNILATERAL NA POLIA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=m85a-zp454s', unilateral:true},
  {nome:'REMADA ABERTA UNILATERAL NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=M-sat6gp-HI', unilateral:true},
  {nome:'PULLDOWN COM BARRA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=opdOQXj2Qe8', unilateral:false},
  {nome:'TRICEPS COM BARRA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=Byff4QmSEc8', unilateral:false},
  {nome:'TRICEPS CORDA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=wOS8UdARevY', unilateral:false},
  {nome:'PULLDOWN COM CORDA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=GMonbLwBHqQ', unilateral:false},
  {nome:'FACEPULL COM', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=j_gbLC6eHXQ', unilateral:false},
  {nome:'CADEIRA ABDUTORA INCLINADO - YOUTUBE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=NQRcrs2NJ5E', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CADEIRA FLEXORA YES - YOUTUBE', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=HxgLLgslej4', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CRUCIFIXO INVERSO COM APOIO - YOUTUBE', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=IbhLDWWnGxc', unilateral:false},
  {nome:'CRUCIFIXO COM HALTERES - YOUTUBE', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=eHUtTovc3Zg', unilateral:false},
  {nome:'DOUBLE BICEPS - YOUTUBE', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=Sz_DKHXqhBA', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL POLIA - YOUTUBE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=uZrno5HI6Cc', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL POLIA - YOUTUBE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=uZrno5HI6Cc', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'MANGUITO DEITADO', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=nDUGYVKYGgI&sttick=0', unilateral:false},
  {nome:'PULLDOWN COM BARRA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=OaDynv2jISg&sttick=0', unilateral:false},
  {nome:'REMADA ABERTA COM BARRA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=u51yEma4v3M', unilateral:false},
  {nome:'REMADA ABERTA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=-AhBdO6ngdE&sttick=0', unilateral:false},
  {nome:'REMADA FECHADA UNILATERAL NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=6_sUwRdMZNE&sttick=0', unilateral:true},
  {nome:'PUXADA ABERTA FRONTAL - YOUTUBE', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=QV4WboLAVj0', unilateral:false},
  {nome:'PUSH UP PLUS - VARIAÇÃO - YOUTUBE', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=pdVvEmF97Rw', unilateral:false},
  {nome:'PUSH UP COM BARRA - YOUTUBE', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=wlxPLG--5oo', unilateral:false},
  {nome:'PUSH UP PLUS EM 4 APOIOS', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=87IHSrAHdq8', unilateral:true},
  {nome:'PUSH UP COM BARRA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=wlxPLG--5oo', unilateral:false},
  {nome:'CRUCIFIXO NA MÁQUINA -FLY', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=L4YtdisHldk', unilateral:false},
  {nome:'ROSCA SCOTT NA MÁQUINA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=cgciNALjjT4&sttick=0', unilateral:false},
  {nome:'CADEIRA ABDUTORA EM PÉ 45º', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=1Kye0sDSAw8', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL NA POLIA - TRONCO INCLINADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=dvsgo4Gj-hA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL NA POLIA - TRONCO INCLINADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=dvsgo4Gj-hA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL COM CANELEIRA NO BANCO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=5BuJFrdn_AY', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'PULLOVER COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=LvrEvExCzV8', unilateral:false},
  {nome:'REMADA ABERTA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=-AhBdO6ngdE', unilateral:false},
  {nome:'ROSCA CONCENTRADA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=ROGbtTlxs0Q', unilateral:true},
  {nome:'ROSCA DIRETA 21', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=l-MktIlWRZg', unilateral:false},
  {nome:'STIFF NA POLIA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=lBlWY-illWg', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'TRÍCEPS CORDA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=--sJN_gWRUc', unilateral:false},
  {nome:'TRICEPS POLIA ALTA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=abdcKPAuyxM', unilateral:false},
  {nome:'CADEIRA ABDUTORA - VARIAÇÃO TRONCO INCLINADO A FRENTE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=aKzeShloHJs', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL COM CANELEIRA NO BANCO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=5BuJFrdn_AY', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL NA MÁQUINA - VARIAÇÃO PARA MAIOR AMPLITUDE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=IWpwUAgwnpI', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'ABDUÇÃO DE QUADRIL COM ROTAÇÃO EXTERNA NA POLIA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=lhw9UXBJ35Q', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'ABDUÇÃO DE QUADRIL COM CANELEIRA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=fE5qN5-6VUI', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'4 APOIOS COM CANELEIRA - ALTO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=aOZ5aeoTyX0', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL CURTINHAS NO ALTO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=QXrJE4FF-TM', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'PUXADA ABERTA ARTICULADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=YCK83Z7iQX8', unilateral:false},
  {nome:'SUPINO INCLINADO COM BARRA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=Py7wmd4Mw1Y', unilateral:false},
  {nome:'LEG PRESS 45º', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=qx2g8W6qoaQ', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'LEG PRESS 45º', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=qx2g8W6qoaQ', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'TRÍCEPS TESTA COM BARRA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=gANUNaWe8WE', unilateral:false},
  {nome:'REMADA FECHADA ARTICULAR - YOUTUBE', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=q-jGW3oHTJg', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS BILATERAL - YOUTUBE', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=zogtbslROLk', unilateral:false},
  {nome:'STEP UP', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=NOvtICHyccY', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'CADEIRA EXTENSORA - TRONCO INCLINADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=CpQevcceBhg', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'POLICHINELO', grupo:'Aeróbico', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=rJbrYLTNymc', unilateral:false},
  {nome:'AFUNDO COM HALTERES', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=eyhLRHuEjYA', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'TERRA SUMÔ COM BARRA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=xX3YS6MpyTQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'LEVANTAMENTO TERRA - YOUTUBE', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=w2W-WtAQj2I', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO SUMÔ COM HALTERES - YOUTUBE', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=6zoYEflTEYM', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'RETRAÇÃO DE ESCÁPULAS - YOUTUBE', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=T1MvJi_mCAg', unilateral:false},
  {nome:'VAI E VEM DE FRENTE E COSTAS', grupo:'Aeróbico', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/eQzb92p_lZI', unilateral:false},
  {nome:'PULDOWN COM BARRA NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/shlGmlzAUGE', unilateral:false},
  {nome:'RETRAÇÃO DE ESCÁPULAS', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/d15LLZ_Uxvo', unilateral:false},
  {nome:'ROSCA DIRETA ALTERNADA COM ISOMETRIA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/XYOtweuFkzA', unilateral:false},
  {nome:'ROSCA DIRETA ALTERNADA COM HALTERES', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/Tzz8CUv-LNk', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL UNILATERAL NA POLIA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/qCQOT8Py038', unilateral:true},
  {nome:'TRÍCEPS COICE NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/uFaZr3cSM7A', unilateral:true},
  {nome:'TRÍCEPS UNILATERAL NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/udH-RZ2Wb2Y', unilateral:true},
  {nome:'TRÍCEPS COICE UNILATERAL NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/cQ7TeC_rdlg', unilateral:true},
  {nome:'MANGUITO ROTAÇÃO EXTERNA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/MizDY8p0gDY', unilateral:false},
  {nome:'MANGUITO ROTAÇÃO EXTERNA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/MizDY8p0gDY', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL COM BARRA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/n0sRwlEbFRY', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL COM BARRA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/n0sRwlEbFRY', unilateral:false},
  {nome:'ELEVAÇÃO LATERAL INCLINADA NA ROLDANA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/IVqSmYZ2awM', unilateral:false},
  {nome:'LEG HORIZONTAL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/0KLrU6JXOUE', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'PANTURRILHA SENTADO NA MÁQUINA', grupo:'Panturrilha', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/t8vU7HK_wcU', unilateral:false},
  {nome:'REMADA FECHADA COM APOIO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/6lAxaZwpfDg', unilateral:false},
  {nome:'REMADA ABERTA COM APOIO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/lmr6pVTTJT0', unilateral:false},
  {nome:'AFUNDO NO SQUAT', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/4NuNYnr1dbs', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO FRONTAL NO SQUAT ( ESTÍMULO GLÚTEOS )', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/-WxH7ND9NEg', unilateral:false},
  {nome:'AGACHAMENTO FRONTAL NO SQUAT ( ESTÍMULO GLÚTEOS )', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/-WxH7ND9NEg', unilateral:false},
  {nome:'STIFF NA MÁQUINA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/iHJxAoJYGBE', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AGACHAMENTO SUMÔ NA MÁQUINA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/vMJLDXo8MDw', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'TERRA SUMÔ NA MÁQUINA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/nq14XPTvfHs', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'SUPINO RETO NA MÁQUINA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/ILPmqVVNDfs', unilateral:false},
  {nome:'SUPINO INCLINADO NA MÁQUINA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/po7Hmy5Lths', unilateral:false},
  {nome:'SUPINO DECLINADO NA MÁQUINA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/ULZctWY2bas', unilateral:false},
  {nome:'REMADA FECHADA PRONADA NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/aYzUsR5mCY0', unilateral:false},
  {nome:'CADEIRA FLEXORA UNILATERAL', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/2YEghbjQQwU', camada:'base', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'AGACHAMENTO PENDULAR', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/zxhM2HGb4tQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'REMADA CURVADA ABERTA NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/SdGuFWsi4Wg', unilateral:false},
  {nome:'REMADA CURVADA FECHADA NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/CyFwxuxdD1Y', unilateral:false},
  {nome:'REMADA ABERTA COM APOIO NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/xxhfjVcemrk', unilateral:false},
  {nome:'REMADA FECHADA COM APOIO NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/Krg5OX5fTXA', unilateral:false},
  {nome:'REMADA ABERTA PRONADA NA MÁQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/2ZLbfQoVBas', unilateral:false},
  {nome:'PUXADA FRONTAL ABERTA ARTICULADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/jnSfxuvJ6Lg', unilateral:false},
  {nome:'ABDUÇÃO DE QUADRIL NA POLIA COM ROTAÇÃO - YOUTUBE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/VrSHOnWrlYQ', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL COM APOIO NO BANCO - YOUTUBE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/HmcyJwfM92w', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL NA POLIA COM TRONCO INCLINADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/_Z19zMD6ams', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'REMADA FECHADA NA MAQUINA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/4DyI2sMROzQ', unilateral:false},
  {nome:'POLICHINELO (variação)', grupo:'Aeróbico', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/RraVgclk9UM', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS UNILATERAL', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/Nolk5Rqh5SI', unilateral:true},
  {nome:'BOM DIA EM CASA', grupo:'Isquiotibiais', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/ZG44uKbHK4Q', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'STIFF UNILATERAL EM CASA', grupo:'Isquiotibiais', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/6oPT1WncH8U', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'GLÚTEO 4 APOIOS JOELHO FLETIDO EM CASA', grupo:'Glúteos', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/4M4f8ZBLFkA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'GLÚTEO 4 APOIOS JOELHO ESTENDIDO EM CASA', grupo:'Glúteos', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/xb_lnHwwZE4', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'CRUCIFIXO DEITADO EM CASA', grupo:'Peito', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/Zu_EiENKB-s', unilateral:false},
  {nome:'SUPINO RETO COM BARRA EM CASA', grupo:'Peito', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/4HRd2iAIZgQ', unilateral:false},
  {nome:'TRÍCEPS SUPINADO EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/UNBGmEOLDtA', unilateral:false},
  {nome:'TRÍCEPS SUPINADO EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/UNBGmEOLDtA', unilateral:false},
  {nome:'TRÍCEPS TESTA EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/bpp_l5Gdv7c', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS BILATERAL EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/rT5dEQzyZy8', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS UNILATERAL EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/ZgC9-xF_5Zw', unilateral:true},
  {nome:'FLEXÃO NO SOLO EM CASA', grupo:'Peito', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/PD8IJVrCIF0', unilateral:false},
  {nome:'REMADA ABERTA CURVADA COM BARRA EM CASA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/4rIMCTLueQA', unilateral:false},
  {nome:'ABDUÇÃO DE QUADRIL CONTRA O SOLO EM CASA', grupo:'Glúteos', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/tTwLVwhe8Bg', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'BURP ADAPTADO!! EM CASA', grupo:'Abdômen', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/CtnnoxhzD7U', unilateral:false},
  {nome:'EXTENSÃO DE OMBROS EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/wJCjio_nhjk', unilateral:false},
  {nome:'AGACHAMENTO FRONTAL EM CASA', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/oMoGcwdus3w', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CADEIRINHA EM CASA', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/BOHl-YMWFnk', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXERCÍCIO PASSADA EM CASA', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/XpUCJjVJloI', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'ELEVAÇÃO FRONTAL PEGADA NEUTRA EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/2KOu-qMUffQ', unilateral:false},
  {nome:'CRUCIFIXO INVERSO COM PESO EM CASA', grupo:'Peito', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/mtwV9vh1Yrc', unilateral:false},
  {nome:'REMADA ABERTA COM PESO EM CASA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/HOD-Um14gFg', unilateral:false},
  {nome:'REMADA NEUTRA CURVADA EM CASA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/mdtC-9_avxY', unilateral:false},
  {nome:'REMADA NEUTRA CURVADA EM CASA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/mdtC-9_avxY', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL COM BARRA EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/dVpeHSXqbis', unilateral:false},
  {nome:'ELEVAÇÃO LATERAL EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/7CpesOm6Z5U', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/Giue8EVBUdw', unilateral:false},
  {nome:'SKIPPING EM CASA', grupo:'Aeróbico', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/6F0ModFWzIg', unilateral:false},
  {nome:'POLICHINELO EM CASA', grupo:'Aeróbico', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/6AIlJkFNrKA', unilateral:false},
  {nome:'ROSCA BÍCEPS COM BARRA EM CASA', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/NpnW6BzuNkE', unilateral:false},
  {nome:'ROSCA BÍCEPS COM BARRA EM CASA', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/NpnW6BzuNkE', unilateral:false},
  {nome:'ROSCA BÍCEPS ALTERNADA EM CASA', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/VbneTGKw5pE', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL ALTERNADA EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/kGQP3CIow4s', unilateral:false},
  {nome:'ELEVAÇÃO DIAGONAL EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/7_9oQNFkqU0', unilateral:false},
  {nome:'ELEVAÇÃO DIAGONAL EM CASA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://www.youtube.com/shorts/7_9oQNFkqU0', unilateral:false},
  {nome:'FLEXÃO NÓRDICA REVERSA', grupo:'Quadríceps', categoria:'Quadríceps', ambiente:'Academia', nivel:'Avançado', metodo:'Nenhum', video:'', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEXÃO NO SOLO ADAPTADA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/uY3Y9eF79iw', unilateral:false},
  {nome:'CRUCIFIXO INVERSO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/mtwV9vh1Yrc', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL ALTERNADA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/kGQP3CIow4s', unilateral:false},
  {nome:'ROSCA MARTELO COM CORDA NA POLIA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/KMpBtSwNRcw', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS BIL', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/rT5dEQzyZy8', unilateral:false},
  {nome:'PRANCHA VENTRAL DINÂMICA OMBRO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/jSqggWVyTVA', unilateral:false},
  {nome:'FLEXÃO ADAPTADA NO SOFÁ', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/DcOgOamrNVQ', unilateral:false},
  {nome:'CRUCIFIXO INVERSO C PESO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/mtwV9vh1Yrc', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Giue8EVBUdw', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS APOIADO', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1XNQYUWWr4w', unilateral:false},
  {nome:'PRANCHA DINÂMICA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/SgSRuUNSaSg', unilateral:false},
  {nome:'SUPINO RETO COM BARRA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/kwBj4YYcHN0', unilateral:false},
  {nome:'ROSCA DIRETA COM BARRA W', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/DP_r0Y3jaDQ', unilateral:false},
  {nome:'TRÍCEPS COM CORDA NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/5Pi0xpkSJ7I', unilateral:false},
  {nome:'ABS BICICLETA ALT', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ZowzFb-NlVc', unilateral:false},
  {nome:'FLEXÃO DIAGONAL NA POLIA', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/_g-x6FKny3U', unilateral:false},
  {nome:'CRUCIFIXO INVERSO COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Qgi26Oy7Izk', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL GUIADA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/n0sRwlEbFRY', unilateral:false},
  {nome:'ROSCA DIRETA COM HALTERES', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/8UyF5zKDyiw', unilateral:false},
  {nome:'TRÍCEPS FRA UNIL', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ZgC9-xF_5Zw', unilateral:true},
  {nome:'FLEXÃO NO SOLO', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/PD8IJVrCIF0', unilateral:false},
  {nome:'ENCOLHIMENTO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/AprTVnp8Glc', unilateral:false},
  {nome:'EXTENSÃO DE OMBROS', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/86tIbUP2QDQ', unilateral:false},
  {nome:'ROSCA MARTELO ALTERNADA COM HALTERES', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/-FFDmkFHLNI', unilateral:false},
  {nome:'ABS FLEXÃO ALTERNADA E CURTA QUADRIL', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/48duQzz6s3o', unilateral:false},
  {nome:'CRUCIFIXO DEITADO', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Zu_EiENKB-s', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL VOADOR', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/xRvxuj-CYJU', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL COM HALTERS', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/8qBPQ9II95g', unilateral:false},
  {nome:'ROSCA DIRETA COM CORDA NA POLIA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Zvco9SHfvb4', unilateral:false},
  {nome:'TRÍCEPS CORDA N POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/8QoVDfqOfT0', unilateral:false},
  {nome:'PRANCHA VENTRAL', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/BDwijwkx3IM', unilateral:false},
  {nome:'VOADOR', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Xf-gNOsEAkk', unilateral:false},
  {nome:'ROSCA SCOTT UNILATERAL', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/eMuzfwJLMHA', unilateral:true},
  {nome:'TRÍCEPS APOIADO', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1XNQYUWWr4w', unilateral:false},
  {nome:'ABS TESOURINHA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ZamgsrcklEU', unilateral:false},
  {nome:'FACEPULL NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/nyD-Hl3ReJI', unilateral:false},
  {nome:'MANGUITO ROT. EXTERNA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/MizDY8p0gDY', unilateral:false},
  {nome:'ROSCA ALTERNADA COM HALTERES', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Tzz8CUv-LNk', unilateral:false},
  {nome:'ABS REMADOR CURTO C', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/FAvPpXN-UrQ', unilateral:false},
  {nome:'DESENVOLVIMENTO FRONTAL HALTERES', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/NwAXR-vZc4Y', unilateral:false},
  {nome:'ROSCA MARTELO COM CORDA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/81XRt_q084Y', unilateral:false},
  {nome:'TRÍCEPS SUPINADO', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/uEUz0NsQNjY', unilateral:false},
  {nome:'SOBE E DESCE CADEIRA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Nl9Ehzqqq6Q', unilateral:false},
  {nome:'FLEXÃO FECHADA NO SOLO', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/hDbCcR_vCnY', unilateral:false},
  {nome:'ROSCA MARTELO COM HALTERES', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/XX8i5RD3XCQ', unilateral:false},
  {nome:'TRÍCEPS COICE UNI NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/cQ7TeC_rdlg', unilateral:true},
  {nome:'PRANCHA ALTA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1L3sIFCFunU', unilateral:false},
  {nome:'PULLDOWN COM CORDA NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=H7jlJ-QxC9I', unilateral:false},
  {nome:'ROSCA MARTELO ALTERNADA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/CjfIwcLz5Xk', unilateral:false},
  {nome:'TRÍCEPS  UNI NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/udH-RZ2Wb2Y', unilateral:true},
  {nome:'ABS OBLÍCUO TOCANDO O PÉ', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/7Wv15aDEhVc', unilateral:false},
  {nome:'ROSCA ALTERNADA COM ISOMETRIA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/43EPuTFKvjQ', unilateral:false},
  {nome:'TRÍCEPS ROLDANA COM BARRA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Q5HfQ30Q9os', unilateral:false},
  {nome:'ABS QUADRIL CONTRA O SOLO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/tTwLVwhe8Bg', unilateral:false},
  {nome:'FLEXÃO DE BRAÇOS ADP', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/At33Ihk33pY', unilateral:false},
  {nome:'PUXADA ABERTA FRONTAL Y', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/SBk4080mWa4', unilateral:false},
  {nome:'ELEVAÇÃO LATERAL COM HALTERES', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/13giDFqI39I', unilateral:false},
  {nome:'FLEXÃO DE COTOVELO COM MOCHILA', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/X42BMTwuPMA', unilateral:false},
  {nome:'ABS ESCALADA LATERAL', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/rLAnLALPjjE', unilateral:false},
  {nome:'PUXADA COM TRIÂNGULO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Ps3_hCKtZ58', unilateral:false},
  {nome:'FLEXÃO COTOVELOS ROLDANA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/oOPtQW5lubk', unilateral:false},
  {nome:'TRÍCEPS TESTA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/bpp_l5Gdv7c', unilateral:false},
  {nome:'PRANCHA LATERAL', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/n9uoOzblcqk', unilateral:false},
  {nome:'SUPINO INCLINADO COM HALTERES', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/aCY-5vtz3n8', unilateral:false},
  {nome:'PUXADA FRONTAL BARRA FIXA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Ds5iplZZf9Q', unilateral:false},
  {nome:'ROSCA SCOTT', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/hFb3-9uXdHo', unilateral:false},
  {nome:'TRÍCEPS BANCO EM CASA', grupo:'Tríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/wkaPvUI9VBg', unilateral:false},
  {nome:'ABS SUPRA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/SEaT1STUVF8', unilateral:false},
  {nome:'CRUCIFIXO COM HALTERES', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/t5LWZvJErcA', unilateral:false},
  {nome:'PUXADA ABERTA SUPINADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/-7QIrHHcIO4', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL DE OMBROS', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/aVfjcMPZkJ4', unilateral:false},
  {nome:'ROSCA BÍCEPS ALTERNADA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/VbneTGKw5pE', unilateral:false},
  {nome:'TRÍCEPS CORDA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/V8OOh_Od_0o', unilateral:false},
  {nome:'ABS INFRA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ujwvd_T1R0c', unilateral:false},
  {nome:'CROSSOVER DIAGONAL', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/_g-x6FKny3U', unilateral:false},
  {nome:'PUXADA FRONTAL SUPINADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/VyIVo-I0rVo', unilateral:false},
  {nome:'TRÍCEPS COM BARRA NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/lglZg5PueKE', unilateral:false},
  {nome:'ABS INFRA CURTO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1bfJ7vBi_Tk', unilateral:false},
  {nome:'SUPINO RETO COM HALTERES', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Z4GP4FWN9YM', unilateral:false},
  {nome:'PUXADA NEUTRA BARRA FIXA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/zYNDXu3HxEw', unilateral:false},
  {nome:'ROSCA MARTELO NA POLIA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/KMpBtSwNRcw', unilateral:false},
  {nome:'MERGULHO NA PARALELA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/f3h0EtJIIhY', unilateral:false},
  {nome:'ABS ESCALADA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/cV6K_j5FLt8', unilateral:false},
  {nome:'PUXADA NEUTRA FRONTAL', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/TmG8s45O-BI', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL C BARRA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/dVpeHSXqbis', unilateral:false},
  {nome:'TRÍCEPS FRANCÊS BILATERAL', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/rT5dEQzyZy8', unilateral:false},
  {nome:'ABDOMINAL CANIVETE', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/kjoeOZKVxDM', unilateral:false},
  {nome:'REMADA ABE CURV C BARRA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/4rIMCTLueQA', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL NA POLIA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/AcbMzWL2E7Q', unilateral:false},
  {nome:'ROSCA DIRETA INCLINADA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/VnU96uihAJc', unilateral:false},
  {nome:'ABS REMADOR COMPLETO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/cPePOSFPFQQ', unilateral:false},
  {nome:'REMADA ABERTA C/ HALTER', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/HOD-Um14gFg', unilateral:false},
  {nome:'EXTENSÃO HORIZINTAL HALTER', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/54jt9FkDs_w', unilateral:false},
  {nome:'ROSCA 21 COM BARRA', grupo:'Bíceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/l-MktIlWRZg', unilateral:false},
  {nome:'ABS BICICLETA UNIL.', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/IQMbbBpLWaM', unilateral:false},
  {nome:'REMADA ABERTA COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/giQfgYKYrGc', unilateral:false},
  {nome:'ELEVAÇÃO LATERAL HALTERES', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/xaZ1GlPzse4', unilateral:false},
  {nome:'PRANCHA VENTRAL 3 APOIOS', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/vJkSlg0__A4', unilateral:true},
  {nome:'REMADA ABERTA ELÁSTICA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/soytMrqBEBs', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/x6kVaimzO7o', unilateral:false},
  {nome:'ELEVAÇÃO FRONTAL HALTERES', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/x6kVaimzO7o', unilateral:false},
  {nome:'ABS CRUZADO ALTO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/b1qXTPHPpjw', unilateral:false},
  {nome:'CROOS OVER', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/_g-x6FKny3U', unilateral:false},
  {nome:'REMADA ABERTA PRONADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Je-c5O5DR5c', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL UNI NA POLIA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/qCQOT8Py038', unilateral:true},
  {nome:'ABS REMADOR CURTO', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/FAvPpXN-UrQ', unilateral:false},
  {nome:'REMADA CURVADA SUPINADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/3VU_r-9snjg', unilateral:false},
  {nome:'PRANCHA LATERAL ADAP.', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/8l3N6o2SLMM', unilateral:false},
  {nome:'SUPINO INCLINADO HALTERES', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/TXXG_sevBMk', unilateral:false},
  {nome:'REMADA BAIXA NEUTRA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/gvRz36ZV9SQ', unilateral:false},
  {nome:'ELEVAÇÃO LATERAL', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/7CpesOm6Z5U', unilateral:false},
  {nome:'PRANCHA LATERAL DINÂMICA', grupo:'Abdômen', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Z-V-LAchIn0', unilateral:false},
  {nome:'REMADA BAIXA Y', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/iKW5nXARbMs', unilateral:false},
  {nome:'DESENVOLVIMENTRO FRONTAL COM MOCHILA', grupo:'Ombros', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/RcHEOOACjHk', unilateral:false},
  {nome:'MANGUITO UNILATERAL', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/286eFOvP_MI', unilateral:true},
  {nome:'TRÍCEPS BARRA NA POLIA', grupo:'Tríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Byff4QmSEc8', unilateral:false},
  {nome:'SUPINO RETO ARTICULAR', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/kn6zOZXNW1Q', unilateral:false},
  {nome:'REMADA CURVADA ABERTA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Je-c5O5DR5c', unilateral:false},
  {nome:'ELEVAÇÃO DIAGONAL H', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/7_9oQNFkqU0', unilateral:false},
  {nome:'REMADA CURVADA COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ReiU3emM_8o', unilateral:false},
  {nome:'FLY', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/L4YtdisHldk', unilateral:false},
  {nome:'REMADA CURVADA COM MOCHILA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/7nlABnGxDp0', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL POLIA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1MHzijrsAOY', unilateral:false},
  {nome:'SUPINO INCLINADO ARTICULAR', grupo:'Peito', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/oJkLYcqaA5E', unilateral:false},
  {nome:'REMADA FECHADA CURVADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/FeIxXCMuwUI', unilateral:false},
  {nome:'REMADA UNI AMPLITUDE ELEVADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/_I2d2Ij0p5E', unilateral:true},
  {nome:'REMADA UNI COM HALTERES', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/kS0KNWUgU7I', unilateral:true},
  {nome:'REMADA UNILATERAL COM MOCHILA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/i__eZEOAYEE', unilateral:true},
  {nome:'DESENVOLVIMENTO FRONTAL NA MÁQUINA', grupo:'Ombros', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/iT29kcK5KcY', unilateral:false},
  {nome:'EXTENSÃO HORIZONTAL NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1MHzijrsAOY', unilateral:false},
  {nome:'REMADA ABERTA COM LENÇOL', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/DKHa19UU4EU', unilateral:false},
  {nome:'PULLOVER', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Ba4xQwfBz6g', unilateral:false},
  {nome:'REMADA ARTICULAR ABERTA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ueWKZBDgQeA', unilateral:false},
  {nome:'REMADA ARTICULAR FECHADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Bjy5YMPzZao', unilateral:false},
  {nome:'PUXADA ABERTA FRONTAL', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/rubLhTeJWwA', unilateral:false},
  {nome:'PUXADA SUPINADA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/JROH1Y73n6E', unilateral:false},
  {nome:'REMADA BAIXA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/tGWs8IjRN9s', unilateral:false},
  {nome:'REMADA ABERTA NA POLIA', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/-AhBdO6ngdE', unilateral:false},
  {nome:'CRUCIFIXO INV COM APOIO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/IbhLDWWnGxc', unilateral:false},
  {nome:'REMADA CAVALINHO', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/MkfpH3Sl1lY', unilateral:false},
  {nome:'REMADA ABERTA NO TRX', grupo:'Costas', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/KoxP8668V1Y', unilateral:false},
  {nome:'TRÍCEPS FRAN BIL', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/rT5dEQzyZy8', unilateral:false},
  {nome:'PULLDOWN COM CORDA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/Zg9L-zYE0wI', unilateral:false},
  {nome:'REMADA ABERTA SUPINADA', grupo:'Costas', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/3VU_r-9snjg', unilateral:false},
  {nome:'EXTENSÃO LOMBAR DINÂMICA', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/y_5kPLWzrFQ', unilateral:false},
  {nome:'EXTENSÃO LOMBAR GUIADO', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/M7wFYedPnLc', unilateral:false},
  {nome:'SUPER MAN', grupo:'Bíceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/lhg93GRGkGQ', unilateral:false},
  {nome:'AFUNDO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/JM5sJCb1lsI', camada:'base', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'BOM DIA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ZG44uKbHK4Q', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'ABDUÇÃO DE QUADRIL NA POLIA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/NOkHTTQNqSk', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'FLEXÃO PLANTAR', grupo:'Panturrilha', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/bc12xKf4-9A', unilateral:false},
  {nome:'AFUNDO ALT. PLIOMÉTRICO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1tY7QZwhpGU', camada:'base', nivelMinimo:'Avançado', unilateral:true},
  {nome:'CADEIRA FLEXORA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Ve2BanEgCqY', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEXÃO PLANTAR NA MÁQUINA', grupo:'Panturrilha', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/bs3NdonHzhs', unilateral:false},
  {nome:'AFUNDO ALTERNADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/oY_nnRig9J4', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'CADEIRA ABDUTORA( ABRIR )', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/WMx7JeUTXJs', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AFUNDO C/FLEX DE QUADRIL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/OMQhwRW23ZI', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'CADEIRA ADUTORA ( FECHAR )', grupo:'Panturrilha', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/lzuSEi65_IQ', unilateral:false},
  {nome:'ELEVAÇÃO PÉLVICA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/O84nvpikZiU', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AFUNDO DIN. C FLEX QUADRIL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/OMQhwRW23ZI', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'ELEVAÇÃO PÉLVICA UNILATERAL', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/WM4Blto4Uq0', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AFUNDO GUIADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/xMTJPIEVLpo', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/uhfO-KWURBE', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'AFUNDO NA BARRA GUIADA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/EcvoioUwyH8', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO BÚLGARO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/GGvEWR0Y4Wc', camada:'base', nivelMinimo:'Avançado', unilateral:true},
  {nome:'STIFF UNILATERAL', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/6oPT1WncH8U', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO FRONTAL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/oMoGcwdus3w', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CAMA FLEXORA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ByKiWoif3iA', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL VARIAÇÃO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/2lb69Bohm_8', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AGACHAMENTO GUIADO P', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/3SXMi9dGQkM', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEXÕ NÓRDICA', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/DIRx__DoJqo', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'FLEX. DE JOELHOS NO SOLO', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/fWW4ASUCi_c', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO GUIADO Y', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ajf2fUekZr4', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEX. DE JOELHOS DE PÉ', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/-ixMlKa11bA', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO HACK', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/-qYxx4mRjoc', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'STIFF COM HALTERES', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/5rJy8qqQofQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'FLEX. JOELHOS ADP', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/6bE5P4x36-o', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO HACK Y', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/hqTvBjX4lgY', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'STIFF UNI SEM APOIO', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/E6QF1VWS1qY', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO LIVRE', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/3Mq1lLgE00g', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'STIFF UNI COM APOIO', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/GyDrpYEPWQw', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'AGACHAMENTO PC', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/pcfm6MBXeMg', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEXÃO DE JOELHOS EM PÉ', grupo:'Isquiotibiais', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/7H6O9-WYZ74', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'GLÚT. 4 APOIOS JOE. ESTENDIDO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/xb_lnHwwZE4', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'AGACHAMENTO PLIOM. ALTERNADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1tY7QZwhpGU', camada:'base', nivelMinimo:'Avançado', unilateral:false},
  {nome:'GLÚT. 4 APOIOS JOE. FLETIDO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/4M4f8ZBLFkA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'AGACHAMENTO PLIOMÉTRICO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/jg5y0pyv3Xo', camada:'base', nivelMinimo:'Avançado', unilateral:false},
  {nome:'STIFF', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/D5sRfuzL0e4', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO SUMÔ COM PESO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/l_t_84gmZQQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'TERRA ROMENO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/VpjkLAOz8CU', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO SUMÔ', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/4Gm4RDvDSNk', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'TERRA ISOLADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/E7CZQS-PRoo', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'LEVANTAMENTO TERRA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/EscCinkCoBI', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AJOELHA E LEVANTA', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/0Zgh3NIsMfc', unilateral:true},
  {nome:'ABDUÇÃO QUADRIL DEITADO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/yBAkhqWLmYE', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'AVANÇO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/l957uDIyzX0', unilateral:true},
  {nome:'ELEVAÇÃO PÉLVICA COM BARRA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/bkLj_jbodUE', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'BURP ADAPTADO', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/CtnnoxhzD7U', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CADEIRA EXTENSORA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Ox4ZtBUAGo4', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'ABDUÇÃO QUADRIL NO SOLO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=tTwLVwhe8Bg', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'CADEIRA EXTENSORA UNI', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/HRjzMl7a-IY', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'Glúteo 4 apoios com caneleiras', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/JmnIUWP5gno', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'Extensão de quadril  na polia com tronco inclinado', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/dvsgo4Gj-hA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'FLEXÃO DE QUADRIL DEITADO', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/GJ4DAuWxuVU', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'Extensão de quadril no banco romano', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/6ODl8JFyyYA', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'FLEXÃO DE QUADRIL EM PÉ', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/Cj7FbWvklPE', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'Abdução de quadril na polia com tronco inclinado', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtube.com/shorts/MpTUBFeZya8', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'LEG PRESS 45', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/HHYpoJ1l4Z0', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'Abdução de quadril na polia com rotação', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtube.com/shorts/HmcyJwfM92w', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'LEG PRESS 45 Y', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/V5Y0w8nbmdU', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'LEG PRESS HORIZONTAL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/0KLrU6JXOUE', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'CADEIRA ABDUTORA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/L0L9s4OTbOU', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'LEVANTAMENO TERRA EM CASA', grupo:'Quadríceps', ambiente:'Casa', nivel:'A definir', metodo:'', video:'https://youtu.be/Hgas-jdhnEQ', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'CADEIRA ABDUTORA-TRONCO A FRENTE', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/aKzeShloHJs', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'LEVANTAMENTO TERRA COM BARRA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/EscCinkCoBI', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'PASSADA COM HALTERES', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/1sbNNUMstns', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'extensão de quadril na máquina - variação', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/IWpwUAgwnpI', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'PASSADA LIVRE', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/4UIJPNFNHG0', camada:'base', nivelMinimo:'Intermediário', unilateral:true},
  {nome:'EXTENSÃO DE QUADRIL NA MÁQUINA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/0jzdxWcaT5I', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'SENTA E LEVANTA', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/iqK9m1CLnQE', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'EXTENSÃO DE QUADRIL COM CANELEIRAS NO BANCO', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/5BuJFrdn_AY', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'ELEVAÇÃO PÉLVICA NA MÁQUINA', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/K1QVmQb59rI', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'extensão de quadril curtinha no alto', grupo:'Glúteos', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/QXrJE4FF-TM', camada:'acessorio', nivelMinimo:'Iniciante', unilateral:true},
  {nome:'TERRA SUMÔ Y', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/ofnXsX-8aU8', camada:'base', nivelMinimo:'Intermediário', unilateral:false},
  {nome:'AGACHAMENTO SQUAT', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://www.youtube.com/watch?v=I3RAHGIS92I', camada:'base', nivelMinimo:'Iniciante', unilateral:false},
  {nome:'AGACHAMENTO BÚLGARO COM PESO CONTRALATERAL', grupo:'Quadríceps', ambiente:'Academia', nivel:'A definir', metodo:'', video:'https://youtu.be/RSULENERtug', camada:'base', nivelMinimo:'Avançado', unilateral:true}
];
...
function playExercicioVideo(nome){
  ...
  el.innerHTML =
    ...
    '<button class="btn-gold" style="background:var(--card-2);color:var(--gold-soft);border:1px solid var(--border);" onclick="abrirEdicaoExercicio(\'' + nome.replace(/'/g,"\\'") + '\')">Editar</button>' +
...
function confirmarEdicaoExercicio(nome){
  const e = exerciciosBanco.find(function(x){ return x.nome === nome; });
  if(!e) return;
  const novoNome = document.getElementById('edicao-nome').value.trim();
  if(!novoNome){ alert('O nome não pode ficar vazio.'); return; }
  const nomeAntigo = e.nome;
  e.nome = novoNome;
...
async function excluirEVoltarParaLista(nome){
...
const mobilidadeBanco = [
  {nome:'Mobilidade Quadril Extensão', trabalho:'Mobilidade /Alongamento', grupo:'Ilio Psoas', video:'https://youtu.be/NrF08RNhKyY', unilateral:false},
  {nome:'Mobilidade Quadril Extensão Diagonal', trabalho:'Mobilidade /Alongamento', grupo:'Ilio Psoas/ Adutores', video:'https://youtu.be/t-xiIUGzO3s', unilateral:false},
  {nome:'Alongamento Adutores Sentado', trabalho:'Alongamento', grupo:'Adutores', video:'https://youtu.be/v0gqJQNR96k', unilateral:false},
  {nome:'Mob.Quadril Adutores 4 Apoios', trabalho:'Mobilidade /Alongamento', grupo:'Adutores', video:'https://youtu.be/eT_hBpaNirQ', unilateral:true},
  {nome:'Alengamento Reto Abdômen', trabalho:'Alongamento', grupo:'Abdomen', video:'https://youtu.be/uw2lrqLXv-I', unilateral:false},
  {nome:'Mob. Quadrado Lombar', trabalho:'Alongamento', grupo:'Quadrado Lombar', video:'https://youtu.be/O2LUiv4LmMU', unilateral:false},
  {nome:'Mob. Glúteo Médio E Máximo', trabalho:'Alongamento', grupo:'Glúteos', video:'https://youtu.be/H_OAHdhD2Ho', unilateral:false},
  {nome:'Mob. Tensor De Fascia Lata', trabalho:'Alongamento', grupo:'Tensor Da Fáscia Lata', video:'https://youtu.be/HAXkyuefy2c', unilateral:false},
  {nome:'Mobilidade Isquiotibiais Em Pé', trabalho:'Alongamento', grupo:'Isquitibiais', video:'https://youtu.be/lmtoMuWbuw4', unilateral:false},
  {nome:'Mob.Deslocamento Lateral Elástico', trabalho:'Mobilidade', grupo:'Glúteo Méd, Mín', video:'https://youtu.be/RWsBO0rqkgU', unilateral:false},
  {nome:'Mob.Retração De Espcapulas', trabalho:'Mobilidade', grupo:'Cintura Escapular', video:'https://youtu.be/d15LLZ_Uxvo', unilateral:false},
  {nome:'Mob.Rotação Externa De Ombro', trabalho:'Mobilidade /Alongamento', grupo:'Cintura Escapular', video:'https://youtu.be/O6sxaHfOR6s', unilateral:false},
  {nome:'Mob. Tornozelo', trabalho:'Mobilidade /Alongamento', grupo:'Tornozelo', video:'https://youtu.be/zgg3NtHWUpc', unilateral:false},
  {nome:'Mob. Quadril Retração', trabalho:'Mobilidade', grupo:'Pelve', video:'https://youtu.be/Yb8-QOlWScM', unilateral:false},
  {nome:'Mob. Ombros Deitado', trabalho:'Mobilidade', grupo:'Cintura Escapular', video:'https://youtu.be/HwDBulMMpcE', unilateral:false},
  {nome:'Along. Isquitibiais Espaldar', trabalho:'Mobilidade', grupo:'Isquitibiais', video:'https://youtu.be/hfKEOGad5dc', unilateral:false},
  {nome:'Along. P Piriforme', trabalho:'Mobilidade', grupo:'Piriforme', video:'https://youtu.be/H_OAHdhD2Ho', unilateral:false},
  {nome:'Along. Peitoral M. Deltoides', trabalho:'Alongamento', grupo:'Peitoral E Deltoide', video:'https://youtu.be/Wh4dRJsVovQ', unilateral:false},
  {nome:'Wall Ball Slide', trabalho:'Mobilidade', grupo:'Serrátil Anterior', video:'https://youtu.be/SBaDnfc9SgE', unilateral:false},
  {nome:'Push Up Plus - Variação', trabalho:'Mobilidade', grupo:'Serrátil Anterior', video:'https://youtu.be/pdVvEmF97Rw', unilateral:false},
  {nome:'Push Up Plus  Com Barra', trabalho:'Mobilidade', grupo:'Serrátil Anterior', video:'https://youtu.be/wlxPLG--5oo', unilateral:false},
  {nome:'Push Up Plus Em 4 Apoios', trabalho:'Mobilidade', grupo:'Serrátil Anterior', video:'https://youtu.be/87IHSrAHdq8', unilateral:true}
];

const aquecimentoBanco = [
  {nome:'Skipping', video:'https://youtu.be/6F0ModFWzIg'},
  {nome:'Polichinelo', video:'https://youtu.be/6AIlJkFNrKA', unilateral:false}
];
...
// ===== RELATÓRIOS DA SEMANA (frequência, respostas/comentários, resumo de fechamento) =====
...
async function calcularRelatorioFrequenciaSemanal(){
  ...
  const linhas = ativas.map(function(a){
    const prog = getProgressoAluna(a.nome);
    const concluidos = contarDiasDeTreinoConcluidos(prog);
    return { nome: a.nome, concluidos: concluidos, total: totalDiasSemana };
  }).sort(function(x,y){ return x.concluidos - y.concluidos; }); // menor constância primeiro, chama mais atenção

  const totalConcluidos = linhas.reduce(function(s,l){ return s + l.concluidos; }, 0);
  const totalPossivel = linhas.length * totalDiasSemana;
  const pctGeral = totalPossivel > 0 ? Math.round((totalConcluidos / totalPossivel) * 100) : 0;

  return { linhas: linhas, totalConcluidos: totalConcluidos, totalPossivel: totalPossivel, pctGeral: pctGeral, dataGeracao: new Date().toISOString() };
}

function montarTextoRelatorioFrequencia(r){
  let texto = '📋 Relatório de frequência da semana\n\n';
  texto += 'Total geral: ' + r.totalConcluidos + ' de ' + r.totalPossivel + ' treinos (' + r.pctGeral + '%)\n\n';
  r.linhas.forEach(function(l){ texto += l.nome + ': ' + l.concluidos + '/' + l.total + '\n'; });
  return texto;
}

async function gerarRelatorioFrequenciaSemanal(){
  ...
  area.innerHTML = '<div class="info-box"><p class="txt" style="white-space:pre-wrap;font-size:12px;">' + window.__textoRelatorioFrequencia.replace(/\n/g,'<br>') + '</p></div>' +
    '<button class="btn-gold" style="background:#25D366;color:#fff;border:none;margin-top:8px;" onclick="enviarRelatorioWhatsApp(\'__textoRelatorioFrequencia\',\'resultado-envio-frequencia\')"><i class="ti ti-brand-whatsapp" style="margin-right:6px;"></i>Mandar pro meu WhatsApp</button>' +
    '<div id="resultado-envio-frequencia" style="margin-top:6px;"></div>';
}

// --- 2) Respostas e comentários (dúvidas pendentes + desconforto relatado) ---
function calcularRelatorioRespostas(){
  const ativas = alunasPersonal.filter(function(a){ return statusDoPlano(a) === 'ativas'; });
  const duvidas = [], feedbacksComDesconforto = [];
  ativas.forEach(function(a){
    const prog = getProgressoAluna(a.nome);
    (a.duvidasSinalizadas || []).filter(function(d){ return !d.resolvida; }).forEach(function(d){
      duvidas.push({ nome: a.nome, pergunta: d.pergunta });
    });
    (prog.feedbackTreino || []).slice(-3).forEach(function(f){
      if(f.desconforto) feedbacksComDesconforto.push({ nome: a.nome, dia: f.dia, exercicio: f.exercicio, escala: f.escalaDesconforto });
    });
  });
  return { duvidas: duvidas, feedbacksComDesconforto: feedbacksComDesconforto, dataGeracao: new Date().toISOString() };
}

function montarTextoRelatorioRespostas(r){
  let texto = '💬 Relatório de respostas e comentários\n\n';
  texto += r.duvidas.length > 0
    ? '❓ Dúvidas pendentes (' + r.duvidas.length + '):\n' + r.duvidas.map(function(d){ return '- ' + d.nome + ': "' + d.pergunta + '"'; }).join('\n')
    : '❓ Nenhuma dúvida pendente no momento.';
  texto += '\n\n';
  texto += r.feedbacksComDesconforto.length > 0
    ? '⚠️ Desconforto relatado (' + r.feedbacksComDesconforto.length + '):\n' + r.feedbacksComDesconforto.map(function(f){ return '- ' + f.nome + ' (' + f.dia + '): ' + f.exercicio + ' · intensidade ' + (f.escala != null ? f.escala + '/10' : 'não informada'); }).join('\n')
    : '⚠️ Nenhum desconforto relatado recentemente.';
  return texto;
}

async function gerarRelatorioRespostas(){
  const area = document.getElementById('relatorio-respostas-area');
  if(!area) return;
  area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Reunindo respostas e comentários...</p>';
  const r = calcularRelatorioRespostas();
  window.__textoRelatorioRespostas = montarTextoRelatorioRespostas(r);
  await salvarCatalogoPersonal('relatorio_respostas', r.dataGeracao, r);
  area.innerHTML = '<div class="info-box"><p class="txt" style="white-space:pre-wrap;font-size:12px;">' + window.__textoRelatorioRespostas.replace(/\n/g,'<br>') + '</p></div>' +
    '<button class="btn-gold" style="background:#25D366;color:#fff;border:none;margin-top:8px;" onclick="enviarRelatorioWhatsApp(\'__textoRelatorioRespostas\',\'resultado-envio-respostas\')"><i class="ti ti-brand-whatsapp" style="margin-right:6px;"></i>Mandar pro meu WhatsApp</button>' +
    '<div id="resultado-envio-respostas" style="margin-top:6px;"></div>';
}

// --- 3) Resumo de fechamento da semana ---
function calcularResumoFechamentoSemana(){
  const ativas = alunasPersonal.filter(function(a){ return statusDoPlano(a) === 'ativas'; });
  const totalDiasSemana = totalDiasDeTreino();
  let precisamAtencao = 0, precisamThiago = 0;
  const problemas = { frequencia: 0, exercicio: 0, alimentacao: 0, revisarTreino: 0 };
  const nomesAtencao = [], nomesThiago = [];

  ativas.forEach(function(a){
    const prog = getProgressoAluna(a.nome);
    const concluidos = contarDiasDeTreinoConcluidos(prog);
    const pct = totalDiasSemana > 0 ? concluidos / totalDiasSemana : 1;
    let precisaAtencao = false;

    if(pct < 0.5){ precisaAtencao = true; problemas.frequencia++; }
    if(detectarRiscoAbandonoPorConstancia(a.nome)) precisaAtencao = true;

    const duvidasPendentes = (a.duvidasSinalizadas || []).filter(function(d){ return !d.resolvida; });
    if(duvidasPendentes.length > 0){ precisaAtencao = true; problemas.revisarTreino++; }

    const feedbacksRecentes = (prog.feedbackTreino || []).slice(-3);
    if(feedbacksRecentes.some(function(f){ return f.desconforto; })){ precisaAtencao = true; problemas.exercicio++; }

    const semanaAtualNutri = prog.nutricao && prog.nutricao[prog.semana];
    if(semanaAtualNutri && semanaAtualNutri.fugas >= 3){ precisaAtencao = true; problemas.alimentacao++; }

    let precisaThiago = false;
    if(a.sinalRiscoEmocional && !a.sinalRiscoEmocional.visto) precisaThiago = true;
    if(duvidasPendentes.length >= 2) precisaThiago = true;

    if(precisaThiago){ precisamThiago++; nomesThiago.push(a.nome); }
    else if(precisaAtencao){ precisamAtencao++; nomesAtencao.push(a.nome); }
  });

  return {
    totalAtivas: ativas.length, precisamAtencao: precisamAtencao, precisamThiago: precisamThiago,
    nomesAtencao: nomesAtencao, nomesThiago: nomesThiago, problemas: problemas, dataGeracao: new Date().toISOString()
  };
}

function montarTextoResumoFechamentoSemana(r){
  let texto = '📅 Fechamento da semana\n\n';
  texto += r.totalAtivas + ' alunas acompanhadas normalmente\n';
  texto += r.precisamAtencao + ' precisam de atenção\n';
  texto += r.precisamThiago + ' precisam falar com você\n\n';
  if(r.nomesThiago.length > 0) texto += '🔴 Precisam de você: ' + r.nomesThiago.join(', ') + '\n\n';
  if(r.nomesAtencao.length > 0) texto += '🟡 Precisam de atenção: ' + r.nomesAtencao.join(', ') + '\n\n';
  texto += 'Principais problemas relatados:\n';
  texto += '- ' + r.problemas.frequencia + ' aluna(s) com dificuldade na frequência\n';
  texto += '- ' + r.problemas.exercicio + ' aluna(s) relataram desconforto em exercício\n';
  texto += '- ' + r.problemas.alimentacao + ' aluna(s) com dificuldade na alimentação\n';
  texto += '- ' + r.problemas.revisarTreino + ' aluna(s) com dúvida pendente pra revisar';
  return texto;
}

async function gerarResumoFechamentoSemana(){
  const area = document.getElementById('resumo-fechamento-area');
  if(!area) return;
  area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Fechando a semana...</p>';
  const r = calcularResumoFechamentoSemana();
  window.__textoResumoFechamento = montarTextoResumoFechamentoSemana(r);
  await salvarCatalogoPersonal('resumo_fechamento_semana', r.dataGeracao, r);
  area.innerHTML = '<div class="info-box"><p class="txt" style="white-space:pre-wrap;font-size:12px;">' + window.__textoResumoFechamento.replace(/\n/g,'<br>') + '</p></div>' +
    '<button class="btn-gold" style="background:#25D366;color:#fff;border:none;margin-top:8px;" onclick="enviarRelatorioWhatsApp(\'__textoResumoFechamento\',\'resultado-envio-resumo\')"><i class="ti ti-brand-whatsapp" style="margin-right:6px;"></i>Mandar pro meu WhatsApp</button>' +
    '<div id="resultado-envio-resumo" style="margin-top:6px;"></div>';
}

// Compartilhado pelos 3 relatórios acima, manda pro WhatsApp do próprio Thiago (TELEFONE_PERSONAL)
async function enviarRelatorioWhatsApp(chaveTexto, idResultado){
  const area = document.getElementById(idResultado);
  const texto = window[chaveTexto];
  if(!texto) return;
  if(area) area.innerHTML = '<p class="txt" style="font-size:11px;color:var(--text-faint);">Enviando...</p>';
  const saida = await enviarWhatsApp(TELEFONE_PERSONAL, texto);
  if(area) area.innerHTML = saida.sucesso
    ? '<p class="txt" style="font-size:11px;color:var(--success);">✓ Enviado.</p>'
    : '<p class="txt" style="font-size:11px;color:#C9784A;">Não consegui enviar: ' + (saida.motivo || 'motivo não informado') + '</p>';
}

// ===== DESTAQUES DA SEMANA (conquistas visíveis pra todas no Ranking) =====
function detectarDestaquesSemana(){
  const ativas = alunasPersonal.filter(function(a){ return statusDoPlano(a) === 'ativas'; });
  const totalDiasSemana = totalDiasDeTreino();
  const destaques = [];
  ...
  ativas.forEach(function(a){
    ...
    // Progressão de carga essa semana
    const exerciciosComProgressao = [];
    Object.keys(prog.historico || {}).forEach(function(ex){
      prog.historico[ex].forEach(function(r){
        if(r.semana === prog.semana && r.sugestao && r.sugestao.texto === 'Aumentar carga') exerciciosComProgressao.push(ex);
      });
    });
    if(exerciciosComProgressao.length > 0){
      destaques.push(primeiroNome + ' aumentou carga em ' + exerciciosComProgressao.slice(0,2).join(' e ') + (exerciciosComProgressao.length > 2 ? ' e outros exercícios' : '') + ' essa semana! 💪');
    }

    // Consistência de semanas seguidas completas
    let semanasSeguidasCompletas = 0;
    for(let s = prog.semana; s >= 1; s--){
      const concluidos = (prog.diasConcluidos[s] || []).length;
      if(totalDiasSemana > 0 && concluidos >= totalDiasSemana) semanasSeguidasCompletas++; else break;
    }
    if(semanasSeguidasCompletas >= 4){
      destaques.push(primeiroNome + ' completou ' + semanasSeguidasCompletas + ' semanas seguidas de consistência! 🔥');
    }

    // Composição corporal atualizada recentemente (últimos 7 dias)
    if(a.composicaoAtual && a.composicaoAtual.data){
      const dataAtualizacao = converterDataBrParaDate(a.composicaoAtual.data);
      if(dataAtualizacao && (new Date() - dataAtualizacao) < 7*24*60*60*1000){
        destaques.push(primeiroNome + ' registrou nova composição corporal essa semana! 📸');
      }
    }
  });
...
async function gerarEPublicarDestaquesSemana(){
  const area = document.getElementById('destaques-semana-area');
  if(!area) return;
  area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Procurando conquistas da semana...</p>';
  const destaques = detectarDestaquesSemana();
  if(destaques.length === 0){
    area.innerHTML = '<div class="info-box"><p class="txt">Nenhuma conquista nova detectada essa semana ainda.</p></div>';
    return;
  }
  await salvarCatalogoPersonal('destaques_semana', new Date().toISOString(), { destaques: destaques, dataGeracao: new Date().toISOString() });
  destaquesSemanaAtual = destaques;
  area.innerHTML = '<div class="info-box"><p class="lbl" style="color:var(--success);">✓ ' + destaques.length + ' publicado(s) no Ranking</p>' +
    destaques.map(function(d){ return '<p class="txt" style="margin-bottom:4px;">• ' + d + '</p>'; }).join('') +
  '</div>';
}
...
async function loginAluna(){
  ...
  try {
    ...
    let { data: alunaRow, error: erroAlunaRow } = await supabaseClient.from('alunas').select('*').eq('auth_id', data.user.id).maybeSingle();

    // Recuperação automática: se não achou pelo auth_id, tenta achar pelo e-mail — cobre o caso de
    // uma conta de login criada sem o vínculo ter sido salvo corretamente (ex: falha de rede no meio
    // do primeiro cadastro). Se achar por e-mail, conserta o vínculo na hora, sem precisar travar
    // o acesso dela nem depender do Personal pra corrigir manualmente.
    if((erroAlunaRow || !alunaRow) && email){
      const tentativaPorEmail = await supabaseClient.from('alunas').select('*').eq('email', email).maybeSingle();
      if(tentativaPorEmail.data){
        alunaRow = tentativaPorEmail.data;
        erroAlunaRow = null;
        await supabaseClient.from('alunas').update({ auth_id: data.user.id }).eq('email', email);
      }
    }
...
// Antes, quem respondia "não salvar" ficava presa nessa escolha pra sempre, sem jeito de mudar de
// ideia depois. Esse botão (perto de "Sair da conta") deixa ativar ou desativar isso quando quiser.
function alternarPreferenciaLembrarLogin(){
...
async function loginPersonal(){
  ...
  if(encontrarContaPersonal(email, senha)){
    ...
    if(supabaseClient){
      try {
        let resultado = await supabaseClient.auth.signInWithPassword({ email: email, password: senha });
        if(resultado.error){
          // Conta ainda não existe de verdade no Supabase (ex: acesso novo, nunca logado antes) —
          // cria agora, com o nome certo dessa conta específica, em vez de deixar entrar sem sessão real.
          const contaValida = encontrarContaPersonal(email, senha);
          const cadastro = await supabaseClient.auth.signUp({ email: email, password: senha });
          if(!cadastro.error){
            await supabaseClient.from('perfis').insert({ id: cadastro.data.user.id, tipo: 'personal', nome: contaValida.nome });
            resultado = cadastro;
            statusSessao = 'OK, conta criada agora e sessão real estabelecida.';
          } else {
            statusSessao = 'FALHOU ao criar conta: ' + cadastro.error.message;
          }
        } else {
          statusSessao = 'OK, sessão real estabelecida.';
        }
        if(resultado.data && resultado.data.user) sessaoUsuarioAtual = resultado.data.user;
      } catch(erroDeRede){
...
async function renderListaConversas(){
  ...
  voltar.onclick = function(){ showPersonalView('dashboard'); };
  document.getElementById('area-conversa-aberta').innerHTML = '';

  const area = document.getElementById('area-lista-conversas');
  if(!area) return;
  area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Carregando conversas...</p>';
  if(!supabaseClient){ area.innerHTML = '<div class="info-box"><p class="txt">Sem conexão com o banco agora.</p></div>'; return; }
  try {
    const { data, error } = await supabaseClient.from('mensagens_whatsapp').select('*').order('data_hora', { ascending: false });
    if(error){ area.innerHTML = '<div class="info-box"><p class="txt">Não consegui carregar as conversas agora.</p></div>'; return; }
    if(!data || data.length === 0){ area.innerHTML = '<div class="info-box"><p class="txt" style="color:var(--text-faint);">Nenhuma conversa registrada ainda.</p></div>'; return; }

    // Já vem do mais novo pro mais velho, então a 1ª ocorrência de cada telefone é a mensagem mais recente dele
    const porTelefone = {};
    data.forEach(function(m){
      if(!porTelefone[m.telefone]) porTelefone[m.telefone] = { nome: m.aluna_nome || m.telefone, ultima: m, telefone: m.telefone };
    });
    const lista = Object.values(porTelefone).sort(function(a, b){ return new Date(b.ultima.data_hora) - new Date(a.ultima.data_hora); });

    area.innerHTML = lista.map(function(c){
      const dataFormatada = new Date(c.ultima.data_hora).toLocaleString('pt-BR', { day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit' });
      const prefixo = c.ultima.direcao === 'enviada' ? 'Você: ' : '';
      return '<div class="list-item" style="cursor:pointer;flex-direction:column;align-items:stretch;margin-bottom:8px;" onclick="abrirConversaCompleta(\'' + c.telefone + '\',\'' + c.nome.replace(/'/g,"\\'") + '\')">' +
        '<div style="display:flex;justify-content:space-between;"><span style="font-weight:600;">' + c.nome + '</span><span class="tag">' + dataFormatada + '</span></div>' +
        '<p class="txt" style="font-size:12px;color:var(--text-faint);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">' + prefixo + c.ultima.texto + '</p>' +
      '</div>';
    }).join('');
  } catch(erro){
  ...
  try {
    ...
    area.innerHTML += data.map(function(m){
      ...
      return '<div style="display:flex;justify-content:' + (ehEnviada ? 'flex-end' : 'flex-start') + ';margin-bottom:8px;">' +
        '<div style="max-width:75%;background:' + (ehEnviada ? 'var(--gold-soft)' : 'var(--card-2)') + ';color:' + (ehEnviada ? '#1A1409' : 'var(--text)') + ';border-radius:14px;padding:8px 12px;">' +
          '<p style="font-size:13px;margin:0;">' + m.texto + '</p>' +
          '<p style="font-size:9px;margin:4px 0 0;opacity:0.7;text-align:right;">' + hora + '</p>' +
        '</div>' +
      '</div>';
    }).join('');
  } catch(erro){
  ...
  if(container.style.display === 'none' || !container.innerHTML){
    ...
    container.innerHTML = opcoes.length === 0
      ...
      : '<div class="info-box" style="margin:0 0 8px;padding:10px 12px;">' +
          ...
          opcoes.map(function(e){ return '<p class="txt" style="margin-bottom:4px;cursor:pointer;text-decoration:underline;text-decoration-color:var(--gold-soft);text-underline-offset:2px;" onclick="abrirVideoApenasExercicio(\'' + e.nome.replace(/'/g,"\\'") + '\', event)">· ' + capitalizarNomeExercicio(e.nome) + '</p>'; }).join('') +
...
async function carregarTreinosDeTodasAtivas(){
  ...
  const semTreinoCarregado = alunasPersonal.filter(function(a){
    return statusDoPlano(a) === 'ativas' && !a.treinoAtual && a.email;
  });
...
function renderMetricasNegocio(){
  ...
  const htmlStats =
    '<div class="alert-row" style="grid-template-columns:1fr;margin-bottom:6px;">' +
...
function renderFaturamentoMetas(){
  const area = document.getElementById('faturamento-metas-area');
  if(!area) return;
  const m = calcularMetricasNegocio();
  const faturamentoFormatado = m.faturamentoMes.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  area.innerHTML =
    '<div class="alert-row" style="grid-template-columns:1fr;margin-bottom:14px;">' +
...
async function renderCentralDeAvisos(){
  const area = document.getElementById('central-avisos-area');
  if(!area) return;
  const dados = calcularCentralDeAvisos();

  function linhaAviso(titulo, lista, tipo){
    if(lista.length === 0) return ''; // linha zerada não ocupa espaço à toa
    return '<div class="list-item" style="margin-bottom:4px;padding:8px 10px;">' +
      '<span style="font-size:12px;">' + titulo + ' <span class="tag" style="font-size:10px;padding:1px 6px;">' + lista.length + '</span></span>' +
      '<span class="acao-pill" style="font-size:11px;padding:4px 10px;" onclick="enviarAvisosEmLote(\'' + tipo + '\')">Enviar</span>' +
    '</div>';
  }

  const rankingAtual = await calcularRankingComunidade();
  ...
  const linhasAvisos = linhaAviso('Risco de abandono', dados.risco, 'riscoAbandono') +
  ...
  area.innerHTML =
    (linhasAvisos || '<p class="txt" style="font-size:12px;color:var(--text-faint);">Nenhum aviso pendente agora.</p>') +
...
async function avisarDataEspecial(tipo){
  ...
  for(let i = 0; i < elegiveis.length; i++){
    area.innerHTML = '<p class="txt" style="color:var(--text-faint);">Enviando ' + (i+1) + ' de ' + elegiveis.length + '... (de propósito, com pausa entre cada uma, pra proteger seu WhatsApp de bloqueio)</p>';
...
async function enviarAvisosEmLote(tipo){
  ...
  for(let i = 0; i < lista.length; i++){
    const a = lista[i];
    resultadoArea.innerHTML = '<p class="txt" style="color:var(--text-faint);">Enviando ' + (i+1) + ' de ' + lista.length + '... (de propósito, com pausa entre cada uma, pra proteger seu WhatsApp de bloqueio)</p>';
...
// Pausa segura entre mensagens em lote pro WhatsApp: intervalo aleatório entre 4 e 8 segundos,
...
async function enviarWhatsApp(telefone, mensagem, jaTentouDeNovo){
  ...
  try {
    ...
    if(!resposta.ok || dados.error){
      const motivo = dados.error || ('HTTP ' + resposta.status);
      // Antes só tentava de novo se caísse a conexão. Mas o WhatsApp também pode recusar por excesso
      // de velocidade (limite de taxa) e isso não é exceção de rede, é uma resposta "normal" com erro
      // — então também merece uma segunda chance, com uma pausa maior, antes de desistir de vez.
      if(!jaTentouDeNovo){
...
// no banco que soma os pontos sem expor nenhum dado sensível de ninguém pra fora do Supabase.
async function calcularRankingComunidade(){
  const pontosDela = calcularPontosAluna(NOME_ALUNA_LOGADA);
  let pontosTodas = [pontosDela]; // se a busca da comunidade falhar, mostra pelo menos o dela mesma

  if(supabaseClient && sessaoUsuarioAtual){
    try {
      const { data, error } = await supabaseClient.rpc('obter_ranking_comunidade');
      if(!error && data){
        pontosTodas = data
          .filter(function(r){ return r.aluna_id !== sessaoUsuarioAtual.id; }) // tira a linha dela, evita contar 2x
          .map(function(r){ return r.pontos; })
          .filter(function(p){ return p > 0; });
        pontosTodas.push(pontosDela); // usa sempre o valor local dela, o mais atualizado possível
      }
    } catch(erroDeRede){
      console.warn('Não consegui buscar o ranking real da comunidade agora, mostrando só o que sei localmente:', erroDeRede);
    }
  }

  const totalComunidade = pontosTodas.reduce(function(soma, p){ return soma + p; }, 0);
  const posicaoDela = pontosTodas.filter(function(p){ return p > pontosDela; }).length + 1;
...
async function carregarDestaquesSemanaRecentes(){
  if(!supabaseClient) return [];
  try {
    const { data, error } = await supabaseClient.from('catalogo_personal').select('dados').eq('tipo', 'destaques_semana').order('updated_at', { ascending: false }).limit(1).maybeSingle();
    if(error || !data || !data.dados) return [];
    return data.dados.destaques || [];
  } catch(erroDeRede){
    console.warn('Não consegui buscar os destaques da semana agora:', erroDeRede);
    return [];
  }
}
...
async function renderRanking(){
  ...
  // Mostra na hora com o que já sabe localmente, enquanto busca o resto da comunidade de verdade
  const pontosDelaAgora = calcularPontosAluna(NOME_ALUNA_LOGADA);
  container.innerHTML = '<div class="dna-score-card"><div class="dna-score-left">' +
    '<p class="dna-score-label">SEUS PONTOS</p>' +
    '<p class="dna-score-numero">' + pontosDelaAgora + '</p>' +
    '<p class="dna-score-comparativo" style="color:var(--text-faint);">Calculando sua posição na comunidade...</p>' +
  '</div></div>';

  const [r, destaques] = await Promise.all([calcularRankingComunidade(), carregarDestaquesSemanaRecentes()]);
  ...
  if(destaques.length > 0){
    html += '<p class="section-label" style="margin-top:20px;">Destaques da semana</p>' +
      destaques.map(function(d){ return '<div class="info-box" style="margin-bottom:8px;"><p class="txt" style="margin-bottom:0;">✨ ' + d + '</p></div>'; }).join('');
  }
...
function renderMeuProgresso(){
  ...
  dataEmEdicaoCheckIn = null; // toda vez que reabre a aba, volta pro modo "hoje" por padrão
...
function getRodaDaVidaMes(nome, mesISO){
  ...
  if(!prog.rodaDaVida[mesISO]) prog.rodaDaVida[mesISO] = { mes: mesISO, areas: {}, finalizado: false, dataFinalizacao: null };
...
// Acha a finalização mais recente (não importa o mês-calendário em que caiu), pra calcular
// os 30 dias a partir de quando ela realmente preencheu, não do calendário fixo.
function getUltimaFinalizacaoRodaDaVida(nome){
  const prog = getProgressoAluna(nome);
  if(!prog.rodaDaVida) return null;
  const finalizados = Object.values(prog.rodaDaVida).filter(function(r){ return r.finalizado && r.dataFinalizacao; });
  if(finalizados.length === 0) return null;
  finalizados.sort(function(a, b){ return new Date(b.dataFinalizacao) - new Date(a.dataFinalizacao); });
...
// Finaliza o preenchimento do mês atual: trava edição e inicia a contagem real de 30 dias
// a partir de hoje, não do calendário.
function finalizarRodaDaVida(nome){
  const mesISO = getMesAtualISO();
  const registro = getRodaDaVidaMes(nome, mesISO);
...
let mostrandoHistoricoRodaDaVida = false;
let mesHistoricoRodaDaVidaAberto = null;

function alternarHistoricoRodaDaVida(){
  mostrandoHistoricoRodaDaVida = !mostrandoHistoricoRodaDaVida;
  mesHistoricoRodaDaVidaAberto = null;
  renderRodaDaVidaAluna();
}

function abrirMesHistoricoRodaDaVida(mesISO){
  mesHistoricoRodaDaVidaAberto = mesISO;
  renderRodaDaVidaAluna();
}
...
function renderRodaDaVidaAluna(){
  ...
  const diasRestantes = diasParaProximaRodaDaVida(nome);

  // Vendo o histórico de meses anteriores (lista ou um mês específico aberto)
  if(mostrandoHistoricoRodaDaVida){
    const mesesFinalizados = Object.keys(prog.rodaDaVida || {}).filter(function(m){ return prog.rodaDaVida[m].finalizado; }).sort();
    let html = '<p class="section-label" style="margin-top:0;cursor:pointer;" onclick="alternarHistoricoRodaDaVida()"><i class="ti ti-arrow-left" style="font-size:14px;vertical-align:-2px;margin-right:6px;"></i>Voltar</p>';
    if(mesHistoricoRodaDaVidaAberto && prog.rodaDaVida[mesHistoricoRodaDaVidaAberto]){
      const registro = prog.rodaDaVida[mesHistoricoRodaDaVidaAberto];
      const nomeDoMes = new Date(mesHistoricoRodaDaVidaAberto + '-01T00:00:00').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
      html += '<p class="lbl" style="margin-bottom:8px;">' + nomeDoMes.charAt(0).toUpperCase() + nomeDoMes.slice(1) + '</p>' +
        '<div class="info-box">' + gerarSvgRadar(registro.areas) + '</div>';
    } else if(mesesFinalizados.length === 0){
      html += '<p class="txt" style="color:var(--text-faint);">Ainda não tem nenhum mês anterior salvo.</p>';
    } else {
      html += '<p class="lbl" style="margin-bottom:8px;">Escolha um mês</p>';
      mesesFinalizados.slice().reverse().forEach(function(m, i){
        const nomeDoMes = new Date(m + '-01T00:00:00').toLocaleDateString('pt-BR', { month: 'long', year: 'numeric' });
        html += '<div class="list-item" style="cursor:pointer;margin-bottom:6px;" onclick="abrirMesHistoricoRodaDaVida(\'' + m + '\')"><span>' + (nomeDoMes.charAt(0).toUpperCase() + nomeDoMes.slice(1)) + '</span><i class="ti ti-chevron-right" style="color:var(--text-faint);"></i></div>';
      });
    }
    container.innerHTML = html;
    return;
  ...
  // Trava de 30 dias: mostra só o resumo do último preenchimento, sem permitir editar
  if(diasRestantes > 0){
    const ultima = getUltimaFinalizacaoRodaDaVida(nome);
    const resumo = ultima ? calcularPontoForteEAreaFraca(ultima.areas) : null;
    let html = '<p class="section-label" style="margin-top:0;">Sua última Roda da Vida</p>' +
      '<div class="info-box" style="margin-bottom:12px;">' + gerarSvgRadar(ultima.areas) + '</div>';
    if(resumo){
    ...
    html += '<div class="info-box" style="text-align:center;margin-bottom:12px;"><p class="txt" style="font-size:12px;color:var(--text-faint);">Um novo preenchimento libera em <b style="color:var(--gold-soft);">' + diasRestantes + ' dia(s)</b></p></div>' +
      '<span class="chip" style="cursor:pointer;" onclick="alternarHistoricoRodaDaVida()">Ver meses anteriores</span>';
    container.innerHTML = html;
    return;
  }
...
function renderMeuProgressoConteudo(container, nome, a){
  ...
  html += '<div id="checkin-habitos-area" style="margin-top:24px;"></div>';
...
if(type === 'central'){
    ...
    if(d.tabata){
      ...
      corpoTreino = '<div class="badge">' + faseAtual + '</div>' +
        '<button class="btn-gold" style="padding:16px;font-size:15px;margin:10px 0 4px;" id="cronometro-btn" onclick="alternarCronometro()">' + (cronometroRodando ? 'Pausar' : (cronometroSegundos > 0 ? 'Retomar' : 'Iniciar treino')) + '</button>' +
        '<p style="text-align:center;font-size:11px;color:var(--text-faint);margin:0 0 4px;" id="cronometro-display" data-modo-cronometro="1">' + String(Math.floor(cronometroSegundos/60)).padStart(2,'0') + ':' + String(cronometroSegundos%60).padStart(2,'0') + '</p>' +
        '<p style="text-align:center;font-size:11.5px;color:var(--text-faint);margin:0 0 14px;">Clique no nome de cada exercício pra ver o vídeo de execução</p>' +
      ...
      d.ex.forEach(function(linha, j){
        ...
        const conteudoExpandido = playerVideo +
          ...
          '</div>' +
          '<button class="btn-gold" style="margin-top:8px;padding:8px;font-size:12px;" onclick="confirmarSerieExercicio(\'' + j + '\',\'' + nomeEx.replace(/'/g,"\\'") + '\')">Confirmar</button>' +
  ...
  try {
    ...
    d.ex.forEach(function(linha, j){
      ...
      nomesEIndices.forEach(function(item){
        ...
        const sugestao = sugerirAjusteCarga(carga, reps);
        const registro = { semana: prog.semana, carga: carga, reps: reps, sugestao: sugestao };
        const idxExistente = prog.historico[item.nome].findIndex(function(r){ return r.semana === prog.semana; });
        if(idxExistente !== -1) prog.historico[item.nome][idxExistente] = registro; // já tinha confirmado esse exercício antes, substitui em vez de duplicar
        else prog.historico[item.nome].push(registro);
